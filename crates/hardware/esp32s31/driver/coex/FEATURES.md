...
| Timer clock conversion | IMPLEMENTED | [Clock model](src/clock.rs) implements four selector domains and reviewed integer conversion rules. [Wi-Fi HAL](../../hal/src/ieee80211/mac.rs) can sample the custom PAC shared-clock fields in production; this clock access does not compose the validation-only timer bridge or a coexistence runtime. |
