...
fn run(fixture: &Fixture, id: &str, outcome: &str, time: u64, application: &[u8]) -> PathBuf {
    ...
    for path in ["driver.rs", "Cargo.lock"] {
    ...
    write(
        ...
        &json!({"schema":1,"snapshot_id":digest(&serde_json::to_vec(&captured).unwrap()),"archive_sha256":sha256_file(&directory.join("sources.tar")).unwrap(),"files":2}),
    );
    ...
    build["parameters"] = json!({"image":"correctness","runtime_profile":"test","runtime_features":"test","target":"test"});
    build["subjects"] =
...
fn reference(index: &HilEvidenceIndex, run: &str) -> ObservationRef {
    let observation = index.scenarios["exchange"]
        ...
        .unwrap();
    ObservationRef {
        id: observation.observation_id("exchange").unwrap(),
