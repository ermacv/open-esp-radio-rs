...
| Scanning | LOWER SCHEDULER LIFECYCLE CLOSED | The restricted passive LE 1M graph owns two RX nodes, one link state, one scheduler context and three scheduler items in pinned Controller SRAM. Production cold start now claims a fourth, scanner-specific static arena and lends its sole graph to the task service; DTM and advertising memory cannot alias it. Its first CPU-owned candidate enters the same common timeline admission and second-deadline authorization as the other radio roles; only the overlap-resolved reservation is encoded into the private scanner item. That reservation remains affine through list-zero merge, selector-one RX/command/head/interrupt/RUN publication, fenced completion, exact head retirement and the serialized post-unlink removal gate. RX extraction rejects stale producer/epoch sentinels and non-contiguous completion, copies only the bounded on-air PDU plus signed RSSI, and atomically returns both private lists and the timeline slot. Pre-publication cancellation returns the graph and reservation without leaking either owner. The vendor dynamic-manager epoch is intentionally replaced by the stronger sole static affine graph epoch. No upper layer constructs a register image or SRAM word. Portable LL now validates legacy scan interval/window domains, parses reportable advertising-channel PDUs into owned protocol reports and provides a bounded exact duplicate filter. The live time-acquisition/runner and standard `bt-hci` scan commands/events remain missing, so this is not yet an operational scanner. |
