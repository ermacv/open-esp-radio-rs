...
use crate::{net80211_encap::descriptor_priority_byte, net80211_state::Net80211InterfaceRole};
...
pub(crate) struct DescriptorPolicyInput {
    pub role: Net80211InterfaceRole,
    pub priority: u8,
...
pub(crate) const fn ordinary_descriptor_policy(
    ...
    let (selector, index) = match input.role {
        Net80211InterfaceRole::Station => (0, 0),
        Net80211InterfaceRole::AccessPoint => (1, input.node_rate_context_index_0x26),
    };
    ...
    if matches!(input.role, Net80211InterfaceRole::Station) {
    ...
    let byte_0x38 = match input.role {
        Net80211InterfaceRole::Station if input.node_byte_0x38c != 0 => 1,
        Net80211InterfaceRole::Station => input.node_byte_0x45a & 1,
        Net80211InterfaceRole::AccessPoint => 0,
    };
...
mod tests {
    ...
    const BASE: DescriptorPolicyInput = DescriptorPolicyInput {
        role: Net80211InterfaceRole::Station,
        priority: 5,
