...
pub(super) fn with_authorized_ap(test: impl FnOnce(&mut super::super::Esp32s31ApEngine<'_>)) {
    with_authorized_ap_owner(|mut engine| test(&mut engine));
}

pub(super) fn with_authorized_ap_owner(test: impl FnOnce(super::super::Esp32s31ApEngine<'_>)) {
    with_authorized_ap_capabilities(false, test);
}

pub(super) fn with_authorized_ap_capabilities(
    ht: bool,
    test: impl FnOnce(super::super::Esp32s31ApEngine<'_>),
) {
    ...
    let ht_ie = open_esp_radio_ieee80211::ht::ht_capability_ie(
        open_esp_radio_esp32s31_wifi_ap::profile::HT_CAPABILITIES,
        ...
        service
            .associate_open(
                ...
                ApAssociationCapabilities {
                    ...
                    ht: ht.then(|| {
                        open_esp_radio_ieee80211::ht::ht_peer_capabilities(&ht_ie).unwrap()
                    }),
                    qos_supported: ht,
                },
                1,
    ...
    let engine = super::super::Esp32s31ApEngine::start(
        &mut Hardware,
        service,
        &mut beacon,
        &mut keys,
