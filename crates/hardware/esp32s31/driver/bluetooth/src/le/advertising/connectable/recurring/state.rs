...
pub struct LegacyConnectableAdvertisingRecurringHci<Phase, Order> {
    pub(crate) phase: Phase,
    pub(crate) order: Order,
}

impl<Phase, Order> LegacyConnectableAdvertisingRecurringHci<Phase, Order> {
    ...
    >(
            ...
            .try_publish(controller)
        {
            LeControllerResponsePublication::Published(ordered) => {
                ...
                published(LegacyConnectableAdvertisingRecurringHci::from_parts(
                    phase, order,
                ))
            }
