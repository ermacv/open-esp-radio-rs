...
fn failed_head_retirement_retains_acl_and_ll_owners_without_unlink() {
    ...
    assert_eq!(fault.owner_mut().event.event_counter(), 0);
    assert!(fault.owner_mut().acl.controller_event_is_reserved());
    assert!(!fault.owner_mut().acl.can_accept_host_packet());
    assert_eq!(fault.owner_mut().acl.take_completed_host_packets(), 0);
}
