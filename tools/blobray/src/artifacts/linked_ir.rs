...
mod tests {
    ...
    fn validates_identity_claims_and_reads_summary() {
        let path =
            std::env::temp_dir().join(format!("blobray-linked-ir-artifact-{}", std::process::id()));
        ...
        assert!(
            inspect_linked_ir(&path)
                .unwrap_err()
                .to_string()
                .contains("expected schema_version 63")
        );
    ...
    fn bounded_graph_search_skips_external_calls_and_reports_limits() {
        let path =
            std::env::temp_dir().join(format!("blobray-linked-ir-graph-{}", std::process::id()));
        super::super::write_fixture_bundle(
            &path,
            &serde_json::to_string_pretty(&document()).unwrap(),
        )
