...
use crate::{MemoryAccess, ObservableEvent, Result, SymbolicValue, parse_u32};

#[derive(Clone, Debug, Eq, Ord, PartialEq, PartialOrd)]
pub(crate) struct RegisterId {
...
impl ContractEffect {
    pub(crate) fn selector(&self) -> EffectSelector {
        match self {
...
pub(crate) fn parse_effect_rule(
    value: &str,
    line: usize,
) -> Result<(EffectSelector, EffectDisposition)> {
    ...
    let selector = match kind {
        "mmio-read" | "mmio-write" => {
            ...
            let address = parse_u32(address_text)
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub(crate) enum EffectComparison {
    ExactEffectsV1,
}
...
impl EffectPolicy {
    ...
    #[cfg(test)]
    pub(crate) fn rules(&self) -> impl Iterator<Item = (&EffectSelector, &EffectDisposition)> {
        self.rules.iter()
    }
}

#[derive(Clone, Debug, Eq, PartialEq)]
pub(crate) enum EffectComparisonVerdict {
    Match,
    Mismatch(String),
}

pub(crate) fn effects_from_observable(events: &[ObservableEvent]) -> Result<Vec<ContractEffect>> {
