...
| Controller initialization | FAIL-CLOSED | The target typestate preserves the recovered cold order from clock/reset through HAL, scheduler, bounded HCI, source-127 hardware, common PHY, BTBB, BLE PHY, Controller output, runtime timer and stable interrupt-owner publication. Integration provides a one-shot stable slot that must be reserved before global claims or MMIO; the terminal owner is then placed without a lossy second initialization attempt, split exactly once and composed with all three live CPU routes, durable Embassy notification, a standard `bt_hci::ExternalController` Host facade and one eternal hardware runner. DTM current/RF-ready acquisition, scheduler admission, HEAD/RUN publication, completion drain and cancellation retain the exact Controller epoch. The single production cold-start transaction, unrelated-list routing, Link Layer/ACL, sleep-enabled RF wake, complete descriptor semantics, powered teardown and HIL remain missing. |
...
| Capability | Status | Current production boundary |
| --- | --- | --- |
| Direct Test Mode | PARTIAL | The S31 TX/RX/Test-End command bodies, channel/PHY/pattern lowering, interval arithmetic, bounded scheduler path, static graph, completion drain, recycle and Reset/Test-End response order are represented by affine finite transitions. The target Embassy actor owns the full command lifecycle, and the product runner now composes the sole final split, all live ISR/waker routes including source 127, absolute-deadline retries and the `bt-hci` Host facade. Successful hardware `RUN` remains the only edge to a successful start response; terminal or unsupported observations retain their exact owners after full-route quarantine. Production cold-start/spawn, unrelated-list and timer-expiration consumers, remaining hardware-consumed graph semantics, sleep-enabled RF wake and on-air/HIL evidence remain missing. |
