...
The only executable is `vendor-binary-workbench`. It exposes workflow groups;
removed product names and flat command spellings are errors. Product and data
naming are governed by [`NAMING.md`](NAMING.md).
...
```text
vendor-binary-workbench inspect analyze
vendor-binary-workbench inspect trace
vendor-binary-workbench mmio discover
vendor-binary-workbench ir export
vendor-binary-workbench ir build
vendor-binary-workbench reference generate
vendor-binary-workbench reference generate-batch
vendor-binary-workbench execute run
vendor-binary-workbench execute compare
vendor-binary-workbench verify profiles
vendor-binary-workbench verify inventory
vendor-binary-workbench verify contract
vendor-binary-workbench image audit-targets
```

Common artifact, SVD, architecture and ABI options come from project, target
and local run manifests. A command accepts only workflow-specific overrides.
Platform verification is selected as a named contract from the configured
harness.

The artifact layer exposes two intentionally separate symbol inventories.
Evidence-producing verification uses only global and weak code
...
4. Move ESP32-S31 external ABI tables and entry contracts into a dedicated
   ...
   **Implemented.** Neutral semantic interfaces and typed ESP32-S31
   verification are separate crates; the facade is only their registry.
...
6. Extract neutral types into the contracts crate and replace fixed RV32 call
   arguments in shared IR. **Implemented.** MMIO and symbolic/reference IR live
   in the analysis-model crate; verification reports remain in orchestration
   until their platform adapter callbacks are separated.
7. Add the hierarchical CLI and run manifest; remove the old binaries, flat
   aliases and old project-manifest discovery path. **Completed.**
8. Add a synthetic second backend conformance fixture before claiming the
   contracts API is architecture-neutral. ARM Thumb is the preferred first proof;
