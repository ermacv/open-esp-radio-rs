...
use super::{
    DtmStoppingSignal, DtmTestEndResponseSignal, DtmTestEndResponseWaitError, StoppingWaitBackend,
    StoppingWaitRef, StoppingWaitSource, wait_stopping_source, wait_test_end_response_capacity,
};
...
fn production_mapping_routes_every_wait_kind() {
    ...
    assert_eq!(
        ...
        Some(DtmStoppingSignal::PostUnlink)
    );
...
fn real_hci_affinity_rejects_foreign_endpoint_and_reports_capacity() {
    ...
    assert_eq!(
        ...
        Err(DtmTestEndResponseWaitError::EndpointMismatch)
    );
    assert_eq!(
        block_on(wait_test_end_response_capacity(&pending, &first.controller,)),
        Ok(DtmTestEndResponseSignal::Capacity)
    );
}
