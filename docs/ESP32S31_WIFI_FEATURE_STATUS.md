...
| Single-user/multi-user beamformee | report-rate policy implemented; sounding missing | The complete blob link-metric policy now produces a typed report profile and programs both hardware leaves in exact order: three BPSK/QPSK/16-QAM selectors followed by all four ordinary/ER-SU ACK-rate bytes. Capability parsing and average-SNR observation are also owned. The tested Android 16 SoftAP reports HE SU beamformer/beamformee support but no HE MU beamformer; it is a candidate SU sounding peer only. Sounding, feedback-buffer ownership, compressed report generation and HIL are missing. |
