    ...
    J: Esp32s31StaJoinObserver + Default,
{
    ...
    async fn prepare_candidate<'a>(
    ...
    ) -> Result<(), Esp32s31StaAttemptStepError<Self::Error>> {
        ...
        owner.prepared_peer = Some(
            Esp32s31StaPeerPort::prepare(owner.transmit, &owner.station.access_point).map_err(
                |error| {
                    Esp32s31StaAttemptStepError::terminal(Esp32s31StaAttemptTargetError::Candidate(
                        error,
                    ))
                },
            )?,
    ...
    async fn select_channel<'a>(
        ...
        owner
            ...
            .await
            .map_err(|error| {
                Esp32s31StaAttemptStepError::retry_current(Esp32s31StaAttemptTargetError::Channel(
                    error,
                ))
            })
    }
    ...
    async fn run_wpa2_handshake<'a>(
        ...
        let selected_rsn = select_wpa2_psk_rsn(&owner.station.access_point).map_err(|error| {
            Esp32s31StaAttemptStepError::terminal(Esp32s31StaAttemptTargetError::Security(error))
        })?;
        let receive = owner.receive.take().ok_or_else(|| {
            Esp32s31StaAttemptStepError::terminal(Esp32s31StaAttemptTargetError::State(
                Esp32s31StaAttemptStateError::MissingReceive,
