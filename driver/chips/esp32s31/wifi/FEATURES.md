...
The status terms are evaluated against the exact feature named by each row.
...
- **LIVE**: a bounded production path owns the complete operation for the
...
| Immediate Block ACK | PARTIAL | Negotiated immediate BlockAck, hardware bitmap capture/matching, software RX reorder, retained TX MPDUs and selective retry are live for the bounded WPA2/QoS STA/AP aggregate paths. STA TX owns TIDs 0/7/5, AP TX owns TID 0, and Open links do not start a BlockAck agreement; other security/TID combinations are not claimed. |
...
| Legacy/HT MAC | Open/CCMP RX defragmentation and fail-closed RTS-policy propagation are source-owned. Add bounded TX fragmentation, AP per-AC WMM/EDCA, general multi-lease A-MSDU scheduling, U-APSD and multi-PPDU TXOP ownership. | Physical RTS, all CTS-to-Self publication and MCS32 still need complete queue/PLCP/duration/power/retry evidence. |
...
| Beacon and low power | Bind the software beacon epoch to automatic filter enable/disable, raw timeout units and the reviewed wake cause; make doze cancellation-safe across every owner, and add a wake contract before allowing passive ESP-NOW RX with station power save. | Actual modem sleep and TWT installation require RF/PHY/BB/clock retention, wake compare and rollback evidence. Explicit TWT additionally needs the Information action and schedule-update semantics. |
| HE control plane | Add selected MBSSID profile ownership, dual-NAV state, spatial-reuse policy, OMC/UPH/BSR exchanges and bounded MU control-session state. Implement UORA OBO/backoff, RU eligibility and attempt/result ownership instead of treating its cold defaults as a feature. | HE-TB/OFDMA, UORA publication and beamforming feedback cannot go live before their final PHY vectors and doorbells are source-proven. |
| Ranging and special PHYs | The portable FTM codec/requester is source-owned; next bind its connected publication to a reviewed timestamp frontier and add a board antenna-routing contract. Model ESP-NOW LR's low-rate gate as an affine TX/RX epoch before any publication. | Distance needs calibrated antenna-point timestamps; ESP-NOW LR/encryption, antenna switching and channel 14 each need their missing physical or board contract. |
