...
impl<'a> ManifestContext<'a> {
    ...
    pub(crate) fn error(
        ...
        BlobrayError::manifest_source(self.kind, self.path, self.input, message, span)
    }
...
pub(crate) enum BlobrayError {
    ...
    #[diagnostic(
        code(blobray::verification::suite),
        help(
...
impl From<open_esp_radio_register_model::Error> for BlobrayError {
...
pub(crate) type Result<T> = std::result::Result<T, BlobrayError>;
...
mod tests {
    ...
    fn json_diagnostic_retains_the_physical_source_line() {
        ...
        let error = BlobrayError::manifest_document(
            "fixture report",
        ...
        assert!(matches!(
            error,
            BlobrayError::ManifestSource { span, .. }
