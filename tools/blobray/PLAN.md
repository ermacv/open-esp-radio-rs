...
| 22.5 | active | Triage every untriaged location of the index, starting with functions that have a production counterpart: a follow-up case that reaches it with an expected verdict, or a reviewed decision with its reason. Production-implemented vendor paths (for example the channel-14 MIC and 802.11p configuration) are cases, not exclusions. Each surviving mutant becomes a case that kills it (checked with a targeted mutation run) or a reviewed decision: `analog/temperature.rs` 22-24, 135, 139, 149 (both comparisons), 151, 153, 155; `analog/rfpll.rs` 36, 37, 65, 73, 77-80, 304 (both constants), 485; `analog/dcode.rs` 194, 204. The stage closes with an empty untriaged list and no untriaged survivor. |
