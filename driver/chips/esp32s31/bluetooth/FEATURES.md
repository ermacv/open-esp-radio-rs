...
| Legacy advertising | PRE-ADMISSION | A hardware-independent `open-esp-radio-bluetooth-ll` foundation validates and encodes exact `ADV_NONCONN_IND` PDUs, retains public/random TxAdd semantics and advances an affine non-connectable lifecycle over a non-empty channel 37/38/39 map only after an exact generation/event/channel completion. The S31 chip boundary consumes a pinned physical-SRAM graph, installs the PDU directly in its role-neutral TX allocation and closes the reviewed allocation-time chain `packet -> TX header -> link state <-> first scheduler item -> separate common scheduler context`; the restricted RX chain remains absent. A private codec now consumes only the semantic dBm request and prepared PDU to apply the complete reviewed LE 1M/no-RX/no-CTE/no-privacy link-state reset, including public and direct-random address paths, advertising Access Address and CRC preset. Exact portable identity plus primary-channel frequency survive both transitions, and cancellation rebuilds the reusable allocation graph. No raw SRAM image or rounded-power code escapes the memory crate. The owner deliberately cannot mint `InFlight` or publish `HEAD`/`RUN` until frequency/whitening, the initial scheduler window and completion ticket are closed. HCI advertising commands, response-capable mode and on-air evidence also remain absent. |
