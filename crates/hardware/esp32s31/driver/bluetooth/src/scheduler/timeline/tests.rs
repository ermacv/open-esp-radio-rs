...
use super::{
    ...
    SchedulerReservationError, SchedulerReservationReleaseError, SchedulerTimeline,
    SchedulerTimelineSlot, SchedulerTimingPolicy, SchedulerWindowReservation,
};
...
) -> Result<SchedulerWindowReservation<SchedulerInitialAdmissionResolved>, SchedulerReservationError>
{
...
fn non_overlapping_window_keeps_its_position() {
    let mut timeline = SchedulerTimeline::<2>::new();
...
fn empty_window_is_not_a_scheduler_candidate() {
    assert!(SchedulerRawWindow::from_projected_scheduler_window(30, 30).is_none());
}

#[test]
fn overlap_chain_delays_once_per_occupied_interval_and_preserves_duration() {
    let mut timeline = SchedulerTimeline::<3>::new();
...
fn recurring_collision_rejects_without_displacing_or_mutating_the_timeline() {
    ...
    assert!(matches!(
        ...
        Err(SchedulerReservationError::RecurringOverlapUnsupported)
    ));
...
fn full_timeline_applies_backpressure_without_replacing_an_owner() {
    ...
    assert!(matches!(
        ...
        Err(SchedulerReservationError::TimelineFull)
    ));
...
fn rejected_release_returns_the_exact_reservation_and_preserves_both_timelines() {
    ...
    assert_eq!(
        failure.error(),
        SchedulerReservationReleaseError::IdentityMismatch
    );
...
fn invalid_window_fails_before_occupying_storage() {
    ...
    assert!(matches!(
        ...
        Err(SchedulerReservationError::WindowOutsideForwardHalfRange)
    ));
    assert!(timeline.is_empty());
}
...
fn initial_admission_and_recurring_sequence_are_distinct_gates() {
    ...
    assert!(matches!(
        ...
        Err(SchedulerReservationError::InitialDeadlineExpired)
    ));
    assert!(timeline.is_empty());
...
fn exhausted_free_slot_does_not_wrap_its_generation() {
    let mut timeline = SchedulerTimeline::<1> {
    ...
    assert!(matches!(
        ...
        Err(SchedulerReservationError::GenerationExhausted)
    ));
    assert!(timeline.is_empty());
}
