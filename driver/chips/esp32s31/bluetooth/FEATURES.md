...
| Typed HCI Controller | PARTIAL | The in-process transport exposes disjoint Host, raw Controller and bootstrap endpoints through `bt-hci::ExternalController`. A finite portable classifier returns owned semantic bootstrap and DTM commands plus owned malformed or Unknown Command responses; no result borrows receive scratch storage. The ESP32-S31 first-event runner consumes Receiver/Transmitter Test v1 command ownership through hardware RUN. A portable generic order aggregate owns the standard start Command Complete beside an arbitrary radio owner: queue full, validation failure and a foreign endpoint retain both axes, successful publication destroys the response authority exactly once, and only the published state opens same-epoch command affinity. Host tests exercise full-queue retry, cross-epoch rejection, exact-once publication and FIFO ordering through the real transport. The S31 active session uses that same transaction while independently advancing completion, recycle and recurring RUN. The endpoint command loop, epoch-bound received-command token, active Test End and ACL semantics remain absent. |
...
proof, preserving radio progress during HCI backpressure. The remaining session
work is endpoint-bound command intake and retaining Test End through the final
completion.
...
| Embassy controller owner | PARTIAL | A target-only adapter drives every immediately ready first-event phase directly into the two-axis active session. Borrow-only scheduler and post-unlink readiness futures register before recheck without owning the task runtime or consuming durable state, so they can be selected beside HCI capacity while the affine session stays outside every future. Serialized primary ISR service wakes the ordinary scheduler and lock/modify consumers as well as the post-unlink waiter, without creating duplicate event state. The adapter preserves every cancellation/retry owner but does not yet compose those readiness sources, command intake, Test End, ISR route installation or powered shutdown into one sole task. |
