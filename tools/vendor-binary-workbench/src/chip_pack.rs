...
impl ChipPack {
    ...
    pub(crate) fn load(path: &Path) -> Result<Self> {
        let path = path
            .canonicalize()
            .map_err(|error| WorkbenchError::manifest("chip pack", path, error))?;
        let input = fs::read_to_string(&path)?;
        let document: ChipDocument = toml_edit::de::from_str(&input).map_err(|error| {
            WorkbenchError::manifest_source("chip pack", &path, &input, &error, error.span())
        })?;
...
mod tests {
    ...
    fn owns_chip_resources_and_rejects_project_independent_absolute_paths() {
        let root =
            std::env::temp_dir().join(format!("vendor-workbench-chip-pack-{}", std::process::id()));
        if root.exists() {
