...
| Scanning | FIRST WINDOW REACHES RUN | The restricted passive LE 1M graph owns two RX nodes, one link state, one scheduler context and three scheduler items in pinned Controller SRAM. Production cold start claims a fourth, scanner-specific static arena and lends its sole graph to the task service; DTM and advertising memory cannot alias it. Portable LL owns validated interval/window parameters, enabled/in-flight state, primary-channel rotation, legacy PDU parsing and a bounded exact duplicate filter. The production first-window runner now acquires a fresh scheduler current plus ordered always-awake BLE-PHY observation, projects the complete preparation/receive interval through the retained Controller epoch, performs admission and sequence-deadline checks, and reaches selector-one RX/command/head/interrupt/`RUN` publication without exposing an integer register or SRAM image. Its reservation remains affine through fenced completion, exact head retirement and the serialized post-unlink removal gate. RX extraction rejects stale producer/epoch sentinels and non-contiguous completion, copies only the bounded on-air PDU plus signed RSSI, and atomically returns both private lists and the timeline slot. Pre-publication cancellation returns the graph and reservation without leaking either owner. The completion/recurrence composition and standard `bt-hci` scan commands/events remain missing, so this is not yet an operational scanner. |
