...
impl EcosystemPack {
    ...
    pub(crate) fn load(path: &Path) -> Result<Self> {
        let path = path
            .canonicalize()
            .map_err(|error| BlobrayError::manifest("ecosystem pack", path, error))?;
        let input = fs::read_to_string(&path)?;
        let document = toml_edit::Document::parse(input.as_str()).map_err(|error| {
            BlobrayError::manifest_source("ecosystem pack", &path, &input, &error, error.span())
        })?;
...
mod tests {
    ...
    fn invalid_pack_span(input: &str, name: &str) -> (usize, usize, String) {
        let path = std::env::temp_dir().join(format!(
            "blobray-platform-diagnostic-{}-{name}.toml",
            std::process::id()
        ));
    ...
    fn pack_loads_relative_ecosystem_knowledge_without_chip_authority() {
        let root =
            std::env::temp_dir().join(format!("blobray-ecosystem-pack-{}", std::process::id()));
        if root.exists() {
    ...
    fn malformed_pack_reports_its_manifest_path() {
        let path = std::env::temp_dir().join(format!(
            "blobray-platform-malformed-{}.toml",
            std::process::id()
        ));
