...
//!
//! This module is the finite boundary between an Embassy-owned Ethernet lease
//! and the ESP32-S31 ordinary TX queue. It deliberately owns no logging,
    ...
    T: WifiTxTimer,
{
    ...
    pub(crate) fn take_protected_metadata(&mut self, tid: u8) -> Option<StaProtectedEthernetFrame> {
    ...
    pub(crate) const fn hardware_key_selector(&self) -> u8 {
        self.key.hardware_index()
    }

    pub(crate) const fn config(&self) -> SingleMpduTxConfig {
        self.config
    }

    pub(crate) const fn ordinary(&self) -> &OrdinaryTxOwner<'slot, P, E, T, BUFFER_SIZE> {
        &self.ordinary
    }

    pub(crate) fn ordinary_mut(&mut self) -> &mut OrdinaryTxOwner<'slot, P, E, T, BUFFER_SIZE> {
        &mut self.ordinary
    }
    ...
    pub(crate) fn copy_encoded_retry(
        &mut self,
        encoded: &[u8],
    ) -> Result<usize, SingleMpduTxError> {
    ...
    pub(crate) fn start_prepared_encoded_retry<H: TxHardware>(
        &mut self,
        hardware: &mut H,
        frame_length: usize,
                ...
                OrdinaryTxPlan {
                    ...
                    exchange: MacTxPlan {
                        ...
                        initial_rate: TxPhyRate::Legacy(
                            open_esp_radio_esp32s31_wifi_lmac::tx::LegacyRate::Dsss1MLong,
                        ),
                        publication_limit: self.config.exchange.publication_limit,
                        publication_timeout_micros: self.config.exchange.publication_timeout_micros,
                    },
...
mod tests {
        ...
        assert_eq!(
            embassy_futures::block_on(tx.service(
                ...
                WifiTxWake::Interrupt {
                    events: open_esp_radio_esp32s31_wifi_lmac::irq::MAC_INT_TX_COMPLETE,
                },
            )),
    ...
    fn timeout_waits_sixteen_micros_and_uses_the_bounded_retry_budget() {
        ...
        let timeout = WifiTxWake::Interrupt {
            events: open_esp_radio_esp32s31_wifi_lmac::irq::MAC_INT_TX_TIMEOUT,
        };

        assert_eq!(
            embassy_futures::block_on(tx.service(&mut hardware, timeout)),
            Ok(WifiTxProgress::Pending)
        );
    ...
    fn collision_retries_without_marking_an_untransmitted_mpdu_as_retry() {
        ...
        let collision = WifiTxWake::Interrupt {
            events: open_esp_radio_esp32s31_wifi_lmac::irq::MAC_INT_COLLISION,
        };

        assert_eq!(
            embassy_futures::block_on(tx.service(&mut hardware, collision)),
            Ok(WifiTxProgress::Pending)
        );
        hardware.collision = true;
        assert_eq!(
    ...
    fn executor_deadline_without_hardware_state_requires_radio_reset() {
        ...
        assert_eq!(
            embassy_futures::block_on(tx.service(&mut hardware, WifiTxWake::Deadline)),
