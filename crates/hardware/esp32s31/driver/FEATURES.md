...
| RF sleep / modem power-down | PARTIAL | PHY exposes exact RF close followed by temperature-sensor shutdown, retained route-clock release and cold-owner reconstruction. The production supervisor uses this path for explicit idle cold restart. A direct retained wake and complete platform clock/reset reversal are absent. Protocol power-save signaling does not supply this boundary. |
...
| Full powered shutdown | PARTIAL | The production supervisor composes runtime/IRQ quiescence, last-client release, exact RF/analog shutdown and cold-owner return. The platform layer still has to restore its remaining cold power-up clock/reset state for a complete powered-off claim. |
| Cold reconstruction after shutdown | PARTIAL | The idle control path invokes the source-owned shutdown and `restart_esp32s31_radio`, which captures the final post-tracking cache and feeds it directly into validated cold registration. Repeated hardware lifecycle evidence is not yet present. |
