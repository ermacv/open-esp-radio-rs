...
| Capability | Status | Current production boundary |
| --- | --- | --- |
| Direct Test Mode | PARTIAL | The exact S31 TX/RX/Test-End command bodies, allocator roles, common scheduler path and recycle callback are mapped. Channel/PHY selection, all eight TX patterns, packet duration, interval/tick arithmetic, initial/recurring windows and bounded overlap resolution are typed. The 936-byte graph is a non-movable static allocation with exact RX/TX headers, private-chain anchors, scheduler context, scheduler-item-to-link-state link and role-specific event/bookkeeping transforms; TX readiness survives every CPU-owned transition. Scheduler initialization proves all heads and its source-owned software list empty, and the sole-item merge is identity-checked and cancellable. Idle and active chip owners now consume command-ready authority, scratch storage and the combined endpoint in one intake-and-route operation; a standalone classification can no longer cross the chip boundary. Accepted RX/TX v1 commands become opaque role-specific deferred starts which retain that authority and the semantic command across cold or warm current acquisition, preparation and HEAD publication. In the ESP32-S31 composition, successful hardware RUN is the only edge to a successful response, while a failed response requires restoration of the idle graph and complete task service. Cold and warm cancellation drains cannot reach that edge while an orphaned time request remains; invariant, identity, ownership, list and post-HEAD failures retain fail-stop owners. The portable failed-start domain currently contains only Hardware Failure, and the chip selects it only for finite timing/resource rejection after recovery. Generic runner cancellation is a neutral Controller-shutdown transition whose clean task, epoch, drain and preparation-cleanup owners expose no HCI response edge; Operation Cancelled by Host remains absent until a real Host semantic action exists. Every other pre-RUN failure retains the exact deferred start and radio owner in a fail-closed recovery type. A second opaque runner consumes the resulting active owner through fresh completion capture, both retained finished-list continuations, fenced head retirement, atomic unlink/mailbox wait and role-correct recycle. A third bounded runner consumes the reclaimed TX/RX boundary and follows the role-specific recurring time/preparation order back through HEAD and RUN. One active-session aggregate carries that radio state beside an independent response-order axis, so C2H backpressure cannot stop completion, recycle or recurrence. After each successful publication the returned opaque idle command task admits exactly one next command: another TX/RX start becomes standard Controller Busy without releasing the radio, while Test End enters quiescence. Pre-HEAD recurrence is cancelled and drained; a visible HEAD completes exactly one final event before TX zero-count or accumulated RX-count response construction. The response retains task and graph through backpressure; successful enqueue destroys its response authority, returns the next command-ready authority and restores the graph into one opaque idle command task. Active Reset uses the same terminal-neutral quiescence core, restores the reclaimed graph before applying bootstrap Reset and likewise returns the opaque idle command task only after ordered response publication. RX completion metadata remains attached through every wait, retry, cancellation and orphan-drain branch. Every step remains finite and no executor future owns affine hardware state. A sole target-only Embassy command actor now composes idle intake, first-start cleanup, active progression, Test End and Reset. Production final-split/spawn wiring, live ISR and waker routes including source 127, the absolute-deadline provider, unrelated-list dispatch, the sleep-enabled RF wake branch and on-air/HIL evidence remain missing. |
