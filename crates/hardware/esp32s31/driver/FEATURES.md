...
| RF sleep / modem power-down | PARTIAL | PHY exposes exact RF close followed by temperature-sensor shutdown, retained route-clock release, captured Wi-Fi clock/reset restoration and cold-owner reconstruction. The production supervisor uses this path for explicit idle cold restart. A direct retained wake is absent. Protocol power-save signaling does not supply this boundary. |
