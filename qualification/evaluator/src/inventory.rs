...
fn render_program(qualification: &Qualification, output: &Path, root: &Path) -> Result<String> {
    ...
    text.push_str(&format!("- Repository commit: `{}`\n- Repository dirty: `{}`\n- Program: [{}]({}), schema {}, SHA-256 `{}`\n- Vendor evidence index: `{}` ({} entries, {} current)\n- HIL catalog: `{}`\n- HIL runs: `{}` ({} qualifying bundles)\n\n",
        ...
        qualification.evidence_inputs.vendor_evidence_index.display(), qualification.evidence_inputs.verification_entries, qualification.evidence_inputs.verification_current_release_entries,
