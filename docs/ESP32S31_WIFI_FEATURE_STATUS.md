...
| Downlink MU-MIMO | passive common/selected-user vector decoder implemented; payload HIL missing | The RX owner exposes the complete blob-decoded HE-MU common SIG-A view, including compressed-SIG-B user count, GI/LTF and STBC. It bounds and borrows the variable complete-SIG-B bytes and decodes the hardware-selected 21-bit MIMO/non-MIMO user view. An allocation-free iterator now covers all users in the exact blob-proven HE20 non-MU-MIMO layout, including the ten-bit pair gaps; wider and compressed/MU-MIMO complete-stream layouts fail closed. Compressed-stream iteration, payload qualification and beamforming feedback remain open. |
...
| Single-user/multi-user beamformee | hardware feedback sequence configured; sounding HIL missing | The complete blob link-metric policy now produces a typed report profile and programs both hardware leaves in exact order: three BPSK/QPSK/16-QAM selectors followed by all four ordinary/ER-SU ACK-rate bytes. Allocation-free HE NDPA parsing reproduces the complete blob's Dialog Token, four-byte STA Info iteration and local 11-bit AID membership check. The only production caller uses that check for diagnostics and has no software report producer or feedback buffer; complete `hal_init_bf` plus typed non-latched beam/NDP/feedback diagnostics establish that sequencing is hardware-owned. The complete test-only feedback selector is mapped in SVD, but its numeric mode meanings remain unproven. A controlled AX211 HE20 AP emitted no NDPA in four ten-second windows. A capable sounding peer is still required to qualify NDP detection and a transmitted compressed report. |
