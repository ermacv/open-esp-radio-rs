...
pub use codec::{
    ...
    BLUETOOTH_DTM_SCHEDULER_ITEM_BYTES, BLUETOOTH_DTM_TX_PACKET_BYTES, DtmMemoryGraphStorage,
};

use codec::{
    DtmEmptyListRollback, DtmMemoryGraphBinding, DtmRxRotationPlan,
    DtmSchedulerBookkeepingRollback, LeTestPduHeader,
};
...
pub struct DtmSchedulerAllocationConfig {
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum DtmMemoryGraphBindError {
...
///
/// ```compile_fail
/// use oer_esp32s31_bluetooth_memory::DtmMemoryGraphBindFailure;
///
/// fn duplicate(failure: DtmMemoryGraphBindFailure) {
///     let moved = failure;
///     let _ = failure.error();
///     drop(moved);
/// }
/// ```
pub struct DtmMemoryGraphBindFailure {
    storage: &'static mut DtmMemoryGraphStorage,
    error: DtmMemoryGraphBindError,
}

impl DtmMemoryGraphBindFailure {
    fn new(storage: &'static mut DtmMemoryGraphStorage, error: DtmMemoryGraphBindError) -> Self {
        Self { storage, error }
    }

    /// Return the finite reason without releasing the allocation.
    pub const fn error(&self) -> DtmMemoryGraphBindError {
        self.error
    }

    /// Recover the unchanged static allocation and binding error.
    pub fn into_parts(self) -> (&'static mut DtmMemoryGraphStorage, DtmMemoryGraphBindError) {
        (self.storage, self.error)
    }
}

impl core::fmt::Debug for DtmMemoryGraphBindFailure {
    fn fmt(&self, formatter: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        formatter
            .debug_struct("DtmMemoryGraphBindFailure")
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub struct DtmMemoryGraphModelAddress(BluetoothControllerSramAddress);

#[cfg(not(target_arch = "riscv32"))]
impl DtmMemoryGraphModelAddress {
...
pub struct DtmMemoryGraphIdentity(usize);

impl DtmMemoryGraphIdentity {
    fn for_storage(storage: &DtmMemoryGraphStorage) -> Self {
        Self(core::ptr::addr_of!(*storage).addr())
    }
}

impl core::fmt::Debug for DtmMemoryGraphIdentity {
    fn fmt(&self, formatter: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        formatter
            .debug_struct("DtmMemoryGraphIdentity")
            .finish_non_exhaustive()
    }
}
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum DtmMemoryGraphPrepareError<BuildError = Infallible> {
...
/// explicit shutdown.
pub struct DtmMemoryGraphPrepareFailure<BuildError = Infallible> {
    owner: DtmMemoryGraphCpuOwned,
    error: DtmMemoryGraphPrepareError<BuildError>,
}

impl<BuildError> DtmMemoryGraphPrepareFailure<BuildError> {
    /// Borrow the finite failure reason without releasing the owner.
    pub const fn error(&self) -> &DtmMemoryGraphPrepareError<BuildError> {
        &self.error
    }
    ...
    pub fn into_parts(
        ...
        DtmMemoryGraphPrepareError<BuildError>,
    ) {
        (self.owner, self.error)
    }
}

impl<BuildError: core::fmt::Debug> core::fmt::Debug for DtmMemoryGraphPrepareFailure<BuildError> {
    fn fmt(&self, formatter: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        formatter
            .debug_struct("DtmMemoryGraphPrepareFailure")
            .field("error", &self.error)
            .finish_non_exhaustive()
    }
}
...
///
/// ```compile_fail
/// use oer_esp32s31_bluetooth_memory::DtmMemoryGraphPositionalEventPrepared;
///
/// fn cannot_mutate_packet(prepared: &mut DtmMemoryGraphPositionalEventPrepared) {
///     let _packet = prepared.tx_packet_mut();
/// }
/// ```
pub struct DtmMemoryGraphPositionalEventPrepared {
    storage: Pin<&'static mut DtmMemoryGraphStorage>,
    binding: DtmMemoryGraphBinding,
    previous: DtmPositionalEventWords,
}

impl DtmMemoryGraphPositionalEventPrepared {
    ...
    pub fn words(&self) -> DtmPositionalEventWords {
    ...
    pub fn prepare_scheduler_bookkeeping(mut self) -> DtmMemoryGraphSchedulerBookkeepingPrepared {
        let rollback = self.storage.as_mut().prepare_scheduler_bookkeeping();

        DtmMemoryGraphSchedulerBookkeepingPrepared {
    ...
    pub fn cancel(mut self) -> DtmMemoryGraphCpuOwned {
        self.storage
            .as_mut()
...
pub struct DtmMemoryGraphSchedulerBookkeepingPrepared {
    storage: Pin<&'static mut DtmMemoryGraphStorage>,
    binding: DtmMemoryGraphBinding,
    previous: DtmPositionalEventWords,
...
impl DtmMemoryGraphSchedulerBookkeepingPrepared {
    ...
    #[doc(hidden)]
    pub fn prepare_empty_list_link(mut self) -> DtmMemoryGraphEmptyListLinkPrepared {
        let rollback = self.storage.as_mut().prepare_empty_list_link(self.rollback);

        DtmMemoryGraphEmptyListLinkPrepared {
            storage: self.storage,
            binding: self.binding,
            previous: self.previous,
            rollback,
    ...
    pub fn cancel(mut self) -> DtmMemoryGraphPositionalEventPrepared {
        self.storage
            .as_mut()
            .restore_scheduler_bookkeeping(self.rollback);

        DtmMemoryGraphPositionalEventPrepared {
            storage: self.storage,
            binding: self.binding,
            previous: self.previous,
        }
...
pub struct DtmMemoryGraphEmptyListLinkPrepared {
    storage: Pin<&'static mut DtmMemoryGraphStorage>,
    binding: DtmMemoryGraphBinding,
    previous: DtmPositionalEventWords,
...
impl DtmMemoryGraphEmptyListLinkPrepared {
    ...
    pub fn cancel(mut self) -> DtmMemoryGraphSchedulerBookkeepingPrepared {
...
pub struct DtmMemoryGraphHeadPublished {
    storage: Pin<&'static mut DtmMemoryGraphStorage>,
    binding: DtmMemoryGraphBinding,
}

impl DtmMemoryGraphHeadPublished {
...
impl DtmMemoryGraphRunning {
    ...
    pub fn observe_completion(
        self,
        observed: BluetoothSchedulerFinishedHardwareListObserved,
    ) -> DtmMemoryGraphCompletionObservation {
        if observed.index() != BluetoothSchedulerHardwareListIndex::ZERO {
            return DtmMemoryGraphCompletionObservation::ListMismatch {
                owner: self,
                observed,
            };
        }
        ...
        match self.storage.as_ref().get_ref().observe_completion_status() {
            Some(status) => DtmMemoryGraphCompletionObservation::CompletionObserved(
                DtmMemoryGraphCompletionObserved {
                    owner: self,
                    status,
                },
            ),
            None => DtmMemoryGraphCompletionObservation::StillInFlight(self),
        }
    }
}
...
impl DtmMemoryGraphCompletionObserved {
    ...
    #[doc(hidden)]
    pub fn into_parts(self) -> (DtmMemoryGraphRunning, DtmSchedulerItemCompletionStatus) {
    ...
    pub fn prepare_recycle_after_software_list_removal(
    ...
    ) -> Result<DtmMemoryGraphRecyclePrepared, DtmMemoryGraphRecycleFailure> {
        let error = if removal.index() != BluetoothSchedulerHardwareListIndex::ZERO {
            Some(DtmMemoryGraphRecycleError::HardwareListMismatch)
        } else if removal.completed_head().address() != Some(self.scheduler_item_address()) {
            Some(DtmMemoryGraphRecycleError::SchedulerItemMismatch)
        } else {
            None
        };
        if let Some(error) = error {
            return Err(DtmMemoryGraphRecycleFailure {
                error,
                completed: self,
                removal,
            });
        ...
        Ok(DtmMemoryGraphRecyclePrepared {
            completed: self,
            removal,
        })
    }
...
pub struct DtmMemoryGraphRecyclePrepared {
    completed: DtmMemoryGraphCompletionObserved,
    removal: BluetoothSchedulerSoftwareListRemovalReady,
}

impl DtmMemoryGraphRecyclePrepared {
    ...
    #[doc(hidden)]
    pub fn commit(self) -> DtmMemoryGraphRecycleCleaned {
        let Self {
            completed,
            removal: _,
        } = self;
        let DtmMemoryGraphCompletionObserved { owner, status } = completed;
        let DtmMemoryGraphRunning {
            mut storage,
            binding,
        } = owner;
    ...
    pub fn prepare_receiver_success(
        self,
    ) -> Result<DtmMemoryGraphRxSuccessRecyclePrepared, DtmMemoryGraphRxSuccessRecycleFailure> {
        ...
        let plan = match owner.storage.as_ref().get_ref().validate_rx_rotation(
            ...
            Ok(plan) => plan,
            Err(error) => {
                return Err(DtmMemoryGraphRxSuccessRecycleFailure {
                    error,
                    recycle: self,
                });
            }
        };
        Ok(DtmMemoryGraphRxSuccessRecyclePrepared {
            recycle: self,
            plan,
        })
    }
...
pub struct DtmMemoryGraphRxSuccessRecyclePrepared {
    recycle: DtmMemoryGraphRecyclePrepared,
    plan: DtmRxRotationPlan,
}

impl DtmMemoryGraphRxSuccessRecyclePrepared {
    /// Recover the byte-unchanged generic recycle transaction.
    pub fn into_recycle_prepared(self) -> DtmMemoryGraphRecyclePrepared {
        self.recycle
    }
    ...
    pub fn observe(self) -> DtmMemoryGraphRxSuccessObserved {
        let Self { recycle, plan } = self;
        DtmMemoryGraphRxSuccessObserved {
...
pub struct DtmMemoryGraphRxSuccessObserved {
    memory: DtmMemoryGraphRecycleCleaned,
    plan: DtmRxRotationPlan,
}

impl DtmMemoryGraphRxSuccessObserved {
    ...
    ) -> (DtmMemoryGraphRecycleCleaned, ResultValue) {
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum DtmMemoryGraphRxSuccessRecycleError {
...
pub struct DtmMemoryGraphRxSuccessRecycleFailure {
    error: DtmMemoryGraphRxSuccessRecycleError,
    recycle: DtmMemoryGraphRecyclePrepared,
}

impl DtmMemoryGraphRxSuccessRecycleFailure {
    /// Return the semantic reason preflight rejected the unchanged graph.
    pub const fn error(&self) -> DtmMemoryGraphRxSuccessRecycleError {
        self.error
    }
...
impl core::fmt::Debug for DtmMemoryGraphRxSuccessRecycleFailure {
    fn fmt(&self, formatter: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        formatter
            .debug_struct("DtmMemoryGraphRxSuccessRecycleFailure")
            .field("error", &self.error)
            .finish_non_exhaustive()
    }
}
...
pub struct DtmMemoryGraphRecycleCleaned {
    storage: Pin<&'static mut DtmMemoryGraphStorage>,
    binding: DtmMemoryGraphBinding,
    status: DtmSchedulerItemCompletionStatus,
}

impl DtmMemoryGraphRecycleCleaned {
    ...
    #[doc(hidden)]
    pub fn into_cpu_owned(self) -> DtmMemoryGraphRecycled {
        DtmMemoryGraphRecycled {
            owner: DtmMemoryGraphCpuOwned {
                storage: self.storage,
                binding: self.binding,
            },
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum DtmMemoryGraphRecycleError {
...
pub struct DtmMemoryGraphRecycleFailure {
    error: DtmMemoryGraphRecycleError,
    completed: DtmMemoryGraphCompletionObserved,
    removal: BluetoothSchedulerSoftwareListRemovalReady,
}

impl DtmMemoryGraphRecycleFailure {
    /// Exact identity mismatch that prevented CPU ownership return.
    pub const fn error(&self) -> DtmMemoryGraphRecycleError {
        self.error
    }
...
pub struct DtmMemoryGraphRecycled {
    owner: DtmMemoryGraphCpuOwned,
    status: DtmSchedulerItemCompletionStatus,
}

impl DtmMemoryGraphRecycled {
    /// Recover the ordinary CPU graph and its retained completion status.
    pub fn into_parts(self) -> (DtmMemoryGraphCpuOwned, DtmSchedulerItemCompletionStatus) {
        (self.owner, self.status)
    }
}
...
pub enum DtmMemoryGraphCompletionObservation {
    ...
    StillInFlight(DtmMemoryGraphRunning),
    /// A non-sentinel status was observed after the transfer fence.
    CompletionObserved(DtmMemoryGraphCompletionObserved),
}
...
///     DtmMemoryGraphModelAddress, DtmMemoryGraphStorage,
/// };
///
/// let storage = Box::leak(Box::new(DtmMemoryGraphStorage::new()));
/// let base = DtmMemoryGraphModelAddress::new(0x2f00_0100).unwrap();
/// let config = oer_esp32s31_bluetooth_memory::
///     DtmSchedulerAllocationConfig::new(1, 1, 1);
/// let owner = DtmMemoryGraphStorage::pin_static_model(storage, base, config).unwrap();
/// let moved = owner;
/// let _identity = owner.identity();
/// drop(moved);
/// ```
pub struct DtmMemoryGraphCpuOwned {
    storage: Pin<&'static mut DtmMemoryGraphStorage>,
    binding: DtmMemoryGraphBinding,
}
...
/// is available only from [`DtmMemoryGraphCpuOwned`], so a graph still
...
pub struct DtmMemoryGraphReclaimed {
    storage: Pin<&'static mut DtmMemoryGraphStorage>,
    binding: DtmMemoryGraphBinding,
}

impl DtmMemoryGraphReclaimed {
    ...
    pub fn reinitialize(self) -> DtmMemoryGraphCpuOwned {
        let config = self.binding.allocation_config();
        let mut owner = DtmMemoryGraphCpuOwned {
            storage: self.storage,
            binding: self.binding,
        };
...
pub struct DtmMemoryGraphTxPacketPrepared {
    storage: Pin<&'static mut DtmMemoryGraphStorage>,
    binding: DtmMemoryGraphBinding,
    _pdu_header: LeTestPduHeader,
    packet_length: LeTxPacketPreparedLength<BLUETOOTH_DTM_TX_PACKET_BYTES>,
}

impl DtmMemoryGraphTxPacketPrepared {
    ...
    ) -> Result<DtmMemoryGraphPositionalEventPrepared, DtmMemoryGraphPrepareFailure<BuildError>>
    {
    ...
    pub fn discard_packet_readiness(self) -> DtmMemoryGraphCpuOwned {
        DtmMemoryGraphCpuOwned {
            storage: self.storage,
            binding: self.binding,
        }
...
pub struct DtmMemoryGraphTxPacketPrepareFailure {
    owner: DtmMemoryGraphCpuOwned,
    error: DtmTxPacketPrepareError,
}

impl DtmMemoryGraphTxPacketPrepareFailure {
    /// Return the finite packet-header validation failure.
    pub const fn error(&self) -> DtmTxPacketPrepareError {
        self.error
    }

    /// Recover the byte-unchanged owner and exact validation failure.
    pub fn into_parts(self) -> (DtmMemoryGraphCpuOwned, DtmTxPacketPrepareError) {
        (self.owner, self.error)
    }
}

impl core::fmt::Debug for DtmMemoryGraphTxPacketPrepareFailure {
    fn fmt(&self, formatter: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        formatter
            .debug_struct("DtmMemoryGraphTxPacketPrepareFailure")
            .field("error", &self.error)
            .finish_non_exhaustive()
    }
}

impl DtmMemoryGraphCpuOwned {
    ...
    /// [`DtmMemoryGraphReclaimed::reinitialize`] starts a fresh epoch.
    /// No equivalent operation exists on any hardware or completion typestate.
    pub fn into_reclaimed(self) -> DtmMemoryGraphReclaimed {
        DtmMemoryGraphReclaimed {
            storage: self.storage,
            binding: self.binding,
        }
    }
    ...
    pub fn prepare_tx_packet(
    ...
    ) -> Result<DtmMemoryGraphTxPacketPrepared, DtmMemoryGraphTxPacketPrepareFailure> {
        let pdu_header = match LeTestPduHeader::without_cte(payload_type) {
            Ok(pdu_header) => pdu_header,
            Err(error) => {
                return Err(DtmMemoryGraphTxPacketPrepareFailure { owner: self, error });
            }
        };
        ...
        Ok(DtmMemoryGraphTxPacketPrepared {
            storage: self.storage,
            binding: self.binding,
    ...
    ) -> Result<DtmMemoryGraphPositionalEventPrepared, DtmMemoryGraphPrepareFailure<BuildError>>
    {
            ...
            .positional_event_seed(&self.binding)
        {
            Ok(seed) => seed,
            Err(error) => {
                return Err(DtmMemoryGraphPrepareFailure { owner: self, error });
            }
        };
        ...
        let candidate = match build(seed) {
            ...
            Err(error) => {
                return Err(DtmMemoryGraphPrepareFailure {
                    owner: self,
                    error: DtmMemoryGraphPrepareError::Build(error),
                });
            }
        };
        ...
        Ok(DtmMemoryGraphPositionalEventPrepared {
            storage: self.storage,
            binding: self.binding,
            previous,
        })
    ...
    fn initialize_reviewed_allocation(&mut self, config: DtmSchedulerAllocationConfig) {
        self.storage
            .as_mut()
...
impl DtmMemoryGraphStorage {
    ...
    ) -> Result<DtmMemoryGraphCpuOwned, DtmMemoryGraphBindFailure> {
        let base = match u32::try_from(core::ptr::addr_of!(*storage).addr()) {
            Ok(base) => base,
            Err(_) => {
                return Err(DtmMemoryGraphBindFailure::new(
                    storage,
                    DtmMemoryGraphBindError::AddressWidth,
                ));
            }
        };
    ...
    ///
    /// ```compile_fail
    /// use oer_esp32s31_bluetooth_memory::DtmMemoryGraphStorage;
    ///
    /// let storage = Box::leak(Box::new(DtmMemoryGraphStorage::new()));
    /// let config = oer_esp32s31_bluetooth_memory::
    ///     DtmSchedulerAllocationConfig::new(1, 1, 1);
    /// let _ = DtmMemoryGraphStorage::pin_static_model(
    ///     storage,
    ///     0x2f00_0100,
    ///     config,
    /// );
    ...
    fn pin_static_inner(
        ...
        let identity = DtmMemoryGraphIdentity::for_storage(storage);
        let binding = match DtmMemoryGraphBinding::new(identity, base, config) {
            Ok(binding) => binding,
            Err(error) => return Err(DtmMemoryGraphBindFailure::new(storage, error)),
        };
        let mut owner = DtmMemoryGraphCpuOwned {
