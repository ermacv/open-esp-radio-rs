...
fn repository_root() -> &'static Path {
    Path::new(env!("CARGO_MANIFEST_DIR"))
        .ancestors()
        .nth(5)
        .expect("ESP32-S31 host remains under verification/vendor/projects")
}
...
fn target_ownership_partitions_exactly_cover_the_register_model() {
    ...
    let api_path = registers.join("policy/api.toml");
    ...
    let model_path = registers.join("model/device.toml");
    ...
    for fragment in fragments {
        ...
        let document = toml_document(&model_path.parent().unwrap().join(relative));
...
fn ieee802154_vendor_scaffold_is_fail_closed_and_source_scoped() {
    let target = repository_root().join("verification/vendor/projects/esp32s31");
...
fn ecosystem_pack_parses_shared_espressif_family_knowledge() {
    ...
    let target = vendor.join("projects/esp32s31");
...
fn chip_geometry_is_reusable_and_project_facts_stay_sparse() {
    ...
    let registers = repository_root().join("registers/esp32s31");
    assert!(chip.join("chip.toml").is_file());
    assert!(!target.join("chip.toml").exists());
    for relative in ["memory.toml", "device.toml", "reviewed.toml"] {
        assert!(
            registers.join("model").join(relative).is_file(),
            "shared register model lacks {relative}"
        );
    }
    assert!(registers.join("model/peripherals").is_dir());
    assert!(!chip.join("registers").exists());
    assert!(!target.join("registers").exists());
    assert!(!chip.join("memory.toml").exists());
    assert!(!target.join("memory.toml").exists());
    ...
    assert_eq!(
        ...
        Some("../../../../registers/esp32s31/model/reviewed.toml")
    );
    assert_eq!(
    ...
    for (field, relative) in [
        ("memory-map", "model/memory.toml"),
        ("register-model", "model/device.toml"),
    ] {
        assert_eq!(
            chip.join(chip_manifest[field].as_str().unwrap())
                .canonicalize()
                .unwrap(),
            registers.join(relative).canonicalize().unwrap(),
            "chip composition must consume the shared register owner"
        );
    }
    ...
    let reviewed = toml_document(&registers.join("model/reviewed.toml"));
    ...
    let register_model = toml_document(&registers.join("model/device.toml"));
...
fn esp32c5_fixture_configures_successfully() {
    let target = repository_root().join("verification/vendor/projects/esp32c5");
...
fn public_interface_templates_keep_exact_s31_bindings_in_the_project_overlay() {
    ...
    let overlay_path =
        repository.join("verification/vendor/projects/esp32s31/interfaces/reviewed.toml");
