...
| Uplink and downlink OFDMA | parser implemented; scheduler oracle only | Allocation-free Rust now decodes Trigger Common Info, both RU/SS User Info forms, Basic/BFRP/MU-BAR/NFRP dependent fields and TRS/UPH HE-control information from instruction-exact blob evidence. Sustained HE-SU traffic against FRITZ and Android Wi-Fi 6 APs produced no Trigger: FRITZ advertised triggered SU feedback/CQI but did not schedule it, while the Android AP advertised neither and reports no HE MU beamformer. Trigger-frame admission/iteration, RU scheduling, HE-TB TX and OFDMA RX are not yet complete. |
...
| DCM up to 16-QAM | **HIL TX BPSK**; QPSK/16-QAM implemented | Peer TX/RX constellation capabilities are parsed independently. The typed HE-SU BCC path admits only DCM MCS0/1/3, programs HE-SIG-A1 DCM bit 7, uses the recovered RU242 rate tables for all three qualified GI/LTF selectors, reproduces the blob's minimum-subframe-length calculation, owns both the zero-TXOP ROM APEP table and the nonzero-EDCA-TXOP producer, and applies the blob's DCM-halving branch. WMM AC parameters now survive bounded scan parsing into the live HE batch policy. On a controlled AX211 AP that advertises BPSK DCM RX, open MCS0 passed both S-MPDU/ordinary ACK and one-subframe A-MPDU/BlockAck at all three GI/LTF selectors. The HIL originally forced at least two full-size MPDUs; that exceeded the rate-dependent PPDU-duration budget and was not a PHY failure. MCS1/QPSK and MCS3/16-QAM still need a capable peer. MCS4 belongs to the still-unowned LDPC profile; DCM RX remains. |
