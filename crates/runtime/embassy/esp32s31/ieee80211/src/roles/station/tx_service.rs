...
impl<'slot, H, P, E, T, SoftwareFrame, PhysicalFrame, const TX_BUFFER_SIZE: usize>
    ...
    for SingleMpduTx<'slot, P, E, T, TX_BUFFER_SIZE>
where
    H: TxHardware,
    ...
    PhysicalFrame: MaterializedTxFrame,
{
    ...
    {
        async move {
            let progress = SingleMpduTx::start(self, hardware, frame.as_slice())?;
    ...
    fn wait_deadline(&mut self) -> impl Future<Output = ()> + '_ {
        SingleMpduTx::wait_deadline(self)
    }
    ...
    fn service<'a>(
        ...
        async move { SingleMpduTx::service(self, hardware, wake) }
    }
}
