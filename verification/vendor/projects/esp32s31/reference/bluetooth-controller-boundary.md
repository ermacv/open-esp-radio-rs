...
| `r_btdm_task_enable` | split, still incomplete | The hardware-only 50-operation BTDM HAL-init body, exact baseline fault masks and diagnostic capture, controller-output strobes, generated runtime-timer start command, typed three-route ESP-HAL primitives, affine ISR scheduler MMIO, dynamic scheduler classifier, the RTOS-free coalesced scheduler wake and pure controller-time latch/epoch phases are recovered as separate contracts. One affine powered epoch composes scheduler initialization and low-power hardware with borrowed target common-PHY registration, explicit Bluetooth-client acquisition and terminal initial tracking. Only a settled client can cross the finite BTBB gate and remain retained through BLE-PHY register initialization, Controller-output preparation, runtime-timer start and stable interrupt-owner publication. Its powered task endpoint performs the scheduler executor's list-zero steps; the radio runtime drives them for the radio role. Registration and tracking failures retain the fail-stop. The portable LE Controller core serves HCI over this runtime through the portable service loop; no Host facade or final HCI target composition exists yet. Periodic client tracking, client/BTBB/PHY teardown, unrelated-list and source-127 expiration consumers, feature-specific NRT policy, effective controller-counter width and wake causality, typed selector-4/6 actions, on-air BLE readiness and HIL remain unresolved. |
