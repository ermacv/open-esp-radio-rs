...
| --- | --- |
| 1. Selection and leases | CLI/catalog code resolves typed scenarios. The lab guard acquires the cooperative cell, serial, local-wiphy and managed-OpenWrt leases required by that selection; these are local user-account locks, not distributed reservations. |
| 2. Plan and lab provenance | `RunSession` creates a unique directory and writes `plan.json`. While the fixture lock is held, `lab::provenance` records the secret-free topology before any firmware build or flash. |
| 3. Firmware archive | `image` builds the selected class, or `evidence::verify` validates an explicit replay source. `RunSession::record_firmware` stores the subjects and returns the run-local `firmware/<class>/application.bin`. |
