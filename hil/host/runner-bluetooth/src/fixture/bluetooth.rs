...
fn run_profile_in(
    ...
    hil_core::durable::atomic_json(&output.join("result.json"), &summary)?;
    result
}
...
pub fn preflight(adapter: Adapter) -> crate::Result<()> {
...
pub fn preflight_security_failure(adapter: Adapter) -> crate::Result<()> {
    preflight(adapter)?;
