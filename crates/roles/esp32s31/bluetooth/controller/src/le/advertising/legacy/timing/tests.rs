...
use oer_esp32s31_bluetooth_memory::{
    LegacyAdvertisingPrimaryChannel, LegacyConnectableAdvIndPacketInput,
    ...
    LegacyConnectableAdvertisingOwnAddress, LegacyConnectableAdvertisingPostAnchorDuration,
    LegacyConnectableScanResponsePacketInput, NonScanningRxMemoryModelAddress,
...
const fn instant(image: u32) -> SchedulerInstant {
    SchedulerInstant::from_image(image)
}

fn connectable_post_anchor_duration() -> LegacyConnectableAdvertisingPostAnchorDuration {
    ...
    let graph_address = LegacyConnectableAdvertisingMemoryGraphModelAddress::new(0x2f00_0100)
        .expect("the modeled graph address belongs to controller SRAM");
    let graph = LegacyConnectableAdvertisingMemoryGraphStorage::pin_static_model(
        graph_storage,
        graph_address,
    )
    ...
    let receive_address = NonScanningRxMemoryModelAddress::new(0x2f00_4000)
        .expect("the modeled RX address belongs to controller SRAM");
    let receive = NonScanningRxMemoryStorage::pin_static_model(receive_storage, receive_address)
        .expect("the receive pool fits controller SRAM");

    let input = LegacyConnectableAdvertisingMemoryInput::new(
        LegacyConnectableAdvIndPacketInput::try_from_encoded_extent(&ADV_IND_PDU, 9)
        ...
        LegacyConnectableAdvertisingOwnAddress::Random([1, 2, 3, 4, 5, 6]),
...
fn connectable_first_event_reserves_preparation_and_memory_supplied_duration() {
    let scale = oer_esp32s31_bluetooth::controller_hal::reviewed_standalone_time_scale();
    let epoch =
...
fn connectable_recurrence_preserves_portable_phase_and_complete_graph_duration() {
    ...
    let offset = 25_000;
    let (next, _) = LegacyAdvertisingRecurringTimingObservation::new(epoch)
...
fn recurring_event_advances_nominal_phase_and_reserves_the_complete_chain() {
    let first = LegacyAdvertisingEventWindow::first_le_1m(
        SchedulerSoftwareConfig::reviewed_standalone(),
        instant(10_000),
        instant(11_999),
        9,
    );
    let recurring = LegacyAdvertisingEventWindow::recurring_le_1m(
        first.phase(),
        20_000,
        SchedulerSoftwareConfig::reviewed_standalone(),
        9,
...
fn first_advertising_deadline_keeps_two_milliseconds_at_standalone_tick_rate() {
    ...
    let mut timeline = oer_esp32s31_bluetooth::scheduler::timeline::SchedulerTimeline::<1>::new();
    let reservation = timeline
        .reserve_initial_window(
            raw.start(),
            raw.end(),
            oer_esp32s31_bluetooth::scheduler::SchedulerTimingPolicy::from_scheduler_config(
                config, scale,
            ),
