...
| Interrupt observation | PARTIAL | Separate restricted transactions preserve the vendor distinction: primary samples the two masked status banks, NRT samples the two raw banks, and both acknowledge through the shared W1C clear banks. One primary epoch conditionally captures all diagnostic words for the exact baseline fault lanes before dynamic classification. Source 127 has a separate consuming PAC/HAL owner, a bounded four-branch IRQ classifier and a second finite common-handler register step. That step samples and conditionally clears positional states `0024`/`002C`; an idle pair performs the final fresh read and rearms, while either nonzero state is fenced into an opaque software-pending owner. The owner now carries a wrapping positional epoch and exposes exact finite counter-rollover acknowledgement, compare disable/program branches and final-read seam. All paths are loop-free and keep raw PAC ownership hidden. The ISR partition also owns the scheduler reference/work words without a task-side MMIO alias. A source-owned timer queue, bounded event publication, proof-gated final rearm, live source-127 composition and NRT dispatch remain missing. |
