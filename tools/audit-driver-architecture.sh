...
# MODEM_SYSCON radio words belong exclusively to the affine custom PAC. The
# official ESP-HAL singleton may remain as an inert ownership guard, but no
# production module may dereference its associated register block. Publication
# must also keep the carveout in the radio SVD and out of the platform sidecar.
mapfile -t modem_syscon_mmio_owners < <(
    rg -l 'MODEM_SYSCON::regs\(\)' driver --glob '*.rs' | sort
)
if test "${#modem_syscon_mmio_owners[@]}" -ne 0; then
    printf '%s\n' "${modem_syscon_mmio_owners[@]}" >&2
    echo "MODEM_SYSCON MMIO authority escaped the affine custom PAC" >&2
    exit 1
fi
mapfile -t modem_syscon_route_mmio_owners < <(
    rg -l '\bmodem_syscon_radio\b' driver --glob '*.rs' | sort
)
if test "${#modem_syscon_route_mmio_owners[@]}" -ne 2 \
    || test "${modem_syscon_route_mmio_owners[0]}" != "driver/chips/esp32s31/pac-raw/src/lib.rs" \
    || test "${modem_syscon_route_mmio_owners[1]}" != "driver/chips/esp32s31/pac/src/modem_syscon.rs"
then
    echo "raw MODEM_SYSCON route access escaped its custom PAC module" >&2
    exit 1
fi
if ! rg -q '<name>MODEM_SYSCON_RADIO</name>' svd/esp32s31-radio.svd; then
    echo "route-owned MODEM_SYSCON peripheral is absent from the radio SVD" >&2
    exit 1
fi
if rg -q '<name>MODEM_SYSCON(_RADIO)?</name>' svd/esp32s31-platform-radio-deps.svd; then
    echo "MODEM_SYSCON register ownership leaked into the platform sidecar" >&2
    exit 1
fi
if rg -n 'modem_source[.]bits\(\)[[:space:]]*==' driver --glob '*.rs'; then
    echo "whole MODEM_CONF register-image comparison escaped the PAC boundary" >&2
    exit 1
fi
mapfile -t modem_lpcon_route_mmio_owners < <(
    rg -l '\bmodem_lpcon_shared_clock\b' driver --glob '*.rs' | sort
)
if test "${#modem_lpcon_route_mmio_owners[@]}" -ne 2 \
    || test "${modem_lpcon_route_mmio_owners[0]}" != "driver/chips/esp32s31/pac-raw/src/lib.rs" \
    || test "${modem_lpcon_route_mmio_owners[1]}" != "driver/chips/esp32s31/pac/src/modem_shared_clock.rs"
then
    echo "raw MODEM_LPCON shared-clock access escaped its custom PAC module" >&2
    exit 1
fi
if rg -n \
    'read\(\)[.]bits\(\)|<<|>>|[&|][[:space:]]*0x[0-9a-fA-F_]+|[|][[:space:]]*[0-9]+\b|[.]set\([0-9]' \
    driver/chips/esp32s31/pac/src/modem_shared_clock.rs \
    driver/chips/esp32s31/pac/src/modem_syscon.rs
then
    echo "handwritten MODEM PAC code contains register geometry instead of field accessors" >&2
    exit 1
fi
if rg -n \
    'SharedModemClockMapImage|CoexistenceClockGateImage|PhyI2cMasterClockGateImage|LowPowerTimerClockGateImage|ModemLowPowerTimerConfigurationBits' \
    driver/chips/esp32s31/pac/src \
    driver/chips/esp32s31/pac-raw/src/lib.rs
then
    echo "obsolete whole-register MODEM image API survived field-accessor publication" >&2
    exit 1
fi
