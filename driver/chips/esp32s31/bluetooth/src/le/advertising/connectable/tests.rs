...
fn packet_after_accepted_connect_ind_is_not_silently_discarded() {
    ...
    let BluetoothLegacyConnectableAdvertisingPortableRxOutcome::PacketAfterConnection {
        accepted,
        packet,
        rejected_packets,
    } = outcome
...
fn busy_peripheral_rolls_back_the_connectable_graph() {
    ...
    let BluetoothLegacyConnectableAdvertisingRuntimeBeginFailure::PeripheralEventActive {
    ...
    assert_eq!(
        error,
        crate::BluetoothPeripheralConnectionRuntimeBeginError::EventActive
    );
    assert!(connectable.event_is_idle());
    assert!(!peripheral.allocation_is_idle());
...
fn memory_preparation_failure_restores_both_runtime_slots() {
    ...
    let BluetoothLegacyConnectableAdvertisingRuntimeBeginFailure::MemoryPreparation {
        definition: returned_definition,
        error,
    } = failure
    else {
    ...
    assert_eq!(returned_definition, definition);
    assert_eq!(error, open_esp_radio_esp32s31_bluetooth_memory::BluetoothLegacyConnectableAdvertisingMemoryGraphPrepareError::ReceivePoolOverlapsGraph);
    assert!(connectable.event_is_idle());
    assert!(peripheral.allocation_is_idle());
}
...
fn active_connectable_event_does_not_touch_an_unrelated_peripheral_runtime() {
    ...
    let BluetoothLegacyConnectableAdvertisingRuntimeBeginFailure::AdvertisingEventActive {
        definition: returned_definition,
    } = failure
    else {
