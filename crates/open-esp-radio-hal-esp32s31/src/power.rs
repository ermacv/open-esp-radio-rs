...
pub(crate) fn execute_owned(platform: &mut impl PowerClockControl) -> Result<(), PowerError> {
    ...
    // from the pinned S31 esp-hal clock oracle at commit 6899213e, not a
...
mod tests {
    ...
    use super::{execute_owned, PowerCheckpoint, PowerClockControl, PowerClockImages, PowerError};
