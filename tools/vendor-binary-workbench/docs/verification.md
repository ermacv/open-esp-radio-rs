...
is intentionally outside the workbench; the protected oracle workflow checks
caller-configured SHA-256 values before it creates its run spec.

`verify inventory` treats `(vendor source, symbol)` as function identity. This is
...
workbench logic; an `effect-contract` names the canonical effect comparator.
...
binding verifier, generator, generated harness, normalized generated source,
...
disposition entries. Semantic contracts without Binding v1 remain visible but
are not presented as artifact/probe-bound until they adopt
...
- `--gate regression --match-floor 104 --evidence-baseline PATH` passes when
  ...
  the verifier itself therefore also requires an explicit baseline review.
...
## Reviewing an evidence change

An evidence digest intentionally changes when a profile, policy, binding,
generated reference, adapter, comparator, execution engine, or another source
inside its proof boundary changes. Do not resolve that failure by editing the
accepted baseline during the protected run. First preserve the complete
machine-readable result, including a failing result:
...
cargo vendor-binary-workbench verify inventory \
  ...
  --gate regression --match-floor 104 \
  --json-report /tmp/esp32s31-oracle-regression.json
```

Then create a separate, deterministic review candidate. This step reads only
the JSON report and the public project; it does not need vendor artifacts, a
run spec, an SVD, or an available analysis backend:

```console
cargo vendor-binary-workbench verify evidence \
  --project verification/vendor/targets/esp32s31/vendor-project.toml \
  --report /tmp/esp32s31-oracle-regression.json \
...
`verify evidence` reproduces every `EVIDENCE-REGRESSION` and
`EVIDENCE-ADDITION` diagnostic and returns a failing status when an accepted
entry is missing or has another identity. It still writes the candidate so
the change can be reviewed, and prints the source report's SHA-256 so a review
record can bind the transferred protected-run result. Entries are sorted by
`(source, symbol)`, making the file stable across runs. The command refuses to
use the accepted baseline or the JSON report itself as `--candidate`;
accepting a reviewed candidate remains an explicit repository change.
Additions keep the existing non-regression semantics: they are visible in the
candidate but do not make an otherwise valid baseline fail.

## Results and evidence classes

For ROM, `verify source` and `verify inventory` map `phy_NAME` to
...
`--json-report PATH` writes a versioned machine-readable `verify inventory` result
...
`verify source` uses its named concrete profile. It promotes the result to `MATCH`
...
Verification analyzes exactly the paths supplied by the caller. Artifact revision
and authenticity checks belong in the invoking CI job or local harness. A new
chip or ROM revision therefore adds a target, platform pack and evidence
baseline, not a digest constant in the workbench.

`inspect trace` and `inspect compare` are available for focused investigation
of one symbol. Run the command without arguments for their complete syntax.
...
verification commands above remain the required private-oracle integration
checks. The repository CI runs formatting,
workspace tests, strict workbench-only clippy, PAC generation checks and the
