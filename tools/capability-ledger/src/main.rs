...
const USAGE: &str =
    "usage: cargo capability-ledger check --manifest PATH [--root PATH] [--json-report PATH]";
...
mod tests {
    ...
    fn command_requires_one_explicit_manifest() {
        let parsed = parse_arguments([
            "check".to_owned(),
            "--manifest".to_owned(),
            "capabilities/test.ledger".to_owned(),
        ])
        .unwrap();
        assert_eq!(parsed.manifest, Path::new("capabilities/test.ledger"));
