...
| Capability | Status | Current production boundary |
| --- | --- | --- |
| Executor-neutral controller core | PARTIAL | One final Controller owner now splits into disjoint finite ISR service, mutable DTM/scheduler task service and raw HCI/bootstrap endpoints. The task service retains the sole HAL task owner plus the exclusive list identity, so a head published before the split can cross dynamic interrupt preparation, synchronous scheduler event and hardware RUN without recovering the monolithic owner. Lock-free scheduler, lock/modify and timer cells replace RTOS broker objects; every hardware-owned wait returns control. A bounded timeline owns non-overlapping reservations and follows the same powered epoch through PHY, BTBB, BLE-PHY and stable interrupt publication. DTM event typestates retain their reservation through graph preparation, merge, safe first-head publication, the complete synchronous run suffix and fresh Controller-owned completion observation. The completion/recycle suffix is not yet available after the split, and a post-split HCI command still cannot prepare its first head. Lock/modify admission remains an explicit unsafe boundary outside that first empty-list path. Selector-6 policy, registered time-latch notification, Link Layer and live radio input remain absent. The software-only Embassy adapter is unchanged. |
