...
pub fn wait_after_progress(delay: Duration) {
    if !delay.is_zero() {
        eprintln!(
...
pub fn run(
    ...
    use oer_hil_protocol::StationPauseOperation as Op;
    ...
    for attempt in 1..=attempts {
        ...
        let timeout = if operation == oer_hil_protocol::StationPauseOperation::TrackingService {
            Duration::from_micros(oer_hil_protocol::STATION_TRACKING_SERVICE_WINDOW_MICROS)
...
fn has_nonzero_rfpll_correction(detail: Option<oer_hil_protocol::RfpllEvidence>) -> bool {
    detail
...
fn validate_pause(
    ...
    if evidence.result != oer_hil_protocol::StationPauseResult::Resumed {
    ...
    if !matches!(
        ...
        oer_hil_protocol::StationPauseOperation::Access
            ...
            | oer_hil_protocol::StationPauseOperation::TrackingService
    ) && !evidence
        .tracking
    ...
    if operation == oer_hil_protocol::StationPauseOperation::Calibration
        && !evidence
            .tracking
...
fn validate_tracking_parent(evidence: oer_hil_protocol::StationPauseEvidence) -> Result<()> {
...
fn validate_tx_waits(
    timings: Option<oer_hil_protocol::PhyTimingEvidence>,
    waits: Option<oer_hil_protocol::PhyTxWaitEvidence>,
) -> Result<()> {
    match (timings, waits) {
...
fn validate_service(
    ...
    use oer_hil_protocol::{STATION_TRACKING_SERVICE_WINDOW_MICROS, StationPauseOperation};
...
fn validate_rfpll(
    operation: oer_hil_protocol::StationPauseOperation,
    timings: Option<oer_hil_protocol::PhyTimingEvidence>,
    detail: Option<oer_hil_protocol::RfpllEvidence>,
            ...
            if detail.is_valid()
                ...
                && (!matches!(operation, Op::Rfpll | Op::RfpllObserved)
                    || detail.sample_age_micros.is_some_and(|age| {
                        age <= oer_hil_protocol::STATION_RFPLL_SAMPLE_MAX_AGE_MICROS
                    }))
                && (!require_nonzero_correction
                    || detail
                        .correction
...
fn validate_rx_gain(
    timings: Option<oer_hil_protocol::PhyTimingEvidence>,
    detail: Option<oer_hil_protocol::PhyRxGainEvidence>,
) -> Result<()> {
