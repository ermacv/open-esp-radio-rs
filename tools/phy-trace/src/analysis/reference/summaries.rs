...
//!
//! Vendor summaries are accepted only for an exact symbol name, load address,
//! and instruction body. A changed binary therefore falls back to ordinary
//! analysis and fails closed if that analysis cannot prove the behavior.

use super::*;
use crate::{
    BranchCondition, BranchOperation, IndexedMmioRegister, analysis::ESP32S31_ROM_DIVDI3_ADDRESS,
};
use sha2::{Digest, Sha256};

const MAX_REVIEWED_MEMORY_INTRINSIC_BYTES: u32 = 256;
const ROM_MEMCPY_SHA256: &str = "d6011767502f1ba4c9c3368aaede8afa5c436e7b154543ce65264b09742c059f";
const ROM_MEMSET_SHA256: &str = "50232f532bf996f5d738050c60480fab6069d6ef4cb842cfc582170e351630c6";
const ROM_DIVDI3_SHA256: &str = "0370c514570629a8ae4a10747014c6510679e822572c78e75d97a5da6e983cda";
const ROM_DIVDI3_SIZE: usize = 926;
const ROM_PHY_WAIT_RFPLL_CAL_END_SHA256: &str =
    "f9c4486efcf67dee40dc43ddf0357b8fbd000a762c3090544814cfa77145286e";
...
const ROM_PHY_RFPLL_CAP_INIT_CAL_SHA256: &str =
    "15fb1d7873ad0feaa01b5645b542b38cc3886807d5d9b979b1b8d5084595f8d3";
...
const ROM_PHY_SET_RF_FREQ_OFFSET_SHA256: &str =
    "c980b21358bd4854fe271d23456385b08981569a1429c6869a952d4b4fd5130d";
...
const ROM_PHY_IQ_EST_ENABLE_SHA256: &str =
...
fn exact_standard_memory_intrinsic(symbol: &artifact::ArtifactSymbolDefinition) -> bool {
    let expected = match (symbol.name.as_str(), symbol.address, symbol.bytes.len()) {
        ("memcpy", 0x2f80_d260, 224) => ROM_MEMCPY_SHA256,
        ("memset", 0x2f82_20c6, 168) => ROM_MEMSET_SHA256,
        _ => return false,
    };
    format!("{:x}", Sha256::digest(&symbol.bytes)) == expected
}
...
fn exact_wide_signed_divide(symbol: &artifact::ArtifactSymbolDefinition) -> bool {
    let digest = format!("{:x}", Sha256::digest(&symbol.bytes));
    reviewed_digest_matches(
        ...
        ReviewedBodyIdentity {
            name: "__divdi3",
            address: u64::from(ESP32S31_ROM_DIVDI3_ADDRESS),
            size: ROM_DIVDI3_SIZE,
            digest: ROM_DIVDI3_SHA256,
        },
    )
}
...
fn exact_rfpll_calibration_poll(symbol: &artifact::ArtifactSymbolDefinition) -> bool {
    ...
    reviewed_digest_matches(
        ...
        ReviewedBodyIdentity {
            ...
            digest: ROM_PHY_WAIT_RFPLL_CAL_END_SHA256,
        },
    )
}
...
fn exact_rfpll_cap_calibration_search(symbol: &artifact::ArtifactSymbolDefinition) -> bool {
    ...
    reviewed_digest_matches(
        ...
        ReviewedBodyIdentity {
            ...
            digest: ROM_PHY_RFPLL_CAP_INIT_CAL_SHA256,
        },
    )
}
...
fn exact_rf_frequency_offset_scratch_wrapper(symbol: &artifact::ArtifactSymbolDefinition) -> bool {
    ...
    reviewed_digest_matches(
        ...
        ReviewedBodyIdentity {
            ...
            digest: ROM_PHY_SET_RF_FREQ_OFFSET_SHA256,
        },
    )
}
...
fn exact_iq_estimator_poll(symbol: &artifact::ArtifactSymbolDefinition) -> bool {
    ...
    reviewed_digest_matches(
        ...
        ReviewedBodyIdentity {
            ...
            digest: ROM_PHY_IQ_EST_ENABLE_SHA256,
        },
    )
}
...
fn rf_frequency_offset_scratch_trace(
    ...
    let mut arguments = core::array::from_fn(|_| SymbolicValue::Unknown);
        ...
        mmio(
            ...
            Some(
                SymbolicValue::register_read(2, CONFIG1, 32, false)
                    .and(0xfffe_0003)
                    .bitor(SymbolicValue::input(1).shift_left(2).and(0x0001_fffc)),
            ),
        ),
        mmio(MemoryAccess::Read, CONFIG1, None),
        mmio(
...
fn rfpll_cap_calibration_search_trace(
    ...
    let no_arguments = core::array::from_fn(|_| SymbolicValue::Unknown);
...
fn rfpll_calibration_poll_trace(symbol: &artifact::ArtifactSymbolDefinition) -> FunctionAnalysis {
    let mut read_arguments = core::array::from_fn(|_| SymbolicValue::Unknown);
...
pub(super) fn reference_intrinsic_trace(
    ...
    let intrinsic_arguments = core::array::from_fn(|index| SymbolicValue::input(index as u8));
...
fn phy_table_targets(pointer_context: &StructuralPointerContext) -> Option<(u32, u32, u32)> {
    [
        entry_contract::FunctionTable::Esp32s31PhyCold,
        entry_contract::FunctionTable::Esp32s31PhyRegistered,
    ]
    .into_iter()
    .find_map(|table| {
        Some((
...
fn chip_i2c_write_reg_trace(
    ...
    let command = SymbolicValue::input(3)
        .shift_left(16)
        .bitor(SymbolicValue::input(0))
        .bitor(SymbolicValue::input(2).shift_left(8))
        .or(0x0500_0000);
    let mut exit_arguments = core::array::from_fn(|_| SymbolicValue::Unknown);
...
fn host_id_trace(
    ...
    let host_one = HOST_ONE_BLOCKS
        ...
        .fold(SymbolicValue::Constant(0), SymbolicValue::bitor);
...
mod tests {
    ...
    fn exact_host_id_body_has_a_modeled_return_and_mmio_effect() {
        ...
        assert!(trace.reference_exit_a0_modeled());
    ...
    fn i2c_write_summary_requires_exact_body_and_phy_entry_contract() {
        ...
        for (offset, target) in [(0x00, 1), (0x04, 2), (0x0c, 3)] {
            context.function_table_slots.insert(
                (entry_contract::FunctionTable::Esp32s31PhyRegistered, offset),
                target,
            );
        }
    ...
    #[test]
    fn wide_divide_identity_requires_exact_name_address_size_and_digest() {
        assert!(reviewed_digest_matches(
            ReviewedBodyIdentity {
                name: "__divdi3",
                address: u64::from(ESP32S31_ROM_DIVDI3_ADDRESS),
                size: ROM_DIVDI3_SIZE,
