//! Minimal stable projection of schema-v30 linked IR used by function review.
...
impl FunctionFacts {
    pub(crate) fn load(reports: &[(String, PathBuf)]) -> Result<Self> {
        ...
        for (profile, path) in reports {
            ...
            if integer(root, "schema_version", &context)? != 30
                || string(root, "command", &context)? != "ir-export"
            {
                return Err(format!(
                    "function workspace requires a schema-v30 ir-export report for profile {profile:?}"
                )
                .into());
            }
