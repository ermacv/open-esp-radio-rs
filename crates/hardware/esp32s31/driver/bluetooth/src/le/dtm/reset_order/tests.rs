...
use super::{DtmRestoredReset, DtmRestoredResetCompletion};
...
fn restored_reset_retains_affinity_and_response_through_backpressure() {
    ...
    let restored = DtmRestoredReset::new(barrier.map_owner(|()| RestoredOwner(41)));

    let mut foreign_resources = resources();
    let mut foreign = foreign_resources.split();
    let DtmRestoredResetCompletion::EndpointMismatch(restored) =
    ...
    let DtmRestoredResetCompletion::ResponsePending(pending) =
