...
use crate::controller::boot::publication::ControllerOutputActivation;
...
use {
    crate::controller::ControllerInterruptOwnersReady,
...
pub enum ControllerRestartError<E> {
    ...
    Clock(oer_esp32s31_bluetooth::clock::ClockError),
    LowPower(oer_esp32s31_hal::bluetooth::BluetoothModemLpTimerOwnerError),
    Phy(oer_esp32s31_bluetooth::phy::PhyInitializationError),
...
struct Retained<'a, P, S, const SC: usize, const MT: usize> {
    ...
    memory: Option<oer_esp32s31_bluetooth::ble_phy::BlePhyRetiredMemory>,
    hci: oer_bluetooth_hci::LeControllerHciRetired<'a, ()>,
    timer_runtime: oer_esp32s31_bluetooth::runtime_resources::ControllerModemTimerRuntime<'a, MT>,
    timer_storage: &'a S,
...
enum Stage<'a, P, S, const SC: usize, const MT: usize> {
    ...
    Clock(oer_esp32s31_bluetooth::clock::ClockEnableFailure<P>),
    LowPower(
        oer_esp32s31_bluetooth::low_power::ControllerLowPowerHardwareInitializationFailure<
            P,
            MT,
            SC,
        >,
...
pub struct ControllerRestarted<
    'a,
    P,
    S: oer_esp32s31_bluetooth::modem_timer::ModemLpTimerSoftwareOwnerStorage,
    M: RawMutex,
    const SC: usize,
    const MT: usize,
    const H2C: usize,
...
fn complete_initialization<
    ...
    const PC: usize,
>(
    initialized: oer_esp32s31_bluetooth::common_phy_state::ControllerPhyInitialized<P, MT, SC>,
...
>(
    ...
    parts: oer_esp32s31_bluetooth::ble_phy::BlePhyRestartParts<P, MT, SC>,
    controller: &mut LeControllerCommandEndpoint<'a, M, H2C, C2H, PC>,
) -> ControllerRestarted<'a, P, S, M, SC, MT, H2C, C2H, PC> {
    let Retained {
        mut software,
    ...
    let oer_esp32s31_bluetooth::ble_phy::BlePhyRestartParts {
        scheduler,
        physical,
        timing,
        direction_finding,
    } = parts;
    let oer_esp32s31_bluetooth::scheduler::core::SchedulerRestartParts {
        task,
        platform,
        time_scale,
        config,
    ...
    software
        .runtime
        .restore_scheduler_epoch(time_scale, config, scheduler_list);
    *software.scheduler_epoch = None;
    software.ble_phy_timing = timing;
    software.direction_finding_workspace = direction_finding;
    timer_runtime.restore_epoch(runtime.into_started_modem_epoch());
