...
use super::{
    ...
    SemanticVerificationReport, SemanticVerificationSummary, StateFootprintStats,
};
use crate::*;
...
fn single_case_report(
    ...
    artifacts: Vec<SemanticVerificationArtifact>,
    result: &execution::ExecutionResult,
    case: SemanticVerificationCase,
...
pub fn verify_esp32s31_channel(
    ...
    for (case_index, (name, channel_or_frequency, cbw)) in cases.into_iter().enumerate() {
        ...
        let result = vendor_session
            .execute(&image, svd, "phy_chip_set_chan", scenario)
            .map_err(|error| {
                format!(
                    "esp32s31-channel case {name} failed while executing vendor phy_chip_set_chan: {error}"
                )
            })?;
        ...
        if vendor_events != rust_events {
            ...
            case_reports.push(SemanticVerificationCase {
                ...
                call_events: Some(result.ordered_calls.len()),
                state: Some(footprint.into()),
                difference: Some(SemanticVerificationDifference {
