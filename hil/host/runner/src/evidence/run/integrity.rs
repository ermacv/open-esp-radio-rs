...
use crate::evidence::run::{
    Attachment, IntegrityFile, IntegrityIndex, RUN_SCHEMA, UNIQUE_FILE_COUNTER,
};
...
pub(crate) fn write_integrity_index(directory: &Path, run_id: &str) -> Result<PathBuf> {
...
pub(crate) fn collect_integrity_files(directory: &Path) -> Result<Vec<IntegrityFile>> {
