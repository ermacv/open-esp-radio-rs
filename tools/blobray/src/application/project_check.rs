...
#[derive(Serialize)]
pub(crate) struct ProjectCheckReport {
    pub(crate) schema: u32,
    pub(crate) command: &'static str,
    pub(crate) project: String,
    pub(crate) passed: bool,
    pub(crate) stages: Vec<ProjectCheckStage>,
}

#[derive(Serialize)]
pub(crate) struct ProjectCheckStage {
    pub(crate) name: &'static str,
    pub(crate) status: &'static str,
    pub(crate) passed: bool,
    pub(crate) summary: String,
    #[serde(skip_serializing_if = "Vec::is_empty")]
    pub(crate) issues: Vec<ProjectCheckIssue>,
    #[serde(skip_serializing_if = "Vec::is_empty")]
    pub(crate) next_steps: Vec<FollowUpStep>,
}

#[derive(Serialize)]
pub(crate) struct ProjectCheckIssue {
...
pub(crate) struct ProjectCheckRequest {
    pub(crate) deny_unreviewed: bool,
    pub(crate) jobs: usize,
}

pub(crate) fn execute(
    arguments: ProjectCheckRequest,
    session: &ProjectSession,
    verify: impl FnOnce(&ProjectSession) -> Result<crate::verification::ProjectVerificationReport>,
    ...
    let bindings_passed = binding_audit.as_ref().is_ok_and(|audit| audit.passed);
    ...
    let verification = if session.mmio.regions.is_empty() {
        Err(crate::Error::invalid(
            "project verification requires an MMIO region; add memory-map to the project",
        ))
    } else {
        verify(session)
    };
    let verification_passed = verification.as_ref().is_ok_and(|report| report.passed);
    ...
    let passed = ownership.passed()
        ...
        && verification_passed
        && policy.passed
        && publication.as_ref().is_ok_and(|report| report.succeeded());
    let analysis_passed = analysis.succeeded();
    let verification_stage = match verification {
        Ok(verification) => {
        ...
        Err(error) => ProjectCheckStage {
            ...
            status: "failed",
            passed: false,
            summary: "verification could not run".to_owned(),
            issues: vec![ProjectCheckIssue {
                component: "verification".to_owned(),
    ...
    let binding_stage = match binding_audit {
        Ok(binding_audit) => ProjectCheckStage {
        ...
        Err(error) => unavailable_stage("binding-declarations", error),
    };
    let publication_stage = match publication {
        Ok(publication) => {
        ...
        Err(error) => unavailable_stage("publication", error),
    };
...
fn unavailable_stage(name: &'static str, error: crate::Error) -> ProjectCheckStage {
    ProjectCheckStage {
        name,
        status: "failed",
        passed: false,
        summary: "stage could not run".to_owned(),
        issues: vec![ProjectCheckIssue {
