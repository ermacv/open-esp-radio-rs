//! Selected schema-v61 linked-IR evidence used by the manual register report.
...
mod tests {
    ...
    #[test]
    fn loads_and_merges_current_schema_field_evidence() {
        let base =
            std::env::temp_dir().join(format!("blobray-register-review-ir-{}", std::process::id()));
    ...
    fn rejects_schema_drift_and_invalid_field_masks() {
        let path = std::env::temp_dir().join(format!(
            "blobray-register-review-ir-invalid-{}",
            std::process::id()
        ));
        ...
        assert!(error.to_string().contains("expected schema_version 61"));
    ...
    fn rejects_the_same_report_twice() {
        let path = std::env::temp_dir().join(format!(
            "blobray-register-review-ir-duplicate-{}",
            std::process::id()
        ));
        write_report(&path, &report(1, "rtos.event.send"));
