...
use open_radio_vendor_semantics::{
    DriverAdapterCase, DriverAdapterRequest, DriverAdapterVerification, EffectDisposition,
    EffectPolicy, EffectSelector,
};

use crate::{Result, artifact, execution};
...
pub fn verify_esp32s31_wifi_key_role(
    ...
    let vendor_inventory = request
        .vendor_inventory
        .ok_or("Wi-Fi key-role verification requires the caller-owned raw libpp inventory")?;
    let vendor_projection = validate_vendor_projection(vendor_inventory, request.vendor_artifact)?;

    let mut vendor_image = execution::ExecutableImage::load(request.vendor_artifact)?;
    if let Some(companion) = request.vendor_companion {
        vendor_image.add_companion(companion)?;
    }
    let mut rust_image = execution::ExecutableImage::load(request.rust_artifact)?;
    if let Some(companion) = request.rust_companion {
    ...
    let mut canonical = format!(
        ...
        code_closure_sha256(&rust_image, request.rust_symbol)?,
    );
