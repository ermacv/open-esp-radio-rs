...
| Legacy advertising | PRE-ADMISSION | A hardware-independent `open-esp-radio-bluetooth-ll` foundation validates and encodes exact `ADV_NONCONN_IND` PDUs, retains public/random TxAdd semantics and advances an affine non-connectable lifecycle over a non-empty channel 37/38/39 map only after an exact generation/event/channel completion. The S31 chip boundary now consumes a pinned physical-SRAM graph, installs the PDU directly in its role-neutral TX allocation, binds the common buffer header to that packet and retains exact portable identity plus primary-channel frequency; cancellation returns both affine owners. Link-state and scheduler-item capacity is reserved but remains inaccessible and zero-based. The owner deliberately cannot mint `InFlight` or publish `HEAD`/`RUN` until the advertising-specific descriptor reset, timing/list role and completion ticket are reviewed. HCI advertising commands, response-capable mode and on-air evidence also remain absent. |
