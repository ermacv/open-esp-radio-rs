...
| Interrupt masks and CPU route | PARTIAL | Primary source 124, modem-timer source 127 and NRT source 133 are typed policies on the configured Controller core at level 3; primary and modem timer request IRAM residency, while NRT does not. The pinned external PAC exposes typed `BT_MAC`, `MODEM_LP_TIMER` and `BT_MAC_INT1` identities, so the ESP-HAL adapter compile-checks the complete three-source set without raw casts. Its two process-wide critical-section slots atomically retain the primary/NRT register owner and the separate source-127 timer owner before routing. Source 127 cannot repeat MMIO while its owner is software-pending; Controller core durably coalesces both the first pending transition and repeat entry into one task wake epoch, owns the lossless queue/publication/rearm path and retains a failed stable restore. All three finite platform dispatch paths plus core-owned source-124 and source-127 publication boundaries now exist, but the public live-route lifecycle remains closed until executor notification and selector-6 recovery can complete. Baseline fault and dynamic scheduler masks are exact. The 16-bit finished-list transfer and current hardware-list observation are typed, and a bounded task-side worker yields one positional finished-list index per step without claiming an item completion. Current and finished-list observations share the exact hardware-list domain, but per-list affine-item ordering/selection, selector 4 scanner resume, selector 6 post-clear action and feature-specific NRT policy remain missing. A separate no-RTOS Embassy adapter turns the atomic scheduler cell into an affine register-before-recheck wake channel with cancellation-safe coalescing. |
| Controller timer/scheduler | FAIL-CLOSED | The complete controller HAL component and following sixteen-entry scheduler-table prefix are live in their recovered order, and the selected controller-time scale stays bound to that hardware epoch. Initialization retains affine proof that every hardware head was cleared and combines it with an exclusive empty source-owned list. The scheduler can join that epoch to the exact DTM item-side transform as one identity-checked, cancellable first-item merge. Only the terminal powered Controller with both interrupt owners in stable storage and every CPU route inactive can consume the matching merge into hardware-head publication. The PAC orders the complete descriptor graph before the generated MMIO accessor and applies a trailing device fence; pre-MMIO identity or empty-head rejection returns the unchanged merge, while success retains the graph and removes cancellation/CPU mutation. The restricted PAC models BTMAC source 14 as generated W1C clear and interrupt-enable accessors. Its safe affine suffix consumes the published head and dynamic-interrupt proof, performs synchronous `clear stale source -> enable source -> fence`, and permits RUN only by consuming that result. The ESP-HAL stable-storage capability lends the sole shared interrupt owner for the dynamic preparation without moving it or enabling a CPU route. The terminal Controller composes the complete suffix and returns a `BluetoothDtmSchedulerRunning` owner; storage rejection occurs before suffix MMIO and returns the unchanged published head. The task-side finished-list mask transfer and later ISR current-list observation are typed, but neither can yet select this affine running item or establish completion visibility. Conditional lock/modify and execution-lock/modify requests have finite PAC/HAL publications and semantic observations; BUSY remains interrupt-owned and no operation polls. The three insertion-begin outcomes and complete insertion-end command/head/BUSY/sleep/item-status decision order remain typed staged plans. Source 124 and source 127 retain their durable bounded handoffs, while a proven time wake/recheck source, selector-6 invariant, live routes, per-list completion selection, packet visibility/recycle and powered teardown remain absent. |
| Controller memory lists | FAIL-CLOSED | Three positional global RX selectors remain restricted to PAC accessors; the memory layer owns their scanner/non-scanner routing, while DTM kind 5 bypasses them and uses its private graph. Static binding installs every reviewed link and allocation field from explicit source-owned inputs. TX preparation copies the complete declared packet, the bounded timeline supplies a two-sample sequence-ready reservation, and role-specific plans install all nineteen reviewed link-state/scheduler words plus scheduler bookkeeping. The empty-list transform nulls both next links and is cancellable until the scheduler consumes it. The terminal pre-route Controller now composes that memory owner with its exact exclusive list and publishes the first head after a release-side fence; the returned state exposes neither mutation nor cancellation. The current software-list-removal gate is also a finite typed PAC/HAL observation. Hardware current/next rotation, completion-side visibility, item selection/reclamation and the removal worker remain absent. The current blob has no separate software DTM pointer latch. |
...
| Capability | Status | Current production boundary |
| --- | --- | --- |
| Direct Test Mode | ABSENT | The exact S31 TX/RX/Test-End command bodies, allocator roles, common scheduler path and recycle callback are mapped. Channel/PHY selection, all eight TX patterns, packet duration, interval/tick arithmetic, initial/recurring windows and bounded overlap resolution are typed. The 936-byte graph is a non-movable static allocation with exact RX/TX headers, private-chain anchors, scheduler context, scheduler-item-to-link-state link and role-specific event/bookkeeping transforms; TX readiness survives every CPU-owned transition. Scheduler initialization proves all heads and its source-owned software list empty, and the first merge is identity-checked and cancellable. The fully initialized pre-route Controller can consume that merge into list-zero head publication with descriptor-before-head and trailing fences, then admit the retained head through dynamic interrupt preparation, the synchronous BTMAC source-14 event and RUN. Success returns a non-cancellable, non-mutable running graph; it does not claim completion. Static analysis of the complete current DTM event body proves there is no separate software packet-engine latch: publication follows `head -> item +0x08 -> link state -> private TX/RX links`. The RX result/accounting and ordinary re-arm transforms are also present, while swap-reserve remains quarantined. Live clock/RF-ready/margin inputs, a public HCI command-to-running composition, per-list completion selection, acquire-side completion visibility, owned recycle/quiesce and operational HCI opcodes are missing, so the production HCI path still cannot start an on-air command. |
