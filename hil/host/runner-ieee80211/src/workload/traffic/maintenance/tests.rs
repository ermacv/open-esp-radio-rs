...
fn parent_timing(common: bool, wifi: bool) -> oer_hil_protocol::PhyTimingEvidence {
    use oer_hil_protocol::{PhyOperationTiming, PhyTimingEvidence};
    ...
    PhyTimingEvidence {
        ...
        dcode_polls: oer_hil_protocol::PhyPollTiming {
            polls: u32::from(common),
            ..Default::default()
        },
        rx_gain_polls: oer_hil_protocol::PhyPollTiming {
            polls: u32::from(common),
            ..Default::default()
        },
        tx_dc_pwdet_polls: oer_hil_protocol::PhyPollTiming {
            polls: u32::from(wifi),
            ..Default::default()
        },
        ..Default::default()
...
fn physical_timeline_cannot_be_missing_reversed_or_smaller_than_nested_work() {
    ...
    use oer_hil_protocol::StationPauseTimeline;
...
fn repeated_rfpll_polling_stops_only_for_a_nonzero_correction() {
    use oer_hil_protocol::{RfpllCorrectionEvidence, RfpllEvidence};
...
fn resumed_pause_rejects_incomplete_failed_or_invalid_timing() {
    ...
    {
        let mut timings = oer_hil_protocol::PhyTimingEvidence {
            invalid,
            ..Default::default()
        };
        timings.dcode = oer_hil_protocol::PhyOperationTiming {
            started,
            completed,
            failed,
            ..Default::default()
...
fn tx_detail_is_required_exactly_when_operation_timing_is_available() {
    ...
    use oer_hil_protocol::{PhyTimingEvidence, PhyTxWaitEvidence};
...
fn service_window_requires_repeated_observations_and_rejects_faults_or_unrelated_detail() {
    let report = oer_hil_protocol::StationTrackingServiceEvidence {
        operations: [4, 0, 0, 0, 0, 0],
        elapsed_micros: oer_hil_protocol::STATION_TRACKING_SERVICE_WINDOW_MICROS,
        pause_micros: 100,
...
fn observed_rfpll_rejects_missing_or_stale_sensor_age() {
    use oer_hil_protocol::{PhyTimingEvidence, RfpllEvidence, STATION_RFPLL_SAMPLE_MAX_AGE_MICROS};
    ...
    for (sample_age_micros, valid) in [
        ...
        let measured = oer_hil_protocol::RfpllEvidence {
            threshold: 0,
            correction: Some(oer_hil_protocol::RfpllCorrectionEvidence {
                initial_cap: 100,
                selected_cap: 101,
                accepted_samples: 3,
                entries_updated: 85,
...
fn temperature_prerequisite_requires_a_completed_acquisition_only() {
    use oer_hil_protocol::PhyTimingEvidence;
    let mut timings = PhyTimingEvidence::default();
    let evidence = StationPauseEvidence {
        timeline: None,
        result: StationPauseResult::Resumed,
...
fn temperature_detail_is_required_for_acquisition_and_full_parent_operations() {
    let detail = oer_hil_protocol::TemperatureEvidence {
        temperature: -12,
        sensor_index: 4,
        next_dac: 10,
    };
...
fn synthetic_pause_requires_its_hold_and_cannot_hide_phy_work() {
    ...
    let mut evidence = StationPauseEvidence {
        ...
        timings: None,
        tracking: None,
        result: oer_hil_protocol::StationPauseResult::Resumed,
        elapsed_micros: 9_999,
    };
    assert!(validate_pause(operation, evidence).is_err());
    evidence.elapsed_micros = 10_500;
    assert!(validate_pause(operation, evidence).is_ok());
    evidence.tracking = Some(oer_hil_protocol::StationPhyTrackingEvidence {
        inhibited: false,
        common_calibrated: false,
        wifi_calibrated: false,
        bluetooth_ieee802154_calibrated: false,
...
fn rx_gain_detail_is_required_exactly_when_timing_is_available() {
    ...
    use oer_hil_protocol::{PhyRxGainEvidence, PhyTimingEvidence};
...
fn rx_gain_phases_must_be_complete_and_fit_the_parent() {
    ...
    let execution = Some(PhyRxGainExecutionEvidence {
        quality: Some(oer_hil_protocol::PhyRxGainQualityEvidence::default()),
