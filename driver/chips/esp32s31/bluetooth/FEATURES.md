...
| Controller initialization | FAIL-CLOSED | The target typestate preserves the recovered cold order from clock/reset through HAL, scheduler, bounded HCI, source-127 hardware, common PHY, BTBB, BLE PHY, Controller output, runtime timer and stable interrupt-owner publication. The terminal owner can be placed statically, split exactly once and composed with all three live CPU routes, durable Embassy notification, a standard `bt_hci::ExternalController` Host facade and one eternal hardware runner. DTM current/RF-ready acquisition, scheduler admission, HEAD/RUN publication, completion drain and cancellation retain the exact Controller epoch. Production cold-start configuration and static placement, unrelated-list routing, Link Layer/ACL, sleep-enabled RF wake, complete descriptor semantics, powered teardown and HIL remain missing. |
