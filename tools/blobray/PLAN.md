...
| 22.5 | pending | Triage every untriaged location of the index, starting with functions that have a production counterpart and surviving mutants: a follow-up case that reaches it with an expected verdict, or a reviewed decision with its reason. Production-implemented vendor paths (for example the channel-14 MIC and 802.11p configuration) are cases, not exclusions. The stage closes with an empty untriaged list. |
