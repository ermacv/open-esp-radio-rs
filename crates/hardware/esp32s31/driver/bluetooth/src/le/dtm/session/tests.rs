use crate::le::dtm::{
    DtmDefaultTxPowerDbm, DtmPayloadLength, DtmPayloadPattern, DtmTxGraphPrepare,
    event::prepare::DtmQuiescedCpuOwned,
};

use oer_esp32s31_bluetooth_memory::{
    DtmMemoryGraphCpuOwned, DtmMemoryGraphModelAddress, DtmMemoryGraphStorage,
    DtmSchedulerAllocationConfig,
};

use super::{DtmRuntimeConfig, DtmRuntimeResources, DtmRuntimeSessionBeginError, DtmSessionIdle};

const fn runtime_config() -> DtmRuntimeConfig {
    DtmRuntimeConfig::new(
        DtmSchedulerAllocationConfig::new(1, 2, 1),
        DtmDefaultTxPowerDbm::new(7),
    )
}

fn graph_at(address: u32) -> DtmMemoryGraphCpuOwned {
    let storage = std::boxed::Box::leak(std::boxed::Box::new(DtmMemoryGraphStorage::new()));
    let base = DtmMemoryGraphModelAddress::new(address)
        .expect("model address uses the controller SRAM syntax");
    let config = DtmSchedulerAllocationConfig::new(1, 2, 1);
    DtmMemoryGraphStorage::pin_static_model(storage, base, config)
        .expect("model graph fits in controller SRAM")
}

fn graph() -> DtmMemoryGraphCpuOwned {
    graph_at(0x2f00_6800)
}

#[test]
fn idle_reinitializes_the_same_graph_for_each_cpu_owned_epoch() {
    let prepared = DtmSessionIdle::new(graph())
        .begin_epoch()
        .into_graph()
        .prepare_dtm_tx_packet(
            DtmPayloadPattern::Prbs15,
            DtmPayloadLength::from_hci_image(5),
        );

    assert_eq!(prepared.pattern(), DtmPayloadPattern::Prbs15);
    assert_eq!(prepared.length().hci_image(), 5);

    let prepared = DtmSessionIdle::new(prepared.discard())
        .begin_epoch()
        .into_graph()
        .prepare_dtm_tx_packet(
            DtmPayloadPattern::Repeated00001111,
            DtmPayloadLength::from_hci_image(2),
        );

    assert_eq!(prepared.pattern(), DtmPayloadPattern::Repeated00001111);
    assert_eq!(prepared.length().hci_image(), 2);

    let _idle = DtmSessionIdle::new(prepared.discard());
}

#[test]
fn cancelling_graph_ready_returns_the_graph_to_idle_retention() {
    let idle = DtmSessionIdle::new(graph()).begin_epoch().cancel();
...
fn terminal_neutral_quiesced_graph_restores_the_exact_runtime_slot() {
    let mut runtime =
        DtmRuntimeResources::from_claimed_graph(runtime_config().default_tx_power_dbm(), graph());
    let active = runtime
        .begin_session_epoch()
    ...
    let quiesced = DtmQuiescedCpuOwned::from_cpu_owned_for_test(active);
...
fn runtime_checkout_is_exclusive_and_cancelled_session_restores_it() {
    ...
    assert!(matches!(
        ...
        Err(DtmRuntimeSessionBeginError::SessionActive)
    ));
...
fn vacant_runtime_rejects_a_foreign_graph_without_losing_either_owner() {
    let mut runtime = DtmRuntimeResources::from_claimed_graph(
    ...
    let mut foreign_runtime = DtmRuntimeResources::from_claimed_graph(
        runtime_config().default_tx_power_dbm(),
