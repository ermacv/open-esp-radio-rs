...
fn project() -> std::path::PathBuf {
    repository_root().join("verification/vendor/projects/esp32s31/vendor-project.toml")
}
...
fn checked_register_publications_are_typed_reports() {
    for (arguments, expected_status) in [
        ...
        (vec!["registers", "generate-pac-api", "--check"], "verified"),
        (
        ...
        let output = blobray()
            .args(arguments)
            .arg("--project")
            .arg(repository_root().join("registers/esp32s31/publication/vendor-project.toml"))
        ...
        if expected_status == "valid" {
            assert_eq!(
                document["lints"]["schema"], 1,
                "source-only validation must enforce the shared register lint policy"
            );
            assert_eq!(
                Path::new(document["lints"]["pack"].as_str().unwrap())
                    .canonicalize()
                    .unwrap(),
                repository_root()
                    .join("registers/esp32s31/policy/lints.toml")
                    .canonicalize()
                    .unwrap()
            );
            assert_eq!(
                document["evidence"]["catalogs"], 9,
                "source-only validation must check every reviewed register evidence catalog"
            );
            assert!(document["evidence"]["sources"].as_u64().unwrap() > 0);
        }
    }
}
...
#[test]
#[ignore = "requires local vendor artifacts and generated Blobray findings"]
fn inspect_function_schema_sixteen_has_one_fail_closed_blocker_route() {
    let output = blobray()
        .args([
            "inspect",
            "function",
