//! Selected schema-v30 linked-IR evidence used by the manual register report.
...
mod tests {
  ...
  "mmio_registers": [{{
    ...
    #[test]
    fn loads_and_merges_schema_v30_field_evidence() {
        let base = std::env::temp_dir().join(format!(
            "vendor-validator-register-review-ir-{}",
            std::process::id()
        ));
    ...
    fn rejects_schema_drift_and_invalid_field_masks() {
        let path = std::env::temp_dir().join(format!(
            "vendor-validator-register-review-ir-invalid-{}.json",
            std::process::id()
        ));
        std::fs::write(
            &path,
        ...
        assert!(error.to_string().contains("requires linked-IR schema 30"));
    ...
    fn rejects_the_same_report_twice() {
        let path = std::env::temp_dir().join(format!(
            "vendor-validator-register-review-ir-duplicate-{}.json",
            std::process::id()
        ));
