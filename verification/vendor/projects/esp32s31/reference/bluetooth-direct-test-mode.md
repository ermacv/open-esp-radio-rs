...
This reference describes the authenticated DTM descriptor, scheduler, timing
...
facts remain bounded to the pinned inputs. No LE Controller currently composes
DTM; supported HCI commands belong to the portable
...
The fixed `0x71764129` word is retained by a private protocol-level LE
...
The fixed low-24-bit `0x555555` image is retained by a private
...
The reset transaction itself is reproduced as a non-publishable reviewed
...
does not invent separate hardware names for the two bits.
...
event does not write this word. The composed Rust plan retains that
post-reset distinction; the value remains a CPU-owned positional word rather
than a named packet-engine field.

The scheduler-item update is a second non-publishable reviewed region.
...
The channel domain must come from an authenticated caller or entry contract.
A byte load at caller-owned `arg0+0x0e` remains an observed memory input; table
size alone cannot bound its value. Generic analysis must retain uncertainty
when that input domain is unproved. This does not affect the checked DTM
parameter types used by production code.
...
carries that outcome as a divergent platform boundary in generated
...
Address zero is not modeled as RAM; non-assert logging levels retain their
...
but is unrepresentable for RX. The Rust event enforces this asymmetry and
...
The lower packet codec validates the standard Type domain before its first
...
Raw header-image constructors are not part of the open API. The
...
consumer requires the publication and device-fence contract described below.
...
The complete tick tail is closed as source-owned S31 behavior. In current
...
The source-owned standalone scheduler policy retains the exact margin 107;
...
The transmitter window above the interval is exact as pure scheduler
...
async bottom half without changing the positional start/end images.
The initial TX window and both RX phases compose directly into the reviewed
...
branch enters the append routine. Complete get/append bodies show that bit
...
graph identity remains inseparable from hardware publication authority.
...
`0x18000000` does not change that complete image. The open static binding
...
open memory transaction models this exact bounded rotation; it exposes no
...
The two register slots in each selector pair have distinct directions.
...
`r_btdm_sched_insert_with_lock_modify`. The named reference
...
The list index has an explicit source contract. Named `r_sched_txn_insertIntoList`
...
and completion visibility follows the reviewed memory contract. Official
...
affine result of that transaction is consumed. The memory layer models
...
The always-awake time-read prefix is exact. Complete
...
`SLEEP_TIMER_LATCHED_TIME_0` at `0x2010_10ac`. Exact same-chip reference-body
