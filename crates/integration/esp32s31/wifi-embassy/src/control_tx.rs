...
// SOURCE: complete `_oracles/libnet80211.a[ieee80211_output.o]` passes
// coexistence events 5/6 for Probe and Authentication/Association. Complete
// `_oracles/libcoexist.a[coexist_core.o]::coex_pti_tab` maps both to PTI one;
// complete `_oracles/libpp.a[hal_mac.o,hal_coex.o]::
...
/// Bounded polling policy used only until the production MAC IRQ runner owns
/// completion wakeups.
...
mod tests {
    ...
    fn authentication_is_encoded_and_completed_by_the_shared_owner() {
        ...
        let result = embassy_futures::block_on(tx.transmit_open_authentication(
            &mut hardware,
    ...
    fn eapol_uses_the_recovered_voice_data_priority() {
        ...
        let result = embassy_futures::block_on(tx.transmit_unprotected_data(
            &mut hardware,
            StaDataFrame {
                source: [2, 3, 4, 5, 6, 7],
                bssid: [0x20, 0x21, 0x22, 0x23, 0x24, 0x25],
    ...
    fn active_handoff_returns_tx_and_crypto_resources_for_later_retry() {
        ...
        assert_eq!(
            embassy_futures::block_on(tx.ordinary.service_polling(&mut hardware, 1)),
