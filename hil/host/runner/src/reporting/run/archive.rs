...
use super::{FirmwareArtifact, FirmwareReplayOrigin, RunSession, atomic_json};
use crate::reporting::build::{self, BuildSubject, BuildSubjectRole};
...
impl RunSession {
    ...
    pub(crate) fn record_replayed_firmware(
        &mut self,
        archived: &super::super::verification::ArchivedFirmware,
    ) -> Result<PathBuf> {
        build::verify_sources_unchanged(&self.repository_root, &self.source_materials)?;
