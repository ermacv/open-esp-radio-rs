...
use super::{
    BLUETOOTH_CONTROLLER_PHYSICAL_SRAM_HIGH, PassiveScanMemoryGraphBindError,
    PassiveScanMemoryGraphModelAddress, PassiveScanMemoryGraphStorage,
    PassiveScanSchedulerAllocationConfig,
};

fn reset_config() -> PassiveScanResetConfig {
...
fn model_graph(base: u32) -> super::PassiveScanMemoryGraphCpuOwned {
    let storage = std::boxed::Box::leak(std::boxed::Box::new(PassiveScanMemoryGraphStorage::new()));
    let address = PassiveScanMemoryGraphModelAddress::new(base)
        .expect("the model address is controller-encodable");
    PassiveScanMemoryGraphStorage::pin_static_model(
        storage,
        address,
        reset_config(),
...
fn initialized_graph_contains_the_scanner_link_state_and_receive_chain() {
    ...
    assert!(
        link_state.retains_rx_head(PassiveScanRxHeadProjection::from_bound(bindings[0].header))
    );
    assert_eq!(link_state.crc_init(), LeCrcInit::LE_PRESET);
    assert_eq!(
        link_state.access_address(),
        LeAccessAddress::PRIMARY_ADVERTISING
    );
    assert_eq!(
    ...
    let window = PassiveScanSchedulerWindow::from_controller_ticks(500, 1_500)
    ...
    assert_eq!(event.channel(), PassiveScanPrimaryChannel::Channel37);
    ...
    assert_eq!(
        prepared.selector(),
        crate::RxMemoryListClass::Scanning.selector()
    );
}
...
fn failed_extent_binding_retains_the_exact_static_allocation() {
    ...
    let address =
        PassiveScanMemoryGraphModelAddress::new(BLUETOOTH_CONTROLLER_PHYSICAL_SRAM_HIGH - 4)
            .expect("the aligned address remains controller-encodable");

    let failure = match PassiveScanMemoryGraphStorage::pin_static_model(
        storage,
        address,
        reset_config(),
        PassiveScanSchedulerAllocationConfig::new(0, 0)
    ...
    assert_eq!(
        failure.error(),
        PassiveScanMemoryGraphBindError::ExtentOutsidePhysicalSram
    );
...
fn completed_receive_chain_rejects_a_gap_before_packet_access() {
    ...
    assert_eq!(
        ...
        Err(super::LeRxError::CompletionChainGap)
    );
}

#[test]
fn scheduler_limits_must_fit_every_retained_item() {
    assert!(PassiveScanSchedulerAllocationConfig::new(0, 0).is_some());
    assert!(PassiveScanSchedulerAllocationConfig::new(u16::MAX, u16::MAX).is_none());
}
