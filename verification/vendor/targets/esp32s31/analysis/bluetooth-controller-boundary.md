...
| Controller timer and scheduler | Radio timebase, prepare/abort/doorbell/completion and collision policy | The always-awake latch request/self-clear/read order has a live private keyed worker retained by the powered Controller. The terminal published-owner state exposes an affine Pending borrow; one recheck returns Waiting with the same exact owner or a non-decomposable `TimeObservedAfterEnable` proof bound to that Controller. Drop/cancel converts only the matching request into an explicit sample-discarding orphan drain; mismatch faults the worker. This path neither performs nor proves a sleep/wake transition, and neither the private sample nor an RF-ready instant can escape. Bidirectional wrapping scheduler-epoch projection and nine-word DTM item update are typed, but phase-specific DTM consumption, a proven wake/recheck source, physical counter contract and complete scheduler command/status semantics remain absent | Virtual-time model plus register trace proves one scheduled event, cancellation and late/error handling. |
