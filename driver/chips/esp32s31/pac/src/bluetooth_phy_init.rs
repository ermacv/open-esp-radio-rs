...
#[cfg(test)]
mod tests {
    use super::super::{BluetoothControllerSramAddress, RadioHardware};
    use super::{
    ...
    #[test]
    fn raw_pac_owners_cover_every_register_in_transaction_order() {
        // Pointer inspection validates generated ownership and performs no
        // volatile MMIO. Duplicate entries intentionally encode every fresh
        // read/write edge rather than only unique addresses.
        let cold = RadioHardware::for_validation().into_bluetooth();
        let (task, _interrupts) = cold.separate_interrupt_owner();
        let bluetooth = &task.bluetooth;
        let btmac = &bluetooth.btmac_ble_phy_init;
        let controller = &bluetooth.bluetooth_controller_core;

        let addresses = [
            bluetooth.ble_phy_init_toggle.init_toggle().as_ptr() as usize,
            bluetooth.ble_phy_init_toggle.init_toggle().as_ptr() as usize,
            btmac.init_control_00b4().as_ptr() as usize,
            btmac.init_ones_00b8().as_ptr() as usize,
            btmac.lc_tx_on_delay_config().as_ptr() as usize,
            btmac.lc_tx_on_delay_config().as_ptr() as usize,
            bluetooth.bt_v3_2_baseband.le_tx_on_delay().as_ptr() as usize,
            btmac.init_value_0138().as_ptr() as usize,
            btmac.init_bytes_04a4().as_ptr() as usize,
            btmac.init_bytes_04a8().as_ptr() as usize,
            btmac.init_dynamic_image_04a0().as_ptr() as usize,
            btmac.init_value_04ac().as_ptr() as usize,
            btmac.init_value_045c().as_ptr() as usize,
            btmac.init_bytes_0254().as_ptr() as usize,
            btmac.init_bytes_0254().as_ptr() as usize,
            btmac.init_bytes_0254().as_ptr() as usize,
            btmac.init_bytes_0254().as_ptr() as usize,
            btmac.init_zero_0074().as_ptr() as usize,
            bluetooth.ble_phy_init_phase.init_phase().as_ptr() as usize,
            bluetooth.ble_hw_accelerator.init_config().as_ptr() as usize,
            bluetooth.ble_hw_accelerator.init_sram_region_0().as_ptr() as usize,
            bluetooth.ble_hw_accelerator.init_sram_region_1().as_ptr() as usize,
            bluetooth.ble_hw_resolving_list.base_pointer().as_ptr() as usize,
            btmac.init_control_0400().as_ptr() as usize,
            btmac.init_control_0400().as_ptr() as usize,
            btmac.init_value_0540().as_ptr() as usize,
            btmac.init_bytes_0550().as_ptr() as usize,
            btmac.init_bytes_0550().as_ptr() as usize,
            btmac.init_bytes_0550().as_ptr() as usize,
            btmac.init_bytes_0554().as_ptr() as usize,
            btmac.init_bytes_0554().as_ptr() as usize,
            btmac.init_bytes_0554().as_ptr() as usize,
            btmac.init_bytes_0554().as_ptr() as usize,
            btmac.init_bytes_055c().as_ptr() as usize,
            btmac.init_bytes_055c().as_ptr() as usize,
            btmac.init_bytes_055c().as_ptr() as usize,
            btmac.init_bytes_055c().as_ptr() as usize,
            btmac.init_bytes_0558().as_ptr() as usize,
            btmac.init_bytes_0558().as_ptr() as usize,
            btmac.init_bytes_0558().as_ptr() as usize,
            btmac.init_bytes_0558().as_ptr() as usize,
            btmac.init_high_half_0458().as_ptr() as usize,
            btmac.init_low_5_054c().as_ptr() as usize,
            bluetooth.ble_phy_init_phase.init_phase().as_ptr() as usize,
            btmac.init_branch_control_0470().as_ptr() as usize,
            bluetooth
                .ble_hw_runtime_control
            ...
            bluetooth
                ...
                .as_ptr() as usize,
            btmac.init_control_00b4().as_ptr() as usize,
            btmac.init_control_00c4().as_ptr() as usize,
            controller.phy_init_zero_0244().as_ptr() as usize,
            controller.phy_init_value_01f0().as_ptr() as usize,
            controller.phy_init_value_0248().as_ptr() as usize,
            controller.phy_init_dynamic_image_024c().as_ptr() as usize,
        ];
