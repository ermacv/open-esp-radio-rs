...
fn source_127_notifies_only_a_fresh_durable_work_epoch() {
    let wakers = ModemTimerWakers::<NoopRawMutex>::new();
    let (counter, task_waker) = counting_waker();
    wakers.worker_waker.register(&task_waker);

    assert_eq!(
        wakers.notify_modem_timer_service(ModemLpTimerPublishedInterruptStep::SoftwarePending(
    ...
    assert_eq!(
        wakers.notify_modem_timer_service(ModemLpTimerPublishedInterruptStep::AwaitingSoftware(
    ...
    assert_eq!(
        wakers.notify_modem_timer_service(ModemLpTimerPublishedInterruptStep::Spurious),
        ModemLpTimerPublishedInterruptStep::Spurious
    );
    assert_eq!(
        wakers.notify_modem_timer_service(ModemLpTimerPublishedInterruptStep::Rearmed),
        ModemLpTimerPublishedInterruptStep::Rearmed
    );
