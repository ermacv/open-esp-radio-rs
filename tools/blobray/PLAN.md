...
| 24 | active | Lean verification engine (user decision 2026-09-25). A driver verification run executes only what its claims need: vendor and compiled production execution, comparison under typed contracts, and claim results. Effect contracts and projections are typed values in the scenario code, reviewed through git, and identified by content hash in the index; no knowledge transactions. Evidence is results only: verdicts, case counts, coverage and input/source digests; full traces only on request or for DIFF/INCOMPLETE diagnosis. Blobray's own replay, preservation and storage properties are covered by its synthetic tests, not by driver runs. Every existing claim, negative case and verdict is preserved, and the whole scenario run takes seconds on a warm cache. |
| 22 | pending | Verification coverage and sensitivity (22.1–22.4 done; 22.5 resumes after stage 24, user decision 2026-09-25). Every typed scenario reports the vendor branch coverage of the root closures it exercises (basic blocks reached and both directions of each conditional branch), retained with its evidence and carried into the qualification index. A mutation run applies a finite, reviewed set of single-point changes to the production PHY (constants, write order, bounds, guards) and records which scenarios DIFF; surviving mutants and uncovered branches become explicit follow-up cases or recorded out-of-scope decisions. |
| 14 | pending | Remaining suites required by qualification targets or by production firmware paths, as typed scenarios on the stage-12 mechanisms. Each legacy TOML profile/disposition/baseline suite is triaged: ported with checked expected MATCH/DIFF/INCOMPLETE, or recorded as a dropped claim in qualification (user decision 2026-09-25). Compiled production paths are required; exclusions and claim strength are preserved. Split independent large groups into separately accepted sub-stages before activation. |
...
| 24.3 | needs-replan | Interpreter throughput: direct memory lookup instead of region scans, budget checks at a bounded instruction interval instead of every instruction, and address-indexed model dispatch. A benchmark over the real scenarios reaches at least 100 million guest instructions per second, with unchanged verdicts, events and coverage. Evidence: a direct-mapped decode cache keyed by instruction bytes, last-region lookup, one-lookup instruction fetch and 256-instruction accounting intervals keep every verdict and raise the full warm run from 14.3 s to about 9 s, but throughput stays at about 22 million guest instructions per second. The remaining cost is one dynamic `ExecutionMemory` call per access and per instruction, optional registers and per-access event and device dispatch. Reaching the target needs an executor monomorphized over the session memory, which changes the application/backend boundary: replan after stage 24.4 measures how much wall time interpretation still costs. |
| 24.4 | pending | Independent cold case chains run in parallel with deterministic result order; warm chains stay sequential. |
| 24.5 | pending | Vendor results are cached locally by vendor image, invocation, model and engine identity; a production-only change reruns only production and the comparison. A stale or mismatched cache entry is never used. |
| 24.6 | pending | Evidence index schema 3 carries claim results, contract and projection hashes, coverage and input/source digests, without retained execution identities; the qualification evaluator and documentation follow. The full scenario run on a warm cache is measured and recorded here. |
...
| 22.4 | done | Targeted mutation run: `vendor-scenario mutants --target FILE[:START-END]` mutates executed production PHY lines of the named regions (constants, comparisons, guards, min/max bounds, adjacent register-write order), rebuilds the probe incrementally in per-worker worktrees and runs the scenarios reaching each mutant; results are journaled per commit. A whole-crate campaign costs hours and moved to the backlog (user decision 2026-09-25). A partial whole-crate run killed 94 of 129 mutants; its 26 survivors are 22.5 obligations. |
| 22.5 | active | Triage every untriaged location of the index, starting with functions that have a production counterpart: a follow-up case that reaches it with an expected verdict, or a reviewed decision with its reason. Production-implemented vendor paths (for example the channel-14 MIC and 802.11p configuration) are cases, not exclusions. Each surviving mutant becomes a case that kills it (checked with a targeted mutation run) or a reviewed decision. Done: `analog/temperature.rs` (exhaustive sensor comparison kills the conversion constants; the three reset-DAC constants are killed by a host test; the ten `selected_dac` bounds are reviewed equivalent). Open: `analog/rfpll.rs` 36, 37, 65, 73, 77-80, 304 (both constants), 485; `analog/dcode.rs` 194, 204. The stage closes with an empty untriaged list and no untriaged survivor. |
