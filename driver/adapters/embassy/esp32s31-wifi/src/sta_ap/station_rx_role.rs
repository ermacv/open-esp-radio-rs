//! Connected-station RX role for a common STA+AP WDEV owner.
...
use crate::{
    connected_rx_protocol::{ConnectedRxProtocolSink, Esp32s31ConnectedRxProcessor},
    ethernet_rx::{PackedEthernetWriter, record_at},
    wdev::{WdevNetworkRx, WdevRxProgress},
};

/// Finite station publication failure at the paired WDEV boundary.
...
/// WDEV lends the addressed station endpoint.
...
impl<'storage, O> Esp32s31StaApStationRxSink<'storage, O> {
    ...
    ) -> Result<WdevRxProgress, Esp32s31StaApStationRxError> {
        if self.failed {
...
mod tests {
    ...
    impl WdevNetworkRx for Network {
