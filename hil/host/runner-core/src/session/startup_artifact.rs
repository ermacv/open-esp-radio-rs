...
pub fn load_if_present(path: &Path) -> Result<Option<Vec<u8>>> {
    match fs::read(path) {
...
pub fn chunks(bytes: &[u8]) -> Result<Vec<StartupArtifactChunk>> {
...
impl Assembler {
    ...
    pub fn push(&mut self, chunk: &StartupArtifactChunk) -> Result<Option<Vec<u8>>> {
        chunk
            .validate()
