...
impl StationStatusChannel {
    ...
    fn publish_link(&self, state: StationLinkState) {
...
pub(crate) fn publish_station_connected() {
    STATION_STATUS.publish_link(StationLinkState::Connected);
}

pub(crate) fn publish_station_disconnected(reason: ConnectedDisconnectReason) {
    STATION_STATUS.publish_link(StationLinkState::Disconnected(Some(reason)));
}
