# ESP32-S31 Bluetooth LE Controller

This crate composes the LE Controller and its roles over the
[hardware engine](../../../../hardware/esp32s31/driver/bluetooth/): Direct Test
Mode, legacy and connectable advertising, passive scanning and one peripheral
connection, together with Controller bootstrap, HCI binding, retirement and
PHY maintenance. It contains no `unsafe` code and no MMIO access; register and
interrupt authority stays in the hardware engine's safe primitives.
...
- Over-the-air PDU and role policy belongs to the portable
  ...
  that policy onto the engine's scheduler, DMA and interrupt primitives.
- The module table below is the role API map. The engine's
  [`FEATURES.md`](../../../../hardware/esp32s31/driver/bluetooth/FEATURES.md)
  links current limitations to the generated qualification view; it is not a
  second readiness inventory.
...
Public paths identify their owners: `controller` and
`le::{dtm, advertising, scanning, peripheral}`. Internal event and transition
implementations stay private within those namespaces. Publication,
cancellation, reset and quiescence remain explicit lifecycle terms. Shared
controller code is not owned by one LE role; the interrupt, scheduler and
resource primitives it composes belong to the
[hardware engine](../../../../hardware/esp32s31/driver/bluetooth/).
...
The separate [`memory`](../../../../hardware/esp32s31/driver/bluetooth/memory/) crate retains controller-SRAM codecs.
...
after disconnection. Run them with `cargo test -p oer-esp32s31-bluetooth-controller lifecycle`.
...
[PHY timing contract](../../../../hardware/esp32s31/phy/src/tracking/README.md).
