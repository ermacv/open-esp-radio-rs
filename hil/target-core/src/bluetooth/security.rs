...
//!
//! The firmware owns the chip MIC-fault hooks and the HCI reply exchange; this
//! module owns the key selection, phase and counters they report through.
use bt_hci::{
...
pub fn configure(enabled: bool, failure: Option<BluetoothSecurityFailure>) {
...
/// Host-policy evidence; the firmware adds its MIC-injection observations.
pub fn snapshot() -> BluetoothEncryptionEvidence {
...
pub struct KeyReply {
    ...
    pub injected: Option<BluetoothSecurityFailure>,
}

/// Returns the requested handle and its phase-specific public fixture key, or a negative reply.
pub fn observe(packet: &ControllerToHostPacket<'_>) -> Option<(ConnHandle, KeyReply)> {
    observe_with(packet, || {})
}

/// [`observe`], calling `disconnected` before a disconnection retires the phase.
pub fn observe_with(
    packet: &ControllerToHostPacket<'_>,
    disconnected: impl FnOnce(),
) -> Option<(ConnHandle, KeyReply)> {
    if !enabled() {
        return None;
    }
...
/// Record a fault observed by the firmware's HCI reply exchange.
pub fn record_fault() {
    increment(&FAULTS);
}

/// Record a reply the Controller accepted for its requested handle.
pub fn record_reply(reply: KeyReply) {
