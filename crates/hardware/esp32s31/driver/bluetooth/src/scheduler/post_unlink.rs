...
#[cfg(any(target_arch = "riscv32", test, feature = "test-support"))]
pub enum DtmPostUnlinkArmError {
    Busy,
    IdentityExhausted,
    GenerationExhausted,
}
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
#[cfg(any(target_arch = "riscv32", test, feature = "test-support"))]
pub struct DtmPostUnlinkArmKey {
...
#[cfg(target_arch = "riscv32")]
pub struct DtmPostUnlinkMailbox {
...
#[cfg(target_arch = "riscv32")]
impl Default for DtmPostUnlinkMailbox {
    fn default() -> Self {
        Self::new()
    }
}
...
impl DtmPostUnlinkMailbox {
    ...
    pub const fn wake(&self) -> &DtmPostUnlinkWakeCell {
        &self.wake
    }
...
impl<RoleOwner> BluetoothPostUnlinkAwaiting<RoleOwner> {
    pub const fn new(unlinked: RoleOwner, key: DtmPostUnlinkArmKey) -> Self {
        Self { unlinked, key }
    }
}
...
pub struct SoftwareListRemovalPublishedEvent<RoleOwner> {
    unlinked: RoleOwner,
    published: PrimaryPublishedInterruptStep,
}
...
impl<RoleOwner> SoftwareListRemovalPublishedEvent<RoleOwner> {
    pub fn into_parts(self) -> (RoleOwner, PrimaryPublishedInterruptStep) {
        (self.unlinked, self.published)
    }
}

#[cfg(target_arch = "riscv32")]
pub enum PostUnlinkTake<RoleOwner> {
    Recheck {
        key: DtmPostUnlinkArmKey,
        unlinked: RoleOwner,
    },
...
#[cfg(target_arch = "riscv32")]
pub enum PostUnlinkRearm<RoleOwner> {
