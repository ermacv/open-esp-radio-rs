...
fn serialized_primary_notification_wakes_both_fresh_ordinary_consumers() {
    let wakers = RuntimeNotifications::<NoopRawMutex>::new();
...
fn serialized_primary_notification_does_not_wake_coalesced_consumers() {
    ...
    assert_eq!(
        wakers.notify_ordinary(PrimaryOrdinaryPublication::Scheduler {
            scheduler: SchedulerWakePublication::Coalesced,
            lock_modify: SchedulerLockModifyEventPublication::Coalesced,
        }),
        PrimaryOrdinaryPublication::Scheduler {
            scheduler: SchedulerWakePublication::Coalesced,
            lock_modify: SchedulerLockModifyEventPublication::Coalesced,
...
fn borrowed_scheduler_wait_observes_without_consuming() {
    let wakers = RuntimeNotifications::<NoopRawMutex>::new();
    let wake = SchedulerWakeCell::new();

    assert_eq!(
        wake.publish_from_interrupt(SchedulerWorkerWakeClass::Ordinary),
...
fn cancelled_borrowed_scheduler_wait_preserves_replacement_readiness() {
    ...
    drop(cancelled);
    assert_eq!(
        wake.publish_from_interrupt(SchedulerWorkerWakeClass::Marked),
        SchedulerWakePublication::WakeWorker
    );
...
fn post_unlink_publication_before_wait_is_rechecked_without_loss() {
    ...
    assert_eq!(
        wakers.notify_post_unlink(DtmPostUnlinkMailboxPublication::WakeConsumer),
...
fn post_unlink_wait_wakes_once_while_ready_publications_coalesce() {
    ...
    assert_eq!(
        wakers.notify_post_unlink(DtmPostUnlinkMailboxPublication::AlreadyReady),
