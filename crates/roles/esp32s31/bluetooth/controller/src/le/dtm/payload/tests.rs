...
fn every_hci_pattern_selector_roundtrips_and_bounds_the_domain() {
    ...
    for (selector, pattern) in patterns.into_iter().enumerate() {
        ...
        assert_eq!(
            DtmPayloadPattern::from_hci_selector(selector as u8),
            Ok(pattern)
        );
    }
    assert_eq!(
        DtmPayloadPattern::from_hci_selector(8),
        Err(DtmPayloadPatternError::UnsupportedHciSelector)
    );
}
...
fn repeated_patterns_match_all_complete_vendor_branches() {
    let cases = [
        (DtmPayloadPattern::Repeated11110000, 0x0f),
        (DtmPayloadPattern::Repeated10101010, 0x55),
        (DtmPayloadPattern::RepeatedAllOnes, 0xff),
        (DtmPayloadPattern::RepeatedAllZeros, 0x00),
        (DtmPayloadPattern::Repeated00001111, 0xf0),
        (DtmPayloadPattern::Repeated01010101, 0xaa),
    ];

    for (pattern, expected) in cases {
        let mut storage = [0xa5; 255];
        let prepared = pattern
            .prepare(DtmPayloadLength::from_hci_image(255), &mut storage)
...
fn prbs9_matches_the_complete_cross_revision_table_fingerprint() {
    ...
    let prepared = DtmPayloadPattern::Prbs9
        .prepare(DtmPayloadLength::from_hci_image(255), &mut storage)
...
fn prbs15_matches_the_complete_cross_revision_table_fingerprint() {
    let mut storage = [0; 255];
    let prepared = DtmPayloadPattern::Prbs15
        .prepare(DtmPayloadLength::from_hci_image(255), &mut storage)
...
fn preparation_is_fail_closed_and_returns_the_whole_storage() {
    ...
    assert!(matches!(
        DtmPayloadPattern::RepeatedAllZeros
            .prepare(DtmPayloadLength::from_hci_image(4), &mut short,),
        Err(DtmPayloadPreparationError::StorageTooShort)
    ));
    assert_eq!(short, [0xa5; 3]);

    let mut storage = [0xa5; 8];
    let prepared = DtmPayloadPattern::RepeatedAllZeros
        .prepare(DtmPayloadLength::from_hci_image(3), &mut storage)
        .expect("prefix fits");
    assert_eq!(prepared.pattern(), DtmPayloadPattern::RepeatedAllZeros);
...
fn zero_length_preparation_is_defined_for_every_pattern() {
    for selector in 0..=7 {
        let pattern = DtmPayloadPattern::from_hci_selector(selector)
            .expect("selector belongs to the complete domain");
        let mut storage = [];
        let prepared = pattern
            .prepare(DtmPayloadLength::from_hci_image(0), &mut storage)
