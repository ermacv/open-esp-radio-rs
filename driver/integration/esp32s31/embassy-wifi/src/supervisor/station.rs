...
struct ConnectedDatapathTaskReturn {
    runner: ConnectedDatapathRunner,
    result: Result<DatapathRunnerExit<ConnectedDisconnectReason>, ConnectedDatapathError>,
}
...
impl ConnectedDatapathMailbox {
    ...
    pub(super) fn bind(&self, spawner: Spawner) {
        assert!(
    ...
    fn spawn(&'static self, runner: ConnectedDatapathRunner) {
        self.stop.reset();
    ...
    fn take_return(&self) -> ConnectedDatapathTaskReturn {
        self.returned
            .borrow_mut()
            .take()
...
pub(super) fn initialize_connected_datapath_mailbox(
    #[cfg(feature = "connected-datapath-cycle-telemetry")] poll_observer: Option<
        crate::Esp32s31ConnectedDatapathPollObserver,
    >,
...
async fn wait_connected_datapath_completion(
    mailbox: &'static ConnectedDatapathMailbox,
