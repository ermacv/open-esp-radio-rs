//! Verification of the Rust STA management-state replacement for the
...
use open_radio_vendor_semantics::{
    ...
    OmissionReason, PlatformOperation,
};
...
const VENDOR_VAP: u32 = 0x3ffd_4000;
const VENDOR_NODE: u32 = 0x3ffd_5000;
const VENDOR_CONFIG: u32 = 0x3ffd_6000;
const VENDOR_STA_CONTEXT: u32 = 0x3ffd_7000;
const VENDOR_OSI_TABLE: u32 = 0x3ffe_0000;
...
fn vendor_scenario(
    ...
    use crate::execution_model::{TableInstance, TableInstanceSlot, TableSlotTarget};

    let old_state = if new_state == 3 { 2 } else { 0 };
    let g_ic = image
        .symbol_address("g_ic")
        .ok_or("STA join replay image has no g_ic storage")?;
    let mut scenario = execution::Scenario {
        arguments: vec![VENDOR_VAP, new_state, 0],
        private_stack_fill: Some(0),
        table_instances: vec![TableInstance {
            layout_id: "reviewed-platform-services-v9".to_owned(),
            base_address: VENDOR_OSI_TABLE,
            layout_size: 0x200,
            pointer_cells: Vec::new(),
            pointer_cell_symbols: vec!["g_osi_funcs_p".to_owned()],
            slots: vec![
                TableInstanceSlot {
                    offset: 0x0e0,
                    target: TableSlotTarget::ModeledSymbol("timer_arm".to_owned()),
                },
                TableInstanceSlot {
                    offset: 0x0e4,
                    target: TableSlotTarget::ModeledSymbol("timer_disarm".to_owned()),
                },
                TableInstanceSlot {
                    offset: 0x0e8,
                    target: TableSlotTarget::ModeledSymbol("timer_done".to_owned()),
                },
                TableInstanceSlot {
                    offset: 0x0ec,
                    target: TableSlotTarget::ModeledSymbol("timer_setfn".to_owned()),
                },
                TableInstanceSlot {
                    offset: 0x190,
                    target: TableSlotTarget::ModeledSymbol("coex_status_get".to_owned()),
                },
            ],
        }],
        call_responses: BTreeMap::from([
            (
                "wifi_nvs_get".to_owned(),
                VecDeque::from([execution::ModeledCallResponse::scalar(VENDOR_CONFIG)]),
            ),
            (
                "wifi_log".to_owned(),
                VecDeque::from(vec![
                    execution::ModeledCallResponse::scalar(0);
                    if new_state == 2 { 3 } else { 2 }
                ]),
            ),
            (
                "sta_update_power_off_reset_flag".to_owned(),
                VecDeque::from([execution::ModeledCallResponse::scalar(0)]),
            ),
            (
                "pm_on_coex_schm_status_config".to_owned(),
                VecDeque::from([execution::ModeledCallResponse::scalar(0)]),
            ),
            (
                "ieee80211_send_mgmt".to_owned(),
                VecDeque::from([execution::ModeledCallResponse::scalar(0)]),
            ),
            (
                "coex_status_get".to_owned(),
                VecDeque::from([execution::ModeledCallResponse::scalar(0)]),
            ),
            (
                "timer_disarm".to_owned(),
                VecDeque::from([execution::ModeledCallResponse::scalar(0)]),
            ),
            (
                "timer_done".to_owned(),
    ...
    // Explicit zero-filled reviewed fixtures keep every private vendor byte
    // deterministic without treating its layout as an open-driver type.
    for address in (VENDOR_VAP..VENDOR_OSI_TABLE).step_by(4) {
        crate::write_ram_word(&mut scenario, address, 0);
    }
    crate::write_ram_word(&mut scenario, VENDOR_VAP + 0x10, VENDOR_NODE);
    crate::write_ram_word(&mut scenario, VENDOR_NODE + 0x98, old_state);
    crate::write_ram_word(&mut scenario, VENDOR_NODE + 0xe4, VENDOR_CONFIG + 0x500);
    crate::write_ram_word(&mut scenario, VENDOR_NODE + 0xe8, VENDOR_CONFIG + 0x600);
    crate::write_ram_word(&mut scenario, g_ic + 0x10, VENDOR_STA_CONTEXT);
    Ok(scenario)
}

fn vendor_case_reason(result: &execution::ExecutionResult, new_state: u32) -> Option<String> {
    let management = result
        .ordered_calls
        .iter()
        .find(|call| call.symbol == "ieee80211_send_mgmt");
    let expected_management = if new_state == 2 { (0xb0, 1) } else { (0, 0) };
    if !matches!(management, Some(call) if call.arguments[0] != 0
        && call.arguments[1] == expected_management.0
        && call.arguments[2] == expected_management.1)
    {
        return Some(format!(
            "vendor state {new_state} management call differs: {management:?}"
        ));
    }
    let arm = result
        .ordered_calls
        .iter()
        .find(|call| call.symbol == "timer_arm");
    if !matches!(arm, Some(call) if call.arguments[0] != 0
        && call.arguments[1] == VENDOR_STATE_TIMEOUT_MS
        && call.arguments[2] == 0)
    {
        return Some(format!(
            "vendor state {new_state} timer arm differs: {arm:?}"
        ));
    }
    let management_index = result
        .ordered_calls
        .iter()
        .position(|call| call.symbol == "ieee80211_send_mgmt");
    let arm_index = result
        .ordered_calls
        .iter()
        .position(|call| call.symbol == "timer_arm");
    if !matches!((management_index, arm_index), (Some(send), Some(arm)) if send < arm) {
        return Some("vendor timer was not armed after management publication".to_owned());
    }
    None
}
...
pub fn verify_esp32s31_sta_join_state(
    ...
    for (name, state) in [
        ("vendor-authentication-entry", 2),
        ("vendor-association-entry", 3),
    ] {
        let result = super::execute_case(
            &vendor_replay_image,
            svd,
            SYMBOL,
            vendor_scenario(&vendor_replay_image, state)?,
            name,
            "concrete vendor STA state replay",
        )?;
        let reason = vendor_case_reason(&result, state);
        matched &= reason.is_none();
        canonical.push_str(&format!(
            "scenario {name} state={state} steps={} matched={}\n",
            result.steps,
            reason.is_none()
        ));
    ...
    for case in CASES {
        ...
        reports.push(DriverAdapterCase {
            ...
            reason: (!case_matched).then(|| {
                format!(
                    "expected return {:#010x} without effects and padding dependence, got {:#010x}",
                    case.expected_return, first.return_value
                )
            }),
        });
    }
    Ok(DriverAdapterVerification::from_trust(
        crate::driver_adapter_trust(RUST_ADAPTER_ID)
