...
impl<S: ConnectedRxSink> ConnectedRxSink for AlwaysReadyConnectedRxSink<S> {
    fn publish(
        &mut self,
        event: open_esp_radio_esp32s31_wifi_lmac::connected_rx::ConnectedRxEvent<'_>,
    ) {
    ...
    S: ConnectedRxProtocolSink,
{
    ...
    async fn dispatch_reordered_frame(
        ...
        let result = if ordinary {
            if let Some(scratch) = self.reorder_scratch.as_deref_mut() {
                ...
                let segment = open_esp_radio_esp32s31_wifi_lmac::rx::RxSegment {
    ...
    async fn dispatch_segment(
        &mut self,
        segment: open_esp_radio_esp32s31_wifi_lmac::rx::RxSegment<'_>,
    ) -> ConnectedRxDispatch {
...
mod tests {
    ...
    use open_esp_radio_esp32s31_wifi_lmac::{connected_rx::ConnectedRxConfig, rx::RxIngressConfig};
