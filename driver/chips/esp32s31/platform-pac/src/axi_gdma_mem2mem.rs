...
const fn required_descriptors(bytes: usize, burst: BurstSize) -> usize {
...
mod hardware {
    ...
    use esp_hal::peripherals::{AXI_GDMA, DMA_AXI_CH0, HP_SYS_CLKRST};

    use super::{
        BurstSize, DESCRIPTOR_OWNER_DMA, descriptor_flags, descriptor_payload_bytes,
        required_descriptors,
    };
    use crate::{PsramCacheWritebackError, writeback_psram_for_dma_read};

    const INTERNAL_SRAM_START: usize = 0x2f00_0000;
    ...
    const TX_ALL: u32 = TX_DONE
        | TX_EOF
        | TX_DESCRIPTOR_ERROR
        | TX_TOTAL_EOF
        | TX_FIFO_OVERFLOW
        | TX_FIFO_UNDERFLOW;
    const TX_ERRORS: u32 = TX_DESCRIPTOR_ERROR | TX_FIFO_OVERFLOW | TX_FIFO_UNDERFLOW;
    ...
    const RX_ALL: u32 = RX_DONE
        ...
        | RX_FIFO_OVERFLOW
        | RX_FIFO_UNDERFLOW;
    const RX_ERRORS: u32 = RX_ERROR_EOF
        | RX_DESCRIPTOR_ERROR
        | RX_DESCRIPTOR_EMPTY
        | RX_FIFO_OVERFLOW
        | RX_FIFO_UNDERFLOW;
    ...
    extern "C" fn channel0_interrupt() {
    ...
    impl<'d> AxiGdmaMem2Mem<'d> {
        ...
        pub fn prepare<'a>(
            ...
            if source_is_psram {
                writeback_psram_for_dma_read(source)
        ...
        fn configure_channel(&mut self, rx_head: u32, tx_head: u32, burst: BurstSize) {
        ...
        fn stop_and_reset_channel(&mut self) {
            let regs = AXI_GDMA::regs();
            let input = regs.in_ch(CHANNEL);
            let output = regs.out_ch(CHANNEL);
    ...
    fn terminal_status() -> Option<(u32, u32)> {
        let regs = AXI_GDMA::regs();
    ...
    fn enable_and_configure_group() {
        ...
        regs.extr_mem_end_addr().write(|writer| unsafe {
            writer.access_extr_mem_end_addr().bits(PSRAM_END as u32 - 1)
        });
    }
