...
pub(crate) fn load_evidence_baseline(path: &Path) -> Result<EvidenceSet> {
    ...
    let document: BaselineDocument = toml_edit::de::from_str(&text).map_err(|error| {
        BlobrayError::manifest_source("evidence baseline TOML", path, &text, &error, error.span())
    })?;
    finish_evidence_baseline(path, document).map_err(|error| {
        BlobrayError::manifest_document("evidence baseline TOML", path, &text, error)
    })
}
...
pub(crate) fn load_evidence_report(path: &Path) -> Result<EvidenceSet> {
    ...
    parse_evidence_report(path, &text)
        .map_err(|error| BlobrayError::manifest_document("verification report", path, &text, error))
}
...
mod tests {
    ...
    fn malformed_baseline_retains_its_physical_source_line() {
        ...
        let path = std::env::temp_dir().join(format!(
            "blobray-evidence-diagnostic-{}.toml",
            std::process::id()
        ));
