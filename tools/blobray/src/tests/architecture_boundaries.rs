...
fn facade_and_register_model_errors_cannot_implicitly_absorb_strings() {
    ...
    for forbidden in [
        "impl From<String> for BlobrayError",
        "impl From<&str> for BlobrayError",
...
#[test]
fn blobray_source_contains_no_private_paths_or_embedded_digests() {
...
fn facade_does_not_compile_backend_implementation_dependencies_directly() {
    ...
    for forbidden in [
        ...
        assert!(
            ...
            "blobray facade directly owns backend/model dependency {forbidden}"
        );
    }
}
