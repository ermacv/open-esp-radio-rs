//! Executor-clock policy for connected-station beacon loss.
//!
//! This owner performs no MMIO and does not put the modem to sleep. RX supplies
//! typed beacon observations, while `WifiRunner` supplies monotonic time and
//! owns the resulting disconnect edge. A future power-save policy can consume
//! the same TIM observation without importing vendor PM contexts or RTOS
//! timers.
