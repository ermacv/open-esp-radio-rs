...
pub fn run(duration: u16, minimum: u16, output: &Path, context: &Context<'_>) -> Result<()> {
    let adapter = context
        .lab
        .bluetooth_adapter
    ...
    let result = context.with_capture(output, |capture| {
        ...
        hil_core::durable::atomic_json(&output.join("calibration-traffic.json"), &serde_json::json!({
