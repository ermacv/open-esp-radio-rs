...
| Uplink and downlink OFDMA | Trigger admission/iteration, narrow-RU classification and queue publication implemented; scheduler oracle only | Allocation-free Rust decodes Trigger Common Info, both RU/SS User Info forms, Basic/BFRP/MU-BAR/NFRP dependent fields and TRS/UPH HE-control information. Complete blob bodies anchor fail-closed User Info iteration, exact type-dependent strides, the AID12/RU padding sentinel and raw-allocation classification for RU26/52/106/242; wider and gap encodings remain explicitly unselectable. The MAC can publish a bounded Trigger-eligible A-MPDU queue, MPDU-length chain and BSR state. Typed RU rate lookup reproduces all eight complete blob/ROM ordinary/DCM tables across MCS and GI, so OFDMA arithmetic no longer substitutes the full RU242 rate. Sustained HE-SU traffic against FRITZ and Android Wi-Fi 6 APs produced no Trigger: FRITZ advertised triggered SU feedback/CQI but did not schedule it, while the Android AP advertised neither and reports no HE MU beamformer. RU admission against bandwidth/region, real HE-TB TX qualification and OFDMA RX remain incomplete. |
