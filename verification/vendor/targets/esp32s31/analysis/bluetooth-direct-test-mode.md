...
The per-buffer drain, append-entry and ordinary re-arm rules are now exact; the
swap ownership, hardware/software unlink and reclamation remain prerequisites
for a live path.
...
transaction. A fresh interrupt-owned scheduler observation is affine: BUSY
returns `Pending` before the task owner can read either command register. Only
the idle token permits command-zero and then conditional command-one reads.
...
recreating the vendor intrusive list. A later call services and durably
publishes one fresh primary epoch after that unlink, then immediately consumes
its exact scheduler event in the task-side gate. Pending retains the already-
unlinked graph and requires another external event; Ready advances the same
identity without returning descriptor ownership. Selector-6 recovery still
blocks this path fail-closed, and evidence does not yet prove that every later
command-status transition produces a new wake.
...
1. **Close scheduler recycle ownership.** The empty-list manager epoch,
   merge-selected identity, descriptor visibility, hardware head, RUN,
   completion classification, fresh empty-head observation, abstract source
   unlink and finite post-unlink return gate are now one non-reorderable typed
   chain. Add a proven external wake/recheck source for Pending, then consume
   `SoftwareListRemovalReady` through the source-owned recycle transition.
   Return TX and RX-non-success graphs without cloning the vendor intrusive
   manager; keep RX success blocked until returned-header and swap-reserve
   ownership are affine. Only the final recycle may release the exact timeline
   reservation and return CPU ownership.
