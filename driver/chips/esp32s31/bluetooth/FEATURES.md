...
| Legacy advertising | ABSENT | A hardware-independent `open-esp-radio-bluetooth-ll` foundation validates and encodes exact `ADV_NONCONN_IND` PDUs, retains public/random TxAdd semantics and advances an affine non-connectable lifecycle over a non-empty channel 37/38/39 map only after an exact generation/event/channel completion. Rejected admission and stale completion retain their owners; disable joins an in-flight TX before returning to configured state; each successor event requires a fresh typed delay without embedding an RNG. No ESP32-S31 descriptor owner, scheduler/list role, HCI advertising commands, response-capable mode or on-air evidence exists yet, so production advertising remains absent. |
