...
| Controller timer and scheduler | Radio timebase, prepare/abort/doorbell/completion and collision policy | The always-awake latch request/self-clear/read order has a live owner retained by the powered Controller, generation-scoped abandonment and one-observation recheck. Bidirectional wrapping scheduler-epoch projection and nine-word DTM item update are typed. A proven wake/recheck source, physical counter contract and complete scheduler command/status semantics remain absent | Virtual-time model plus register trace proves one scheduled event, cancellation and late/error handling. |
