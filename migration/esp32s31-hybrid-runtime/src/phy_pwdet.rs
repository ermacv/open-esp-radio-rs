...
impl PhyPwdetPbusBinding {
    ...
    pub const fn action(&self) -> PhyPwdetPbusBindingAction {
        match self.phase {
            PhyPwdetPbusPhase::Start => PhyPwdetPbusBindingAction::Start(self.transaction),
            PhyPwdetPbusPhase::SampleCompletion => {
                PhyPwdetPbusBindingAction::SampleCompletion(self.transaction)
            }
            PhyPwdetPbusPhase::Complete => PhyPwdetPbusBindingAction::Complete(self.transaction),
        }
    }

    pub fn started(&mut self) -> Result<(), PhyPwdetPbusBindingError> {
        match self.phase {
            PhyPwdetPbusPhase::Start => {
                self.phase = PhyPwdetPbusPhase::SampleCompletion;
                Ok(())
            }
            PhyPwdetPbusPhase::SampleCompletion => Err(PhyPwdetPbusBindingError::WrongEdge),
            PhyPwdetPbusPhase::Complete => Err(PhyPwdetPbusBindingError::AlreadyComplete),
        }
    }
    ...
    pub fn observe_completed(
        ...
        match (self.phase, completed) {
            (PhyPwdetPbusPhase::SampleCompletion, false) => {
                Ok(PhyPwdetPbusObservation::StillPending)
            }
            (PhyPwdetPbusPhase::SampleCompletion, true) => {
                self.phase = PhyPwdetPbusPhase::Complete;
                Ok(PhyPwdetPbusObservation::Completed)
            }
            (PhyPwdetPbusPhase::Start, _) => Err(PhyPwdetPbusBindingError::WrongEdge),
            (PhyPwdetPbusPhase::Complete, _) => Err(PhyPwdetPbusBindingError::AlreadyComplete),
        }
    }

    #[cfg(target_arch = "riscv32")]
    pub unsafe fn start_target(&mut self) -> Result<(), PhyPwdetPbusBindingError> {
        if self.phase != PhyPwdetPbusPhase::Start {
            return Err(if self.phase == PhyPwdetPbusPhase::Complete {
        ...
        crate::radio_hal::try_start_phy_pbus_force_test(self.transaction).map_err(
            |crate::radio_hal::PhyPbusError::Busy| PhyPwdetPbusBindingError::BusyAtStart,
        )?;
        self.started()
    }
    ...
    pub unsafe fn sample_target_once(
        ...
        if self.phase != PhyPwdetPbusPhase::SampleCompletion {
            return Err(if self.phase == PhyPwdetPbusPhase::Complete {
                PhyPwdetPbusBindingError::AlreadyComplete
            } else {
                PhyPwdetPbusBindingError::WrongEdge
    ...
    pub fn into_completion(self) -> Result<PhyPwdetCompletion, PhyPwdetPbusBindingError> {
        if self.phase != PhyPwdetPbusPhase::Complete {
            return Err(PhyPwdetPbusBindingError::Incomplete);
        }
        Ok(PhyPwdetCompletion::PbusCompleted(self.transaction))
    }
