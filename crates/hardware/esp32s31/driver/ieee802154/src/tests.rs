...
use oer_esp32s31_ieee802154_mac::{MacMeasurementSample, MacTransmitAccess};
...
struct FakeExecutor {
    ...
    policy_error: Option<MacOperationPolicyError>,
    calls: usize,
}
...
fn operation(executor: FakeExecutor) -> MacOperation<FakeExecutor> {
    MacOperation::from_commands(MacCommandCapability::from_model_executor(executor))
}
...
fn acknowledged_irq_decodes_measurement_for_the_active_phase() {
    let energy = AcknowledgedMacEventBatch::from_interrupt(
        interrupt(Ok(Ieee802154Event::EdDone.mask()), None, None, -42, true),
        MacActivePhase::EnergyDetection {
            duration: oer_esp32s31_ieee802154_mac::MacEnergyDetectionDuration::from_hardware_units(
                8,
            ),
        },
    )
...
fn transmit_with_ack_executes_the_complete_plan_in_exact_order() {
    ...
    let active = operation(FakeExecutor::new()).start(actor).unwrap();
    assert_eq!(
...
fn energy_detection_keeps_duration_before_the_final_command() {
    let actor = MacReady::new().request_energy_detection(
        oer_esp32s31_ieee802154_mac::MacEnergyDetectionDuration::from_hardware_units(37),
    );
...
fn partial_start_failure_quarantines_the_affine_actor() {
    ...
    let failure = operation(fake).start(actor).unwrap_err();
    assert_eq!(failure.phase(), MacActivePhase::ClearChannelAssessment);
    assert!(matches!(
        failure.error(),
        MacOperationStartError::Executor {
            step_index: 2,
...
fn incompatible_policy_is_quarantined_before_any_executor_step() {
    let mut fake = FakeExecutor::new();
    let policy_error = MacOperationPolicyError::AcknowledgementReceptionDisabled;
    ...
    assert!(matches!(
        failure.error(),
        MacOperationStartError::IncompatiblePolicy(error) if *error == policy_error
    ));
...
fn sampled_acknowledged_batches_advance_without_reexecuting_start() {
    ...
    let pending = match active.process_batch(sampled).unwrap() {
        MacOperationBatchOutcome::Pending(active) => active,
        MacOperationBatchOutcome::Completed(_) => panic!("TX_DONE must await ACK"),
    };
    assert_eq!(
        pending.phase(),
    ...
    let completed = match pending.process_batch(sampled).unwrap() {
        MacOperationBatchOutcome::Completed(completed) => completed,
        MacOperationBatchOutcome::Pending(_) => panic!("ACK_RX_DONE must be terminal"),
    };
    assert_eq!(completed.completion(), MacCompletion::TransmitAcknowledged);
    let resolved = completed
        .resolve(oer_esp32s31_ieee802154_mac::MacDeferredNext::IdlePolicy)
        .unwrap();
    let (operation, _ready, reclaimed, _, _) = resolved.into_parts();
    assert_eq!(
        operation.hardware.executor.log,
        [
            LogEntry::Quiesce,
            LogEntry::RefreshPolicy,
            LogEntry::PublishTx(DMA_LOW),
    ...
    assert_eq!(
        rx.outcome(),
        oer_esp32s31_ieee802154_mac::MacResolvedAcknowledgementOutcome::Received
    );
    rx.recycle().unwrap();
}
...
fn accepted_tx_done_returns_the_exact_transmit_buffer() {
    ...
    let completed = match active.process_batch(sampled).unwrap() {
        MacOperationBatchOutcome::Completed(completed) => completed,
        MacOperationBatchOutcome::Pending(_) => panic!("TX_DONE must be terminal"),
    };
...
fn accepted_rx_done_delivers_validated_frame_owner_and_allows_rearm() {
    ...
    let mut image = [0_u8; oer_esp32s31_ieee802154_dma::FRAME_BUFFER_SIZE];
    image[0] = 5;
    ...
    let completed = match active.process_batch(sampled).unwrap() {
        MacOperationBatchOutcome::Completed(completed) => completed,
        MacOperationBatchOutcome::Pending(_) => panic!("RX_DONE must be terminal"),
    };
    ...
    assert_eq!(
        received.outcome(),
        oer_esp32s31_ieee802154_mac::MacResolvedRxOutcome::Received
    );
...
fn accepted_rx_abort_reclaims_without_exposing_partial_frame() {
    ...
    let completed = match active.process_batch(sampled).unwrap() {
        MacOperationBatchOutcome::Completed(completed) => completed,
        MacOperationBatchOutcome::Pending(_) => panic!("SFD timeout must be terminal"),
    };
    let resolved = completed.resolve(MacDeferredNext::ReceiveWhenIdle).unwrap();
    ...
    assert_eq!(
        ...
        MacCompletion::ReceiveAborted(
            oer_esp32s31_ieee802154_irq::Ieee802154RxAbortReason::SfdTimeout
        )
    );
    assert_eq!(
        reclaimed.outcome(),
        oer_esp32s31_ieee802154_mac::MacResolvedRxOutcome::Aborted(
            oer_esp32s31_ieee802154_irq::Ieee802154RxAbortReason::SfdTimeout
        )
    );
...
fn acknowledgement_timeout_returns_non_frame_rx_ownership() {
    ...
    let completed = match pending.process_batch(timeout).unwrap() {
        MacOperationBatchOutcome::Completed(completed) => completed,
        MacOperationBatchOutcome::Pending(_) => panic!("timer zero must terminate ACK wait"),
    };
    ...
    assert_eq!(
        ...
        oer_esp32s31_ieee802154_mac::MacResolvedAcknowledgementOutcome::NotReceived
    );
...
fn no_dma_completion_returns_a_reusable_runtime_and_ready_state() {
    ...
    {
        MacOperationBatchOutcome::Completed(completed) => completed,
        MacOperationBatchOutcome::Pending(_) => panic!("ED_DONE must complete CCA"),
    };

    let resolved = completed.resolve(MacDeferredNext::IdlePolicy);
    let (operation, ready, _no_dma, completion, next) = resolved.into_parts();
    assert_eq!(
        completion,
