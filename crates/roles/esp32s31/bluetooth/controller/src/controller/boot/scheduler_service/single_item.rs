...
use {
    ...
    oer_esp32s31_bluetooth::{
        interrupt::{
            PrimaryControllerFault, PrimaryNoSchedulerWork, PrimarySchedulerEvent,
            SchedulerWakeBatch,
        },
        scheduler::{
            ...
            core::{
                ...
                SingleItemSchedulerCompletionObservedDrainStep, SingleItemSchedulerCompletionStep,
...
pub enum SingleItemPostUnlinkArmStep<Role: SingleItemSchedulerRole> {
...
pub enum SingleItemSoftwareListRemovalPublishedStep<Role: SingleItemSchedulerRole> {
...
pub enum SingleItemSchedulerCompletionFaultOwner<Role: SingleItemSchedulerRole> {
...
fn completion_fault<Role: SingleItemSchedulerRole>(
    ...
    SingleItemCompletionFault::new(cause, owner)
}
    ...
    S: SchedulerRunInterruptStorage,
{
        ...
        match self.observe_single_item_completion(running, wake) {
            SingleItemSchedulerCompletionStep::NoFinishedList(running) => {
                Ok(SingleItemRunningProgress::Running(
                    oer_esp32s31_bluetooth::scheduler::SchedulerFinishedListDrainState::Drained(
                        running,
                    ),
                ))
            }
    ...
    fn continue_running_drain(
        &mut self,
        pending: oer_esp32s31_bluetooth::scheduler::SchedulerFinishedListDrainPending<
