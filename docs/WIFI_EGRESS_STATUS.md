...
- after rebasing onto `origin/main` at `71522d1b`, workspace tests and network
  adapter boundary checks pass, but the complete source-only audit stops at
  strict Clippy diagnostics in `open-esp-radio-esp32s31-bluetooth` (unused
  code/imports and large error/enum variants). The same diagnostics reproduce
  in a separate clean checkout of `71522d1b`; this is an inherited main issue,
  not a Wi-Fi rebase conflict. The audit has not been weakened or marked passed.
