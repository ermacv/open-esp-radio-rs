...
| Platform/HAL lifecycle | Clocks, reset, controller HAL, scheduler prefix, PHY, BTBB, BLE PHY, interrupt matrix, entropy/crypto and coexistence leases | Clock/reset consumes into the complete 50-operation controller HAL state; only that state reaches the scheduler-table prefix, preserving the recovered `r_btdm_task_init` hardware order. The selected time scale, independently bounded modem-timer queue and source-owned scheduler timeline, and one bounded HCI bootstrap resource epoch remain affine with that powered scheduler owner. Scheduler/HCI splitting returns one powered task endpoint that joins the sole mutable timeline and workers to the exact task-side HAL owner; its finite lock/modify step cannot export register access. Safe consuming transitions then retain the complete source-127 low-power owner, run concrete common-PHY registration, execute the finite BTBB transaction, publish the BLE-PHY register transaction with its complete static allocation graph, prepare Controller output, start the runtime timer once and atomically publish both disjoint owners in process-wide ESP-HAL ISR slots. The lease composes all three finite interrupt dispositions; source 124 durably publishes both scheduler handoffs into the matching powered Controller runtime, and source 127 transfers software-pending ownership through a durable task-readiness cell and the Controller queue/event/rearm path without allowing MMIO re-entry. Executor notification and the live route epoch remain closed | One owner can power, route all three IRQs, run and return cold without owner duplication. |
...
| `r_btdm_task_enable` | split, still incomplete | The hardware-only 50-operation BTDM HAL-init body, exact baseline fault masks and diagnostic capture, controller-output strobes, generated runtime-timer start command, typed three-route ESP-HAL primitives, affine ISR scheduler MMIO, dynamic scheduler classifier, RTOS-free coalesced wake cells and pure controller-time latch/epoch phases are recovered as separate contracts. One affine Controller owner now composes the fixed scheduler timeline, low-power hardware, common PHY, BTBB, BLE-PHY register initialization, Controller-output preparation, the runtime-timer start, atomic stable publication of both ISR owners, all three finite interrupt dispositions, durable paired source-124 publication and durable source-127 task-readiness plus queue/event/rearm work while retaining the complete static BLE allocation graph. Feature-specific NRT policy, live-route and executor-notification composition, the controller-time wake/recheck source, typed selector-4/6 actions, scheduler-item hardware publication and live scheduler activation remain unresolved. |
