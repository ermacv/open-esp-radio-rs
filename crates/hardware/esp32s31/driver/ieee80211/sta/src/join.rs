...
impl StaJoinObserver for () {}

/// Exact primitive edge which failed inside a concrete join adapter.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum StaJoinPortError<R, T> {
    Receive(R),
    AssociationProfile(StaAssociationProfileError),
    Transmit(T),
}
