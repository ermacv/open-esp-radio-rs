...
| Legacy advertising | PRE-ADMISSION | A hardware-independent `open-esp-radio-bluetooth-ll` foundation validates and encodes exact `ADV_NONCONN_IND` PDUs, retains public/random TxAdd semantics and advances an affine non-connectable lifecycle over a non-empty channel 37/38/39 map only after an exact generation/event/channel completion. The S31 chip boundary installs that PDU in the same role-neutral typed controller TX allocation proven for DTM, while retaining exact portable identity and primary-channel frequency projection; cancellation returns the unchanged enabled event. It deliberately cannot mint `InFlight` or publish `HEAD`/`RUN` until the advertising-specific buffer-header binding, link-state/scheduler graph, timing/list role and completion ticket are reviewed. HCI advertising commands, response-capable mode and on-air evidence also remain absent. |
