//! Architecture and calling-convention selection at the validator boundary.
    ...
    pub(crate) fn load(path: &Path) -> Result<Self> {
        ...
        for (index, raw_line) in input.lines().enumerate() {
            ...
            match directive {
                ...
                "harness" => set_token(&mut harness, value, directive, line_number)?,
...
mod tests {
    ...
    fn write_spec(name: &str, contents: &str) -> std::path::PathBuf {
        let path = std::env::temp_dir().join(format!(
            "open-radio-validator-target-{}-{name}.spec",
            std::process::id(),
        ));
