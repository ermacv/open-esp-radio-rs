...
fn plan_indexed_mmio_action(
    ...
    registers: &[crate::IndexedMmioRegister],
    guard: Option<&crate::IndexedMmioGuard>,
    value: &Option<SymbolicValue>,
    policy: &EffectPolicy,
    bindings: &PacBindingIndex,
    used_rules: &mut BTreeSet<EffectSelector>,
) -> Result<DriverAction> {
    let guard = guard.ok_or("production DriverPlan requires a bounded indexed-MMIO selector")?;
...
fn plan_action(
    ...
    let (selector, action) = match event {
        ...
        ResolvedReferenceEvent::IndexedMmio {
            ...
            return plan_indexed_mmio_action(
                *access,
                *width,
                address,
                registers,
