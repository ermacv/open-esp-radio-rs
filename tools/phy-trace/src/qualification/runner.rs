//! ESP32-S31 semantic qualification command implementations.

use std::{collections::BTreeSet, path::Path};

use crate::*;

pub(crate) fn qualify_esp32s31_channel(
    svd: &MmioRegisterMap,
    ...
    for (case_index, (name, channel_or_frequency, cbw)) in cases.into_iter().enumerate() {
        let mut scenario = qualification::vendor_channel_scenario(
        ...
        let footprint = qualification::vendor_channel_state_footprint(&result, phy_param)?;
        let vendor_events = qualification::normalize_vendor_channel(
            &image,
            &result,
            phy_param,
            channel_or_frequency,
        ...
        let (rust_events, next_state) =
            qualification::rust_channel_events_with_state(rust_state, channel_or_frequency, cbw)?;
...
pub(crate) fn qualify_esp32s31_rf_init(
    svd: &MmioRegisterMap,
    vendor_artifact: &Path,
    vendor_companion: &Path,
    print_oracles: bool,
    ...
    for (case_index, name) in cases.into_iter().enumerate() {
        let mut scenario = qualification::vendor_rf_init_scenario(phy_param, phy_functions_pointer);
        scenario.reset_policy = if case_index == 0 {
            execution::ResetPolicy::ColdBoot
        } else {
            execution::ResetPolicy::Continue
        ...
        let footprint = qualification::vendor_rf_init_state_footprint(&result, phy_param)?;
        let vendor_events = qualification::normalize_vendor_rf_init(&image, &result, phy_param)?;
        let (rust_events, next_state) = qualification::rust_rf_init_events(rust_state)?;
        rust_state = next_state;
        if vendor_events != rust_events {
            let divergence = vendor_events
                .iter()
...
pub(crate) fn qualify_esp32s31_bluetooth_txdc(
    svd: &MmioRegisterMap,
    vendor_artifact: &Path,
    vendor_companion: &Path,
    print_oracles: bool,
    ...
    let phy_functions_pointer = image
        .symbol_address("g_phyFuns")
        .ok_or("vendor artifact has no g_phyFuns symbol")?;
    let scenario = qualification::vendor_bluetooth_txdc_scenario(phy_param, phy_functions_pointer);
    ...
    let vendor_events = qualification::normalize_vendor_bluetooth_txdc(&image, &result, phy_param)?;
    let (rust_events, _) = qualification::rust_bluetooth_txdc_events(
...
pub(crate) fn qualify_esp32s31_bluetooth_tx_power(
    svd: &MmioRegisterMap,
    vendor_artifact: &Path,
    vendor_companion: &Path,
    print_oracles: bool,
    ...
    let phy_functions_pointer = image
        ...
        .ok_or("vendor artifact has no g_phyFuns symbol")?;
    let scenario =
        qualification::vendor_bluetooth_tx_power_scenario(phy_param, phy_functions_pointer);
    ...
    let footprint = qualification::vendor_bluetooth_tx_power_state_footprint(&result, phy_param)?;
    let vendor_events =
        qualification::normalize_vendor_bluetooth_tx_power(&image, &result, phy_param)?;
    let (rust_events, _) = qualification::rust_bluetooth_tx_power_events(
        open_esp_radio_esp32s31_phy::phy_cold::PhyColdState::new(),
    )?;
    let matched = vendor_events == rust_events;
    if !matched {
...
pub(crate) fn qualify_esp32s31_bluetooth_txdc_pwdet(
    svd: &MmioRegisterMap,
    vendor_artifact: &Path,
    vendor_companion: &Path,
    print_oracles: bool,
    ...
    let phy_functions_pointer = image
        ...
        .ok_or("vendor artifact has no g_phyFuns symbol")?;
    let scenario =
        qualification::vendor_bluetooth_txdc_pwdet_scenario(phy_param, phy_functions_pointer);
    ...
    let footprint = qualification::vendor_bluetooth_txdc_pwdet_state_footprint(&result, phy_param)?;
    let vendor_events =
        qualification::normalize_vendor_bluetooth_txdc_pwdet(&image, &result, phy_param)?;
    let (rust_events, _) = qualification::rust_bluetooth_txdc_pwdet_events(
        open_esp_radio_esp32s31_phy::phy_cold::PhyColdState::new(),
    )?;
    let matched = vendor_events == rust_events;
    if !matched {
