    ...
    ControllerIdleCommandTask<'runtime, S, SCHEDULER_CAPACITY>
{
    ...
    > {
        ...
        if let Err(error) = task.runtime.runtime.retirement_ready() {
            return Err((
                ControllerTaskRetirementError::Runtime(error),
                Self::from_parts(task, ready),
            ));
        }
        if let Err(error) = task.runtime.task.controller_time_retirement_ready() {
            return Err((
...
pub struct ControllerTaskHciRetired<'runtime, S, const SC: usize> {
    ...
    _hardware: crate::resources::TaskResources,
    _phy: crate::ble_phy::BlePhyRetainedOwners,
