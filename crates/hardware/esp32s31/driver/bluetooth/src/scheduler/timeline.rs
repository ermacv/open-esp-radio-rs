...
#[derive(Clone, Copy)]
struct SchedulerTimelineSlot {
    window: Option<SchedulerRawWindow>,
    generation: u32,
}

impl SchedulerTimelineSlot {
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum SchedulerReservationError {
...
#[derive(Debug)]
pub enum SchedulerSequenceReady {}

/// Why a retained reservation cannot authorize sequence-time formation.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum SchedulerSequenceAuthorizationError {
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum SchedulerReservationReleaseError {
...
pub struct SchedulerWindowReservation<State> {
    slot: usize,
    generation: u32,
    window: SchedulerRawWindow,
    timing_policy: SchedulerTimingPolicy,
    _state: PhantomData<State>,
}

impl<State> SchedulerWindowReservation<State> {
    /// Return the phase-bound raw-tick window retained by this reservation.
    pub const fn window(&self) -> SchedulerRawWindow {
        self.window
    }
...
impl<State> core::fmt::Debug for SchedulerWindowReservation<State> {
    fn fmt(&self, formatter: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        formatter
            .debug_struct("SchedulerWindowReservation")
...
pub struct SchedulerReservationReleaseFailure<State> {
    error: SchedulerReservationReleaseError,
...
impl<State> SchedulerReservationReleaseFailure<State> {
    /// Exact reason the timeline rejected this reservation.
    pub const fn error(&self) -> SchedulerReservationReleaseError {
        self.error
    }
...
impl<State> core::fmt::Debug for SchedulerReservationReleaseFailure<State> {
    fn fmt(&self, formatter: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        formatter
            .debug_struct("SchedulerReservationReleaseFailure")
...
impl<State> core::fmt::Debug for SchedulerSequenceAuthorizationFailure<State> {
    fn fmt(&self, formatter: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        formatter
            .debug_struct("SchedulerSequenceAuthorizationFailure")
            .field("error", &self.error)
            .finish_non_exhaustive()
    }
}
...
impl SchedulerWindowReservation<SchedulerInitialAdmissionResolved> {
    ...
    pub(crate) fn authorize_sequence(
        ...
        SchedulerSequenceAuthorizationFailure<SchedulerInitialAdmissionResolved>,
    > {
...
impl SchedulerWindowReservation<SchedulerRecurringReserved> {
    ...
    pub(crate) fn authorize_sequence(
        ...
        SchedulerWindowReservation<SchedulerSequenceReady>,
        SchedulerSequenceAuthorizationFailure<SchedulerRecurringReserved>,
    > {
        authorize_sequence(self, sample)
    }
}
...
fn authorize_sequence<State>(
    ...
    SchedulerWindowReservation<SchedulerSequenceReady>,
    SchedulerSequenceAuthorizationFailure<State>,
> {
    if !reservation
        .timing_policy
        .initial_deadline_is_open(sample, reservation.window.start)
    {
        return Err(SchedulerSequenceAuthorizationFailure {
            reservation,
            error: SchedulerSequenceAuthorizationError::DeadlineExpired,
        });
    }

    Ok(SchedulerWindowReservation {
...
impl<const CAPACITY: usize> SchedulerTimeline<CAPACITY> {
    ...
    pub(crate) fn reserve_initial_window(
        ...
        SchedulerWindowReservation<SchedulerInitialAdmissionResolved>,
        SchedulerReservationError,
    > {
        if !timing_policy.initial_deadline_is_open(admission_sample, raw_start) {
            return Err(SchedulerReservationError::InitialDeadlineExpired);
        }
    ...
    > {
        ...
        let Some(candidate) = SchedulerRawWindow::new(raw_start, raw_end) else {
        ...
        if self.slots.iter().any(|slot| {
            ...
            return Err(SchedulerReservationError::RecurringOverlapUnsupported);
        }
    ...
    pub(crate) fn reserve_recurring_window(
        ...
        raw_end: u32,
        timing_policy: SchedulerTimingPolicy,
    ) -> Result<SchedulerWindowReservation<SchedulerRecurringReserved>, SchedulerReservationError>
    {
    ...
    fn reserve_initial_raw_window(
        ...
        SchedulerReservationError,
    > {
        let Some(mut candidate) = SchedulerRawWindow::new(raw_start, raw_end) else {
            return Err(SchedulerReservationError::WindowOutsideForwardHalfRange);
        };
    ...
    fn reserve_window<State>(
        &mut self,
        candidate: SchedulerRawWindow,
        timing_policy: SchedulerTimingPolicy,
    ) -> Result<SchedulerWindowReservation<State>, SchedulerReservationError> {
        ...
        for (slot_index, slot) in self.slots.iter_mut().enumerate() {
            ...
            return Ok(SchedulerWindowReservation {
                slot: slot_index,
                generation,
                window: candidate,
                timing_policy,
    ...
    pub(crate) fn prepare_release<State>(
        ...
        SchedulerReservationReleasePrepared<'_, State>,
        SchedulerReservationReleaseFailure<State>,
    > {
        let Some(slot) = self.slots.get_mut(reservation.slot) else {
            return Err(SchedulerReservationReleaseFailure {
                error: SchedulerReservationReleaseError::SlotOutOfRange,
                reservation,
            });
        };
        if slot.generation != reservation.generation || slot.window != Some(reservation.window) {
            return Err(SchedulerReservationReleaseFailure {
                error: SchedulerReservationReleaseError::IdentityMismatch,
                reservation,
            });
        }
        Ok(SchedulerReservationReleasePrepared {
...
impl<const CAPACITY: usize> Default for SchedulerTimeline<CAPACITY> {
    fn default() -> Self {
        Self::new()
    }
}
