use crate::{NonScanningRxMemoryModelAddress, NonScanningRxMemoryStorage};
...
fn owner(base: u32) -> LegacyConnectableAdvertisingMemoryGraphCpuOwned {
    let storage = std::boxed::Box::leak(std::boxed::Box::new(
        LegacyConnectableAdvertisingMemoryGraphStorage::new(),
    ));
    let base = LegacyConnectableAdvertisingMemoryGraphModelAddress::new(base)
...
fn running(
    ...
    let LegacyConnectableAdvertisingMemoryGraphRxPublished {
        prepared,
        rx_publication,
    } = published;
    (
...
fn completion_requires_list_zero_and_a_non_sentinel_item_status() {
    ...
    let running = match running.observe_completion(finished_list(0)) {
        LegacyConnectableAdvertisingMemoryGraphCompletionObservation::StillInFlight(running) => {
            running
        }
...
fn removal_list_mismatch_retains_the_completed_graph_and_proof() {
    ...
    assert_eq!(
        failure.error(),
        LegacyConnectableAdvertisingMemoryGraphRecycleError::HardwareListMismatch
    );
...
fn removal_head_mismatch_retains_the_completed_graph_and_proof() {
    ...
    assert_eq!(
        failure.error(),
        LegacyConnectableAdvertisingMemoryGraphRecycleError::SchedulerItemMismatch
    );
    let (completed, removal) = failure.into_parts();
    assert_eq!(completed.scheduler_item_address(), scheduler_item);
...
fn selector_mismatch_retains_publication_and_all_cpu_rollback_authority() {
    ...
    let publication = owner
        ...
        .prepare_empty_list_link()
        .prepare_publication();
    let mismatched = RxMemoryListPublished::from_parts_for_validation(
    ...
    assert_eq!(
        mismatch.error(),
        LegacyConnectableAdvertisingMemoryGraphPublicationError::SelectorMismatch
    );
...
fn receive_head_mismatch_retains_both_affine_owners() {
    ...
    assert_eq!(
        mismatch.error(),
        LegacyConnectableAdvertisingMemoryGraphPublicationError::HeadMismatch
    );
    let (publication, mismatched) = mismatch.into_parts();
    assert_eq!(publication.identity(), graph_identity);
    assert_eq!(publication.receive_identity(), pool_identity);
...
fn packet_fit_is_proved_without_interpreting_protocol_headers() {
    let oversized = [0; 40];
    assert_eq!(
        LegacyConnectableAdvIndPacketInput::try_from_encoded_extent(&oversized, 38),
        Err(
    ...
    assert_eq!(
        LegacyConnectableScanResponsePacketInput::try_from_encoded_extent(&[0xaa, 0xbb, 0xcc], 2,),
        Err(
