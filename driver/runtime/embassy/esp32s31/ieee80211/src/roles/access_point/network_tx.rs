...
impl BufferedUnicastRelease {
    fn can_publish(&self, engine: &Esp32s31ApEngine<'_>) -> bool {
        engine
    ...
    B: StableDmaBacking,
{
    ...
    pub fn unresolved_airtime_work(
        ...
        AccessPointAirtimePeer,
        open_esp_radio_wifi_softmac::MacTxWork,
    )> {
        self.airtime
            .as_ref()
    ...
    N: SoftwareTxFrame,
{
            ...
            + open_esp_radio_esp32s31_wifi_mac::tx_ampdu::HtAmpduHardware,
    {
        ...
        if let Err(error) = aggregate.publish_standby(ordinary, hardware) {
            return Err(Esp32s31AccessPointDatapathError::Aggregate(error));
        }
        let _batch = self
            .prepared_standby
            .take()
