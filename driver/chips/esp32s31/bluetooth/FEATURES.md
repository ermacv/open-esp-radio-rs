...
| Legacy advertising | ABSENT | A hardware-independent `open-esp-radio-bluetooth-ll` foundation now validates and encodes exact `ADV_NONCONN_IND` PDUs, retains public/random TxAdd semantics and advances an affine non-connectable event over a non-empty channel 37/38/39 map only after successful backend transmission. Its interval and fresh per-event delay are typed without embedding an RNG. No ESP32-S31 descriptor owner, scheduler/list role, HCI advertising commands, response-capable mode or on-air evidence exists yet, so production advertising remains absent. |
