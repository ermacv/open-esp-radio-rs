//! Qualification of the register-owned prefix of `hal_mac_txq_enable`.
//!
...
use open_radio_vendor_validator_semantic::{
    DraftReferenceEvent, DriverAdapterQualification, EffectDisposition, EffectPolicy,
...
fn validate_domain(
    ...
    registers: &[open_radio_vendor_validator_semantic::IndexedMmioRegister],
    guard: Option<&open_radio_vendor_validator_semantic::IndexedMmioGuard>,
) -> Result<()> {
...
#[allow(
    ...
    reason = "qualification binds caller-owned vendor and Rust artifacts plus the closed policy"
)]
pub fn qualify_esp32s31_hal_mac_txq_enable_register_slice(
    svd: &MmioRegisterMap,
    ...
    let vendor_inventory = vendor_inventory
        .ok_or("hal_mac_txq_enable qualification requires the caller-owned raw libpp inventory")?;
    if print_oracles {
