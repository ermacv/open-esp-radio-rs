//! Strict projection of schema-v30 linked-IR JSON into register-review evidence.
...
pub(super) fn parse_report(path: &Path) -> Result<Vec<ReviewIrRegister>> {
    ...
    if integer(root, "schema_version", "linked-IR report")? != 30 {
        return Err(format!(
            "register review requires linked-IR schema 30 in {}",
            path.display()
        )
        .into());
    }
