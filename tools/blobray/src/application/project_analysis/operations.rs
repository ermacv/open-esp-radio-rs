...
use super::{
    ...
    cache::{ProjectAnalysisCache, ProjectAnalysisCachePlan},
};
...
impl ResolvedProjectAnalysisOperations<'_> {
    ...
    fn register_catalog_inputs(&self) -> Result<Vec<std::path::PathBuf>> {
        let mut paths = self.session.svd_paths.clone();
        ...
        if let Some(registers) = self.session.project.registers.as_ref()
            && registers.model.is_file()
            && RegisterModel::is_model_file(&registers.model)?
        {
...
mod cache_domain_tests {
    use super::{ensure_unique_replay_outputs, linked_ir_stage_cacheable, stage_configuration};
    use crate::{
