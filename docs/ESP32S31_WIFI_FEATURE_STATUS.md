...
| Uplink and downlink OFDMA | parser implemented; scheduler oracle only | Allocation-free Rust now decodes Trigger Common Info, both RU/SS User Info forms, Basic/BFRP/MU-BAR/NFRP dependent fields and TRS/UPH HE-control information from instruction-exact blob evidence. Trigger-frame admission/iteration, RU scheduling, HE-TB TX and OFDMA RX are not yet complete. |
