...
use super::*;
use crate::verification::effect_contract::{
...
use crate::{MmioRegisterMap, pinned_vendor_digest};
...
#[derive(Clone, Debug)]
pub(crate) struct IqEstimatorQualification {
...
pub(crate) fn qualify_esp32s31_iq_est_enable(
    svd: &MmioRegisterMap,
    vendor_artifact: &Path,
    vendor_companion: Option<&Path>,
    rust_artifact: &Path,
    ...
    policy: &EffectPolicy,
    print_oracles: bool,
) -> Result<IqEstimatorQualification> {
    let vendor_digest = pinned_vendor_digest(vendor_artifact)?;
    ...
    Ok(IqEstimatorQualification { matched, canonical })
}
