...
pub(super) fn run(
    ...
    while let Some(argument) = arguments.next() {
        ...
        match argument.as_str() {
            // Transitional aliases for existing caller-owned run specs.
            "--rom-artifact"
            | "--rom-companion"
            | "--rom-prefix"
            | "--archive-artifact"
            ...
            | "--archive-companion"
            | "--archive-prefix" => {
                let (source, field) = argument
                    .trim_start_matches("--")
                    .split_once('-')
                    .expect("legacy source option has a field");
                let value = take_value(&mut arguments, &argument)?;
                let input = source_mut(&mut source_inputs, source)?;
                match field {
                    "artifact" => set_path(&mut input.artifact, value, &argument)?,
                    "inventory" => set_path(&mut input.inventory, value, &argument)?,
                    "companion" => set_path(&mut input.companion, value, &argument)?,
                    "prefix" => set_string(&mut input.prefix, value, &argument)?,
                    _ => unreachable!(),
                }
            }
