...
| Typed HCI Controller | ABSENT | The in-process transport exposes disjoint Host, raw Controller and bootstrap endpoints through `bt-hci::ExternalController`. A finite portable classifier returns owned semantic bootstrap commands without mutating the bootstrap epoch, owned semantic DTM commands, owned malformed-command responses or the unchanged unsupported packet for outer routing. Session policy can therefore retain Reset until live DTM hardware quiesces. No ESP32-S31 session runner yet turns those commands into hardware sessions, events or ACL semantics. |
