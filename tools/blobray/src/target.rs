//! Architecture and calling-convention selection at the Blobray boundary.
...
pub(crate) enum TargetError {
    ...
    #[diagnostic(code(blobray::target::read))]
    Read {
        path: PathBuf,
        #[source]
        source: io::Error,
    ...
    #[error("{message}")]
    #[diagnostic(code(blobray::target::invalid))]
    Invalid {
        message: String,
        #[source_code]
    ...
    #[diagnostic(
        code(blobray::target::abi_mismatch),
    ...
    #[diagnostic(code(blobray::target::backend_unavailable))]
    ...
    #[diagnostic(code(blobray::target::harness_unavailable))]
...
mod tests {
    ...
    fn write_spec(name: &str, contents: &str) -> std::path::PathBuf {
        let path = std::env::temp_dir().join(format!(
            "open-radio-blobray-target-{}-{name}.toml",
            std::process::id(),
        ));
