...
| Single-user/multi-user beamformee | NDPA admission and report-rate policy implemented; feedback generation missing | The complete blob link-metric policy now produces a typed report profile and programs both hardware leaves in exact order: three BPSK/QPSK/16-QAM selectors followed by all four ordinary/ER-SU ACK-rate bytes. Allocation-free HE NDPA parsing reproduces the complete blob's Dialog Token, four-byte STA Info iteration and local 11-bit AID membership check. Capability parsing and average-SNR observation are also owned. The complete test-only feedback selector is mapped in SVD, but its numeric mode meanings and production caller remain unproven. The tested Android 16 SoftAP reports HE SU beamformer/beamformee support but no HE MU beamformer; it is a candidate SU sounding peer only. NDP qualification, feedback-buffer ownership, compressed report generation and HIL are missing. |
