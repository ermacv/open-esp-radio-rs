...
//! words through the global `phy_param` pointer. Current-vendor
//! `phy_wakeup_init` consumes those saved words through `phy_set_pbus_reg`.
//!
...
//! completion. Rust does not retain the six opaque register images: their
//! twelve packed boundaries are deterministic products of the same group
//! sizes and are regenerated by [`PhyPbusBoundaryRestoreTransition`] during
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub struct PhyPbusBoundaryRestoreOutcome;

#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum PhyPbusBoundaryRestoreAction {
    Restore(PhyPbusMemoryGroupBoundary),
    Complete(PhyPbusBoundaryRestoreOutcome),
}

#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum PhyPbusBoundaryRestoreCompletion {
    Restored(PhyPbusMemoryGroupBoundary),
}

#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum PhyPbusBoundaryRestoreError {
    WrongCompletion,
    AlreadyComplete,
}

#[derive(Clone, Copy, Debug, Eq, PartialEq)]
enum PhyPbusBoundaryRestoreStep {
    Restore { group: u8, header_accumulator: u32 },
    Complete,
}

/// Regenerate the twelve PBUS group boundaries consumed by retained wake.
///
/// The transition retains semantic first/last entry indices. Packing two
/// groups into each of the six physical words remains inside the PAC.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub struct PhyPbusBoundaryRestoreTransition {
    step: PhyPbusBoundaryRestoreStep,
}

impl PhyPbusBoundaryRestoreTransition {
    pub const fn new() -> Self {
        Self {
            step: PhyPbusBoundaryRestoreStep::Restore {
                group: 0,
    ...
    pub const fn action(self) -> PhyPbusBoundaryRestoreAction {
        match self.step {
            PhyPbusBoundaryRestoreStep::Restore {
                group,
                header_accumulator,
            } => PhyPbusBoundaryRestoreAction::Restore(group_boundary(group, header_accumulator)),
            PhyPbusBoundaryRestoreStep::Complete => {
    ...
    pub fn advance(
        &mut self,
        completion: PhyPbusBoundaryRestoreCompletion,
    ) -> Result<(), PhyPbusBoundaryRestoreError> {
        self.step = match (self.step, completion) {
            (
                ...
                PhyPbusBoundaryRestoreCompletion::Restored(completed),
            ) if completed == group_boundary(group, header_accumulator) => {
            ...
            (PhyPbusBoundaryRestoreStep::Complete, _) => {
                return Err(PhyPbusBoundaryRestoreError::AlreadyComplete);
            }
            _ => return Err(PhyPbusBoundaryRestoreError::WrongCompletion),
        };
        Ok(())
    }
}

impl Default for PhyPbusBoundaryRestoreTransition {
    fn default() -> Self {
        Self::new()
    }
}
...
pub struct PhyPbusBoundaryRestoreMmioBinding {
    boundary: PhyPbusMemoryGroupBoundary,
}

impl PhyPbusBoundaryRestoreMmioBinding {
    pub const fn new(
        action: PhyPbusBoundaryRestoreAction,
    ) -> Result<Self, PhyPbusMemoryBindingError> {
        match action {
            PhyPbusBoundaryRestoreAction::Restore(boundary) => Ok(Self { boundary }),
            PhyPbusBoundaryRestoreAction::Complete(_) => {
    ...
    ) -> Result<PhyPbusBoundaryRestoreCompletion, PhyPbusMemoryBindingError> {
        oer_esp32s31_hal::phy::memory::configure_pbus_memory_group_boundary(
            registers,
            oer_esp32s31_hal::phy::memory::PbusMemoryGroupBoundary {
                group: self.boundary.group(),
                first_entry: self.boundary.first_entry(),
                last_entry: self.boundary.last_entry(),
            },
        )
        .map_err(|_| PhyPbusMemoryBindingError::InvalidGroup)?;
        Ok(PhyPbusBoundaryRestoreCompletion::Restored(self.boundary))
    }
}
