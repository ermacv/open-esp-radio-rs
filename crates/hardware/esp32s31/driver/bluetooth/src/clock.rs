...
pub struct ClockError {
    ...
    pub checkpoint: ClockCheckpoint,
}
...
pub struct ClockedResources<P> {
    registers: Option<ColdOwner>,
    platform: Option<P>,
    cleanup: fn(&mut ColdOwner, &mut P),
    cleanup_armed: bool,
}
...
impl<P> Drop for ClockedResources<P> {
    fn drop(&mut self) {
...
trait SharedClockControl {
    ...
    fn modem_syscon_clock_state(&self) -> ModemClockState;
...
impl SharedClockControl for ColdOwner {
    ...
    fn prepare_shared_modem_clock_map(&mut self) {
        ColdOwner::prepare_shared_modem_clock_map(self);
    }

    fn retain_coexistence_clock(&mut self) {
        ColdOwner::retain_coexistence_clock(self);
    }

    fn release_coexistence_clock(&mut self) {
        ColdOwner::release_coexistence_clock(self);
    }

    fn retain_main_xtal_low_power_clock(&mut self) {
        ColdOwner::retain_main_xtal_bluetooth_low_power_clock(self);
    }

    fn release_bluetooth_low_power_timer(&mut self) {
        ColdOwner::release_bluetooth_low_power_timer(self);
    }
    ...
    fn bluetooth_shared_clock_observation(
        ...
        ColdOwner::bluetooth_shared_clock_observation(self)
    }
}
...
pub struct ClockEnableFailure<P> {
    stopped: BluetoothStopped<P>,
    error: ClockError,
}

impl<P> ClockEnableFailure<P> {
    /// Return the semantic reason for the failed transaction.
    pub const fn error(&self) -> ClockError {
        self.error
    }
...
impl<P> BluetoothStopped<P> {
    ...
    /// owners are returned.
    pub fn enable_clocks(self) -> Result<ClockedResources<P>, ClockEnableFailure<P>> {
        let (mut registers, platform) = self.into_parts();
        if let Err(error) = enable_owned(&mut registers) {
            return Err(ClockEnableFailure {
...
fn validate_state(state: ClockState) -> Result<(), ClockError> {
    let failed = if !state.controller_clocks_enabled {
        Some(ClockCheckpoint::ControllerClocks)
    } else if !state.apb_clocks_enabled {
        Some(ClockCheckpoint::ApbClocks)
    } else if !state.controller_resets_released {
        Some(ClockCheckpoint::ControllerReset)
    } else if !state.exclusive_main_xtal_selected {
        Some(ClockCheckpoint::LowPowerClockSource)
    } else if !state.low_power_divider_configured {
        Some(ClockCheckpoint::LowPowerClockDivider)
    } else if !state.low_power_timer_enabled {
        Some(ClockCheckpoint::LowPowerTimerClock)
    } else {
        None
    };

    match failed {
        Some(checkpoint) => Err(ClockError { checkpoint }),
        None => Ok(()),
    }
}

fn disable_owned(resources: &mut impl SharedClockControl) {
