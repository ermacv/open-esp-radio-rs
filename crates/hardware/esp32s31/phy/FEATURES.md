...
| Current-archive measured RFPLL correction | IMPLEMENTED | [Measured correction](src/tracking/rfpll/README.md) owns the thermal gate, bounded search, signed frequency-memory update and typed frequency-control restoration. The outer RFPLL action and read-only inspection use this child; its completion commits the reference only after restoration. Registered policy and the opt-in observation service select it ahead of other due work and recheck sample freshness after admission. Compiled comparisons cover child effects and the RFPLL-enabled parent. A bounded HIL scenario covers the first nonzero heating correction and same-epoch traffic restoration; the opposite thermal direction, repeated cycles and radiated RF quality remain unqualified. Exclusive caller admission does not qualify joint coexistence grants. |
...
| Connected observation-driven service | IMPLEMENTED / OPT-IN | Explicit configuration waits on a deadline or coalesced observation event. Selected work requires exclusive Wi-Fi admission. Joint-radio grants, live-TX compensation and thermally safe deferral limits remain unqualified. |
...
| Periodic parameter/calibration tracking | PARTIAL: opt-in connected-station observation service selects the current Wi-Fi/RFPLL graph at role boundaries; no default service or complete thermal/RF qualification | PARTIAL: initial tracking exists; periodic Controller maintenance remains incomplete | PARTIAL: target tracking entry exists; RF/runtime service remains incomplete |
