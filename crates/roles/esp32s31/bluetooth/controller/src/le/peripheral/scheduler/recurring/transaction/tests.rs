...
use {
    crate::le::peripheral::connection::{
    ...
    oer_esp32s31_bluetooth::{
        ControllerSchedulerEpoch, ControllerTimeSample, scheduler::SchedulerSoftwareConfig,
    },
};
...
fn epoch(micros_anchor: u32) -> ControllerSchedulerEpoch {
    ControllerSchedulerEpoch::new(
        ...
        micros_anchor,
        oer_esp32s31_bluetooth::controller_hal::reviewed_standalone_time_scale(),
    )
}
...
fn phase(packet_start_micros: u32) -> PeripheralConnectionRecurringPhase {
    PeripheralConnectionRecurringPhase::from_nominal_anchor(
        oer_esp32s31_bluetooth::SchedulerInstant::from_image(packet_start_micros),
    )
...
fn six_unanswered_events_preserve_the_window_and_end_establishment() {
    for algorithm_two in [false, true] {
        ...
        let mut current_phase = PeripheralConnectionRecurringPhase::from_nominal_anchor(
            oer_esp32s31_bluetooth::SchedulerInstant::from_image(u32::MAX - 20_000),
        );
...
fn establishment_cannot_skip_a_receive_window() {
    ...
    let original_phase = PeripheralConnectionRecurringPhase::from_nominal_anchor(
        oer_esp32s31_bluetooth::SchedulerInstant::from_image(10_000),
    );
