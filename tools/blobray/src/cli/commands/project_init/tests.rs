...
fn creates_a_valid_project_and_refuses_to_overwrite_it() {
    let parent = std::env::temp_dir().join(format!("blobray-project-init-{}", std::process::id()));
    if parent.exists() {
    ...
    let sparse_review =
        open_radio_vendor_review::ReviewPack::load(&directory.join("reviewed/project-facts.toml"))
            .unwrap();
    assert_eq!(sparse_review.schema, 2);
    assert!(sparse_review.assertions.is_empty());
