...
| Scanning | ONE WINDOW CLOSED END TO END | The restricted passive LE 1M graph owns two RX nodes, one link state, one scheduler context and three scheduler items in pinned Controller SRAM. Production cold start claims a fourth, scanner-specific static arena and lends its sole graph to the task service; DTM and advertising memory cannot alias it. Portable LL owns validated interval/window parameters, enabled/in-flight state, primary-channel rotation, legacy PDU parsing and a bounded exact duplicate filter. The production first-window runner acquires a fresh scheduler current plus ordered always-awake BLE-PHY observation, projects the complete preparation/receive interval through the retained Controller epoch, performs admission and sequence-deadline checks, and reaches selector-one RX/command/head/interrupt/`RUN` publication without exposing an integer register or SRAM image. The active runner now drives fenced finished-list capture, exact head retirement, serialized post-unlink removal, RX extraction, timeline release and production-graph restore to a CPU-owned result. It rejects stale producer/epoch sentinels and non-contiguous completion, then parses each bounded PDU plus signed RSSI through the portable LL boundary; only successful completion rotates the primary channel. Pre-publication cancellation returns the graph and reservation without leaking either owner. Interval-preserving recurrence and standard `bt-hci` scan commands/events remain missing, so this is not yet an operational scanner. |
