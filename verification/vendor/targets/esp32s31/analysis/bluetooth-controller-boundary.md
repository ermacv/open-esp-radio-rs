...
| Platform/HAL lifecycle | Clocks, reset, controller HAL, scheduler prefix, PHY, BTBB, interrupt matrix, entropy/crypto and coexistence leases | Clock/reset consumes into the complete 50-operation controller HAL state; only that state reaches the scheduler-table prefix, preserving the recovered `r_btdm_task_init` hardware order. The selected time scale and one bounded HCI bootstrap resource epoch remain affine with that powered scheduler owner. Safe consuming transitions then retain the complete source-127 low-power owner, run concrete common-PHY registration and execute the finite BTBB transaction without exposing a disconnected production state. The later runtime-timer start command remains disconnected until the following BLE-engine and controller-output states are composed. Typed three-source IRQ primitives exist but the live epoch remains closed | One owner can power, route all three IRQs, run and return cold without owner duplication. |
