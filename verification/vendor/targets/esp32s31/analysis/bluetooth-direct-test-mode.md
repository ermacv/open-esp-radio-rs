...
1. **Close scheduler completion ownership.** The empty-list manager epoch,
   ...
   one non-reorderable typed chain, and the memory layer can consume an affine
   list-zero observation into one volatile status classification. Compose that
   primitive with the exact retained Controller running item, keeping the
   completion-observed graph hardware-owned until unlink. The current-list
