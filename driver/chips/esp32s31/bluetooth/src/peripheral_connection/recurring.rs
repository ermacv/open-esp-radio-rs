...
#![allow(
    dead_code,
    reason = "the following connection recurrence owner consumes this pure timing slice"
)]
...
impl BluetoothPeripheralConnectionRecurringPhase {
    pub(crate) const fn from_normalized_packet_start(
        packet_start: &BluetoothPeripheralConnectionPacketStartTiming,
    ) -> Self {
        let packet_start = packet_start.scheduler_instant();
        Self {
            nominal_anchor: packet_start,
            window_widening_reference: packet_start,
        }
    }
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub(crate) enum BluetoothPeripheralConnectionRecurringTimingError {
...
mod tests {
    ...
    fn phase(packet_start_micros: u32) -> BluetoothPeripheralConnectionRecurringPhase {
        BluetoothPeripheralConnectionRecurringPhase::from_normalized_packet_start(
            &BluetoothPeripheralConnectionPacketStartTiming::from_scheduler_micros(
                packet_start_micros,
            ),
        )
    }
    ...
    #[test]
    fn retrying_from_the_original_phase_reproduces_the_same_candidate() {
        let original = phase(u32::MAX - 10_000);
        let first = original
            .plan(
                request(24, 4),
                LePeripheralConnectionEventDelta::new(7).unwrap(),
                epoch(u32::MAX - 11_000),
                BluetoothSchedulerSoftwareConfig::reviewed_standalone(),
                software_policy(),
            )
            .unwrap();
        ...
        assert_eq!(retry.proposed_anchor(), first.proposed_anchor());
        assert_eq!(retry.window(), first.window());
        assert_eq!(retry.event_span(), first.event_span());
        assert_eq!(retry.receive_wait(), first.receive_wait());
        assert_eq!(
            retry.window_widening_micros(),
            first.window_widening_micros()
        );
    }
