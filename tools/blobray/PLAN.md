...
| 26 | done | Vendor library update to the latest upstream archives (user decision 2026-09-26). Every pin lives in `verification/vendor/projects/esp32s31/artifacts.toml`: `esp-phy-lib` 20f1db05 (libphy, librftest, libbtbb, libbttestmode), `esp32-wifi-lib` af55a0ca, `esp-coex-lib` c758e7b5, `esp32s31-bt-lib` 10c50778 and the official `esp-rom-elfs` 20260528 ROM (content-identical to the former private ELF); `cargo xtask vendor-fetch` downloads and verifies them into `target/vendor/`, and the scenarios default to those paths. Production follows every changed behaviour: 30-sample capacitor searches with direction boundaries in tracking and direct programming (`phy_rfpll_cap_init_cal_track`, `phy_rfpll_cap_init_cal_new`), the `phy_force_txrx_off_new` nesting count as a static `PhyForceTxRxDepth` (channel, common and TX tracking envelopes, TX power tracking, Bluetooth gain child; no force in `phy_close_rf`), the one-step RX-DC policy without reference delta or unit fallback, `phy_rxdc_fine_cal` replacing the RXBB fine delta (`phy_param` shrinks to 492 bytes), RF calibration version 102 and a 508-byte RF-data record (calibration snapshot schema 7). Every suite matches the new archives; the index lists are as before except two new `libpp` functions joining the stage-14.1 backlog. |
