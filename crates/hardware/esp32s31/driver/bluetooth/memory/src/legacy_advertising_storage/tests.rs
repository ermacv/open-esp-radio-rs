...
use super::{LegacyAdvertisingMemoryGraphModelAddress, LegacyAdvertisingMemoryGraphStorage};

fn owner() -> super::LegacyAdvertisingMemoryGraphCpuOwned {
...
fn first_event() -> super::LegacyAdvertisingMemoryGraphEventPrepared {
    event(
        crate::LegacyAdvertisingPrimaryChannelPlan::new(true, false, false)
            .expect("one channel is non-empty"),
    )
}

fn event(
    channels: crate::LegacyAdvertisingPrimaryChannelPlan,
) -> super::LegacyAdvertisingMemoryGraphEventPrepared {
    owner()
...
fn reset_rejects_a_non_advertising_packet_without_losing_the_prepared_graph() {
    ...
    assert_eq!(
        failure.error(),
        super::LegacyAdvertisingPduError::UnsupportedPduType
    );
...
fn three_channel_event_waits_for_and_reports_every_hardware_item() {
    let prepared = event(
        crate::LegacyAdvertisingPrimaryChannelPlan::new(true, true, true)
    ...
    let super::LegacyAdvertisingMemoryGraphCompletionObservation::StillInFlight(running) =
    ...
    let super::LegacyAdvertisingMemoryGraphCompletionObservation::CompletionObserved(completed) =
        running.observe_completion(observed)
    else {
