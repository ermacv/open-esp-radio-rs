...
pub const fn calculate_rfpll_sdm(
    ...
    let encoded = ((((scaled as u64) << shift) / first_divisor / divisor) as u32)
...
impl RfpllFrequencyTransition {
    ...
    pub const fn action(self) -> RfpllFrequencyAction {
        match self.step {
            ...
            RfpllFrequencyStep::CapWriteLow(continuation) => {
                RfpllFrequencyAction::WriteByte {
    ...
    fn finish_cap_phase(&mut self, search: CapSearchState) {
        if search.phase == CapSearchPhase::Down {
            self.step =
                RfpllFrequencyStep::CapWriteLow(CapWriteContinuation::Search(
                    CapSearchState {
            ...
            self.step =
                RfpllFrequencyStep::CapWriteLow(CapWriteContinuation::Final {
    ...
    pub fn advance(
        ...
        self.step = match (self.step, completion) {
            ...
            (RfpllFrequencyStep::SdmWrite(index), _)
                if Self::matches_write(action, completion) =>
            {
                if index == 5 {
            ...
            (
                ...
                CapWriteContinuation::Search(search) => {
                    RfpllFrequencyStep::CapStatusRead(search)
                }
...
mod tests {
    use super::{
        calculate_rfpll_sdm, RfpllFrequencyAction, RfpllFrequencyCompletion,
        RfpllFrequencyFailure, RfpllFrequencyRequest, RfpllFrequencyTransition,
    };
