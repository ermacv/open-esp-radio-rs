...
pub enum DtmPayloadPatternError {
...
impl DtmPayloadPattern {
    ...
    pub const fn from_hci_selector(selector: u8) -> Result<Self, DtmPayloadPatternError> {
        match selector {
            0 => Ok(Self::Prbs9),
            ...
            _ => Err(DtmPayloadPatternError::UnsupportedHciSelector),
        }
    }
    ...
    ) -> Result<DtmPreparedPayload<'storage>, DtmPayloadPreparationError> {
        let payload_length = length.as_usize();
        if storage.len() < payload_length {
            return Err(DtmPayloadPreparationError::StorageTooShort);
        }

        let payload = &mut storage[..payload_length];
        self.fill_reviewed(payload);

        Ok(DtmPreparedPayload {
            pattern: self,
            length,
            storage,
        })
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub struct DtmPayloadLength(u8);

impl DtmPayloadLength {
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum DtmPayloadPreparationError {
...
#[derive(Debug)]
pub struct DtmPreparedPayload<'storage> {
    pattern: DtmPayloadPattern,
    length: DtmPayloadLength,
    storage: &'storage mut [u8],
}

impl<'storage> DtmPreparedPayload<'storage> {
