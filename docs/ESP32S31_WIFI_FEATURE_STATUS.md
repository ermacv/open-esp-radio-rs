...
| Uplink and downlink OFDMA | Trigger admission/iteration and queue publication implemented; scheduler oracle only | Allocation-free Rust decodes Trigger Common Info, both RU/SS User Info forms, Basic/BFRP/MU-BAR/NFRP dependent fields and TRS/UPH HE-control information. The complete blob test anchors fail-closed User Info iteration, exact type-dependent strides and the AID12/RU padding sentinel. The MAC can publish a bounded Trigger-eligible A-MPDU queue, MPDU-length chain and BSR state. Typed RU26/52/106/242 rate lookup now reproduces all eight complete blob/ROM ordinary/DCM tables across MCS and GI, so OFDMA arithmetic no longer substitutes the full RU242 rate. Sustained HE-SU traffic against FRITZ and Android Wi-Fi 6 APs produced no Trigger: FRITZ advertised triggered SU feedback/CQI but did not schedule it, while the Android AP advertised neither and reports no HE MU beamformer. Raw allocation-to-RU scheduling, real HE-TB TX qualification and OFDMA RX remain incomplete. |
