...
use super::{
    ClockCheckpoint, ClockedResources, ColdOwner, ModemClockState, SharedClockControl,
    disable_owned, enable_owned,
};
...
fn record_cleanup(_: &mut ColdOwner, platform: &mut FakePlatform) {
    platform.operations.borrow_mut().push(Operation::Cleanup);
}

fn validation_clocked(operations: Rc<RefCell<Vec<Operation>>>) -> ClockedResources<FakePlatform> {
    ClockedResources {
        registers: Some(ColdOwner::from_radio_hardware(
