//! Portable contract between IEEE 802.11 protocol code and a MAC service.
//!
...
//! [`MacOperationOwner::Software`] may be implemented by generic HMAC policy,
//! a chip-specific LMAC state machine, or their explicit composition.  It
//! means only that a caller must not assume the operation is an offload.

use crate::wmm::WmmAccessCategory;
...
/// this copyable value. A concrete LMAC adapter combines this policy with an
...
/// Observable state of a MAC transmit queue at the HMAC/LMAC boundary.
///
...
/// Normalized terminal status returned from LMAC to portable MAC policy.
///
...
pub struct MacAmpduTxStatus<Rate> {
    ...
    /// Terminal status of a detached one-MPDU retry, when the LMAC policy used
