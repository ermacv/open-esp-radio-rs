...
| Wi-Fi channel retune | PARTIAL | [Channel transition](src/channel.rs) and target HAL entry points own the supported Wi-Fi channel domain, temperature-dependent work and bounded frequency-ready observations; MAC restart has a composed variant. TX gain uses the current S31 libphy coefficient profile with an independent additive Wi-Fi adjustment. Concrete channel comparison covers ordered effects and channel/temperature state; enclosing RXCAL/TXCAL and RF qualification remain separate. This is not a generic BT/IEEE channel API. |
...
| D-code calibration | IMPLEMENTED | [D-code transition](src/analog/dcode.rs) visits four Wi-Fi channels through frequency-table switching, reuses its NRX update, and owns CKGEN updates and eight measured codes. Requires initialized frequency tables; complete RX calibration remains a separate boundary. |
...
| TX DC power-detector calibration | IMPLEMENTED | [DC detector](src/tx/dc_power_detector.rs) owns the current Wi-Fi/BT root, finite search, initial and per-row PBus setup, bounded SAR observations and retained-register restoration. Compiled vendor comparisons cover DC results and ordered control effects with synthetic measurements; the combined RXCAL/TXCAL comparison also covers their parent integration; RF accuracy remains separate. |
...
| Current-archive measured RFPLL correction | IMPLEMENTED | [Measured correction](src/tracking/rfpll/README.md) owns the thermal gate, bounded search, signed frequency-memory update and typed frequency-control restoration. The outer RFPLL action and read-only inspection use this child; the connected service selects it ahead of other due work and rechecks sample freshness after admission. Compiled comparisons cover child effects and the RFPLL-enabled parent. A bounded HIL scenario covers the first nonzero heating correction and same-epoch traffic restoration; the opposite thermal direction, repeated cycles and radiated RF quality remain unqualified. Exclusive caller admission does not qualify joint coexistence grants. |
...
| Dated temperature observation | IMPLEMENTED | Runtime sensor completion records acquisition start/end in the runtime clock. Undated replacements invalidate freshness; values remain PHY sensor units. |
| Independent Wi-Fi maintenance operations | IMPLEMENTED | Temperature, Wi-Fi power/I2C, common and TX calibration reuse finite children and the complete physical restoration boundary. Unrelated periodic deadlines are preserved. |
| Connected observation-driven service | IMPLEMENTED | `RadioConfig` enables a one-second observation cadence by default and permits an explicit alternative or disable. The service waits on a deadline or coalesced observation event. Selected work requires exclusive Wi-Fi admission. Joint-radio grants, live-TX compensation and thermally safe deferral limits remain unqualified. |
...
| Periodic parameter/calibration tracking | IMPLEMENTED: connected-station observation service defaults to one second and selects the current Wi-Fi/RFPLL graph at role boundaries; complete thermal/RF qualification remains separate | PARTIAL: the retained client exposes read-only scheduling plus consuming due/periodic evaluation; Controller quiescence, target execution and restoration remain incomplete | PARTIAL: target tracking entry exists; RF/runtime service remains incomplete |
...
| Resume after RF sleep | IMPLEMENTED: the idle production supervisor composes final-client release, RF close/wake, Wi-Fi client reacquisition and stopped-owner reconstruction while retaining the registered PHY epoch; this immediate cycle is not scheduled modem sleep | ABSENT | ABSENT |
| Complete last-client RF/analog shutdown | IMPLEMENTED: the production Wi-Fi supervisor quiesces an idle role frontier, releases the final client, executes target RF close, restores the captured route-owned clock/reset baseline during cold-owner reunion, and can cold-restart with the final-state cache | PARTIAL: the registered Bluetooth client can release while preserving the exact last-client fact; Controller/BTBB/IRQ/timer quiescence, physical-owner reunion and RF close remain absent | ABSENT |
...
| RF wake from retained sleep | IMPLEMENTED | `RegisteredPhyRfClosed::wake_rf` executes the finite current-vendor wake parent against retained semantic calibration state and returns `RegisteredPhyPoweredIdle` only after terminal restoration. The Wi-Fi owner actor composes an immediate idle cycle through client reacquisition and RX restoration; the HIL scenario verifies repeated data-path recovery and registration-generation retention. Hardware evidence is still required for qualification. |
| RF sleep/power-down | PARTIAL | The last-client owner performs a default-profile pre-close temperature sample and exact finite current-vendor RF close. The production Wi-Fi supervisor can either wake immediately while retaining the registration epoch or continue through captured route-owned clock/reset restoration for a cold restart. Scheduled modem sleep and protocol power-save composition remain absent. |
...
retained wake path is also composed for an immediate idle Wi-Fi cycle, with a
dedicated repeated-cycle HIL scenario. Neither lifecycle supplies scheduled
modem sleep. Repeated connection and data-path HIL cover the Wi-Fi cold-restart
boundary; broad RF qualification remains a separate readiness requirement.
